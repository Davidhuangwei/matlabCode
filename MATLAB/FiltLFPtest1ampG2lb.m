...
for k=1:nfr
    ...
    for m=1:2
        ...
        for n=1:nlag
            ...
            [Ress, A(k,m,n)]=BasicRegression(log(abs(X)),log(abs(Y)));
            [NIP(k,m,n),NI(k,m,n)]=UInd_KCItest(log(abs(X)), Ress);
            fprintf('*')
        end
        fprintf('inv')
    end
...
datat=sprintf('compLFP%d-%d.%dHz.bfr%d-%d.RUN3.log.basictests.amp.Hpp.mat',Channels,cfrequency,bfr(nbfr,:));
