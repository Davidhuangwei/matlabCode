...
aa=55;%100;
bb=[30 40 50]';%[20,30,40]';%8,3,5,:5:3012;%
...
% BURSTS SELECTION 
Bursts=BurstTime((BurstChan>41 & BurstChan<45)& (BurstFreq>60& BurstFreq<120))*FS;
...
nlag=30;
cl=ceil(FS/aa);
tRes=nRes-randi(cl,length(nRes),1);
pl=zeros(nfr,2,nlag);
...
datat=sprintf('compLFP%d-%d.%dHz.basictests.hphase.mat',Channels,aa);
save(datat,'pci','pst','cst','cci','cpap','nte','me','ste','pval','h','Noi_dist','tRes','aa','nRes','FreqBins')%['8_9nitry', labels{ln}])% 724all bursts 
x=([1:nlag]-nlag)*stp;
y=1:nfr;
nc=3;
nl=2;
for m=1:2
    ...
    set(gca,'Ytick',y,'Yticklabel',num2str(FreqBins(:,2)))
...
%     load(['compLFP6-43.', num2str(sfr(mm)), 'Hz.basictests.hphase.mat'])
    ...
    subplot(nl,nc,(mm-1)*nc+1)
    hold on
    plot(x,sq(abs(pst(:,nn,:))),mks{nn})%,[0,.26]
    legend(num2str(FreqBins(:,2)))
        title('independency test stat')
        axis tight
        ylabel(['from', num2str(sfr(mm)), 'Hz'])
    subplot(nl,nc,(mm-1)*nc+2)
    hold on
    plot(x,sq(abs(nte(:,nn,:)))-sq(abs(me(:,nn,:))),mks{nn})
