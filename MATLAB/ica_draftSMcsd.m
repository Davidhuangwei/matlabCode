...
csd{k}=inactivity;%((inactivity(:,1:(end-2))-2*inactivity(:,2:(end-1))+inactivity(:,3:end)));
...
subplot(133);plot(HP,bsxfun(@plus,W_in/500,2*[1:size(W_in,2)]),'o-')
axis tight
grid on
set(gca,'XTick',HP,'XTickLabel',layer_show(HP))
[icasig, A, W] = fastica(activity_new','initGuess',inv(W));
W_in=u(:,temp_d)/W;
subplot(131)
plot(HP,bsxfun(@plus,W_in/500,2*[1:size(W_in,2)]),'o-')
axis tight
grid on
set(gca,'XTick',HP,'XTickLabel',layer_show(HP))
[weights,~,~,~,~,~,~] = runica(activity_new','sphering','off','stop',1e-4,'verbose','off','weights',W);%20);,'weights',weights{k-1}
W_in=u(:,temp_d)/weights;
subplot(132);plot(HP,bsxfun(@plus,W_in/500,[1:size(W_in,2)]),'o-')
grid on
axis tight
ncomp=size(icasig,1);
...
%     set(gca, 'YTick',HP,'YTicklabel',layer_show(HP))
% end
figure
ls={'o-','+-',':'};
for k=1:3
    subplot(121)
plot(HP,bsxfun(@plus,W_in(((k-1)*14+1):(k*14),:)/500,2*[1:size(W_in,2)]),ls{k})
hold on
axis tight
set(gca, 'XTick',HP,'XTicklabel',layer_show(HP))
subplot(122)
plot(HP,bsxfun(@plus,u(((k-1)*14+1):(k*14),1:size(W,1))*3,2*[1:size(W_in,2)]),ls{k})
hold on
axis tight 
set(gca, 'XTick',HP,'XTicklabel',layer_show(HP))
end
%%
icasig1=W*activity_new';
subplot(122);plot(HP,bsxfun(@plus,W_in*1000,2*[1:size(W_in,2)]),'o-')
axis tight
grid on
set(gca,'XTick',HP,'XTickLabel',layer_show(HP))
[icasig, A, W] = fastica(activity_new','initGuess',inv(W));
...
subplot(121)
plot(HP,bsxfun(@plus,W_in*1000,2*[1:size(W_in,2)]),'o-')
axis tight
grid on
set(gca,'XTick',HP,'XTickLabel',layer_show(HP))
% [weights,~,~,~,~,~,~] = runica(activity_new','sphering','off','stop',1e-4,'verbose','off','weights',W);%20);,'weights',weights{k-1}
