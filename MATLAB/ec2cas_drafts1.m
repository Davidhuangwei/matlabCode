...
for n=1:nlag
    ...
    c=abs(fLFP(tRes-stp*(nlag-n+1),1));
                [oci(n,:,1,k), ~]=...
                    KCIcausal_new(a,b,[c,abs(dd(tRes-stp*(nlag-n+1)-1)),real(Z)-real(filtLFP_temp(tRes-stp*(nlag-n+1)-1,1))],par,0,0);%abs(X),abs(Y),abs(Z)
                [oci(n,:,2,k), ~]=...
                    KCIcausal_new(a,b,[c,real(Z)-real(filtLFP_temp(tRes-stp*(nlag-n+1)-1,1))],par,0,0);%
                [oci(n,:,3,k), ~]=...
                    KCIcausal_new(fangle(X),fangle(Y),[fangle(Z),abs(dd(tRes-stp*(nlag-n+1)-1)),real(Z)-real(filtLFP_temp(tRes-stp*(nlag-n+1)-1,1))],par,0,0);%
