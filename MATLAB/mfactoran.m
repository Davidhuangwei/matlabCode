function [Lambda, Psi, T, stats, F] = mfactoran(X, m, varargin)
...
%   $Revision: 1.6.4.13 $  $Date: 2008/12/01 08:09:00 $
...
if ~(isscalar(delta) && isnumeric(delta) && (0<=delta) && (delta<1))
    error('stats:factoran:BadDelta',...
          'The ''delta'' parameter value must be a numeric scalar, 0 = delta < 1.');
end 
    ...
    if isa(rotate,'function_handle') % user-specified rotation
        try
            ...
            if strcmp('MATLAB:UndefinedFunction', ME.identifier) ...
                ...
                throw(addCause(MException('stats:factoran:RotationFunError',...
                      'Error evaluating rotation function ''%s''.',...
                      func2str(rotate)),ME));
            end
        end
    else % a string naming a built-in rotation
        [Lambda, T] = mrotatefactors(Lambda, 'method', rotate, rotateArgs{:});
    end
