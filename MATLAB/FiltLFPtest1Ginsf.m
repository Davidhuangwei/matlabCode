...
% MECS2=load('g09-20120330.SelectBursts4.[14-250].lfpinterp.RUN3..mat');41-48
load('g09-20120330.SelectBursts4.[14-250].lfpinterp.RUN3.17-32.mat')
...
% BURSTS SELECTION 4145
bsts=(BurstChan>19 & BurstChan<22)& (BurstFreq>60& BurstFreq<130);
...
cl=ceil(FS/60);
for tsh=1:2
tRes=repmat(nRes,2,1)-(tsh-1)*randi(cl,2*length(nRes),1);
pl=zeros(nfr,2,nlag);
...
%             if n<nlag(Clu<=FreqBins(k,2))(Clu<=FreqBins(k,2))(Clu<=FreqBins(k,2))(Clu<=FreqBins(k,2))(Clu<=FreqBins(k,2))
                X=abs(fangle(filtLFP_temp(tRes-stp*(nlag-n+1),2))-fangle(filtLFP_temp(tRes-stp*(nlag-n+1)-4,2)));
                Y=abs(fangle(filtLFP_temp(tRes,1))-fangle(filtLFP_temp(tRes-4,1)));
                Z=abs(fangle(filtLFP_temp(tRes-stp*(nlag-n+1),1))-fangle(filtLFP_temp(tRes-stp*(nlag-n+1)-4,1)));
...
datat=sprintf('compLFP%d-%d.%dHz.insfr.mat',Channels,aa);
