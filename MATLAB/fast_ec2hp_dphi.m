...
nRes=AllBursts.BurstTime(AllBursts.BurstChan<=8 & AllBursts.BurstChan>=5 ... % 21 175 29 28
...
fLFP=fangle(fLFP(2:end,:))-fangle(fLFP(1:(end-1),:));
...
figure(3);imagesc(reshape(CIHEs(:,2),8,4),[0 .05])
figure(2);imagesc(reshape(CIEHs(:,2),8,4),[0 .05])

figure(1);imagesc(reshape(CIHEs(:,1),8,4))
figure(4);imagesc(reshape(CIEHs(:,1),8,4))