...
%     filtLFP_temp=hilbert(filtLFP_temp);
    for m=1:2
        ...
        tRes=nRes;%-102+vind;
        for n=1:(2*nlag-1)
            lg=cov(filtLFP_temp(tRes-stp*(11-n),2),filtLFP_temp(tRes,1));
            pl(k,m,n)=lg(2,2)/sqrt(lg(1)*lg(end));%;
            if n<nlag
                oci=...
                    KCIcausal_new(filtLFP_temp(tRes-stp*(nlag-n),2),filtLFP_temp(tRes,1),[filtLFP_temp(tRes-stp*(nlag-n)-40,2),filtLFP_temp(tRes-stp*(nlag-n),1)],par,0,0);%[,fangle(filtLFP_temp(tRes-5*(nlag-n-1),1))]
            ...
            elseif n>nlag
                oci=...
                    KCIcausal_new(filtLFP_temp(tRes-stp*(nlag-n),2),filtLFP_temp(tRes,1),[filtLFP_temp(tRes-stp*(nlag-n)-floor(stp/2),2),filtLFP_temp(tRes+floor(stp/2),2)],par,0,0);%[,fangle(filtLFP_temp(tRes-5*(nlag-n-1),1))]
...
cd ~/data/sm/4_16_05_merged/
save 8_9nitryreal% 724all bursts 
if showr
    nc=3;
    nl=2;
    k=1:nl;
