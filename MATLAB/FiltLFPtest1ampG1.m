...
% BURSTS SELECTION 
Bursts=MB.BurstTime((MB.BurstChan>4 & MB.BurstChan<8)& MB.BurstFreq>60 & MB.BurstFreq<120)*FS;
...
for k=1:nfr
    ...
    for m=1:2
        ...
        a=-filtLFP_temp(:,1);
        b=-filtLFP_temp(:,2);
        tRes(:,1,k)=nRes-3-sl+clocalmin1(a(bsxfun(@plus,nRes,(-4-sl):(-5))),sl);%5
...
datat=sprintf('compLFP%d-%d.%dHz.RUN3.basictests.amp.pp.mat',Channels,cfrequency);
...
%     subplot(1,3,sstk);
    ...
    title(['RUN pp center on ', num2str(cfrequency), 'Hz'])