    ...
    for m=1:2
        ...
        for n=1:(nlag)
            ...
            if n<nlag
                ...
                Z=[(2-ln)*fangle(filtLFP_temp(tRes-stp*(nlag-n+1)-floor(stp/2),2))+(ln-1)*fangle(filtLFP_temp(tRes-stp*(nlag-n+1)+floor(stp/2),2)),fangle(filtLFP_temp(tRes-stp*(nlag-n+1),1))];
                [oci, Noi_dist{k,m,n}]=...
