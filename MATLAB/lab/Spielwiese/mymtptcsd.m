function [y, f, normmat, pow4coh, phloc, FST]=mymtptcsd(varargin)
...
for j=1:nFFTChunks
  ...
  ComputeCsd = zeros(nChannelsAll,1); 
  % a priory skip csd calculation
  ...
  if nargout>5 
    ...
    % f x K x nCh x nS
    PP(:,:,:,j) = Periodogram(:,:,:);
    %PPeven(:,:,:,j) = Periodogram(:,2:2:end,:);
  end
   
  % Now make cross-products of them to fill cross-spectrum matrix
  for Ch1 = 1:nChannelsAll
    % don't compute cross-spectra twice
    for Ch2 = Ch1:nChannelsAll 
      % don't do csd calculation if both signals don't fall into Segments
      if ComputeCsd(Ch1)  & ComputeCsd(Ch2) 
...
% compute the f-statistics to check for existance of significant
% sine wave
%fstat = FStats(PP,f,Tapers,1,WinLength);
FST = FStats(PP,f,Tapers,0,WinLength,0.1);
