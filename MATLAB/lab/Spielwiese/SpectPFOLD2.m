function Spect = SpectPF(FileBase,Eeg,PlaceCell,whl,spike,trial,varargin)
[REPFIG,PLOT,overwrite,CellType] = DefaultArgs(varargin,{0,0,0,0});
%celltype = 0 - do both, 1 - pyrs, 2- ints only
%mtptchd_segs 
x=Eeg;
nFFT = 2^13;
Fs = 1250;
...
if ~FileExists([FileBase '.spect']) | overwrite
  ...
  for neu=1:length(CatPlaceCell.indall)
    if CatPlaceCell.indall(neu,5) ==1 %pyramidal cells here
      if CellType == 2
	continue;
      end
      fprintf([num2str(neu) '... ']);
    ...
    elseif  CatPlaceCell.indall(neu,5) ==2  % deal with
                                            % interneurneurons
		fprintf('this one is interneuron\n');			    
	     if CellType ==1 
	       continue;
	     end
	
	     spikeid = (spike.ind==CatPlaceCell.indall(neu,1));
	     Res =  round(spike.t(spikeid)/16);
	  %  keyboard	     
	     RunPer = load([FileBase '.sts.RUN']);
	     TrialPer = round(trial.itv*EegRateFac);	     
	     %get the intersections of theta periods and the good
             %running trials
	     GoodPer = IntersectRanges(RunPer,TrialPer);
	     
	     [Starts, Assignments] = FitEvenlySpacedBins(2^11,GoodPer,Res);
	     Res = Res(Assignments>0);
	     Clu = ones(size(Res));
	     Starts = Starts(:);
	     Starts(Starts+2^11>length(x))=[]; % get rid of too long ones
	     WhlPer = round(([Starts Starts+2^11])/EegRateFac);
	     PerLen = abs(diff(whl.lin(WhlPer),1,2));
	     PerSpeed = PerLen/2^11*whl.rate; % sm/s ?
	     ...
	     Spect.ind(count,:) = [neu 0 0 CatPlaceCell.indall(neu,5)];
	     [ntr indsp] = histcI(PerSpeed,prctile(PerSpeed,[0 50 100]));
