...
for n=1:length(T)
  ...
  y=(1+cos(2*pi*440*(t-c(n)*T(n))))*0.5.* exp(-(t-T(n)).^2/(sig^2))/sqrt(pi)/sig;
  yy = yy+y;
end
yy = yy/max(yy);
...
yya=0;
for n=1:3
  ya=(1+cos(2*pi*440*(t-c(1)*Ta(n))))*0.5.* exp(-(t-Ta(n)).^2/(0.25^2));
  yya = yya+ya;
end
yya = yya/max(yya);
