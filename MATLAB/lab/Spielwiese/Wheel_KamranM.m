...
for f=file
  ...
  if ~FileExists([FileBase '.whl']) %| overwrite
    WhlFromCelldataKamran(FileBase)
  end
  whl = GetWhl(FileBase); 
  WhlCtr = whl.ctr;
  ...
  if ~FileExists([FileBase '.states']) | overwrite
    figure
    plot(whl.ctr);
    ask = input('Are there more than on state [1/0]\n');
    if ask
      cut = round(ginput(1));
      states.itv = [1 cut(1); cut(1)+1 size(whl.ctr,1)];
    else
      states.itv = [1 size(whl.ctr,1)];
    end
    states.info = {'1=sleep'; '2=run-track'; '3=run-openfield'};
    states.rate = WhlRate; %input('States are in which sampling rate? ');
    for n=1:size(states.itv,1)
      states.ind(n) = input('what state is this [1=sleep; 2=run-track; 3=run-openfield]? ');
    end
    save([FileBase '.states'], 'states')
  end
  load([FileBase '.states'], '-MAT')
  trial = GetTrials(FileBase,whl,overwrite,0,states.itv);
  dummy = CatStruct(trial,'itv');
  run = dummy.itv/WhlRate*EegRate;
  clear dummy;
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %% CCGs
  xcorrl = SVMUccg(FileBase,Eeg,spike,run,overwrite,0,[],EegRate,SampleRate);
  %[overwrite,PLOT,FileOut,EegRate,SampleRate] = DefaultArgs(varargin,{0,0,'.xcorr',1250,20000});
  IN = find(ctype.num==1);
  PC = find(ctype.num==2);
  r=xcorrl.rate;
  goodPC = PC(find(r(PC)>5));
  goodIN = IN(find(r(IN)>5));
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %% compute the spectra -- CA1
  %gPC = xcorrl.goodPC(find(ismember(xcorrl.goodPC,find(spike.clu(:,1)>=9))));
  %gIN = xcorrl.goodIN(find(ismember(xcorrl.goodIN,find(spike.clu(:,1)>=9))));
  gPC = goodPC(find(ismember(goodPC,find(spike.clu(:,1)>=9))));
  gIN = goodIN(find(ismember(goodIN,find(spike.clu(:,1)>=9))));
  ALL(f).spect = SVMUspect(FileBase,Eeg,spike,run,PrintBase,gPC,gIN,overwrite,PLOT,PRINTFIG,'.spectCA1',EegRate,SampleRate);
  %[goodPC,goodIN,overwrite,PLOT,PRINTFIG,FileOut,EegRate,SampleRate] = DefaultArgs(varargin,{[],[],0,0,0,'.spect',1250,20000});

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %% compute the spectra -- CA3
  gPC = goodPC(find(ismember(goodPC,find(spike.clu(:,1)<9))));
  gIN = goodIN(find(ismember(goodIN,find(spike.clu(:,1)<9))));
  ALL(f).spectM = SVMUspect(FileBase,Eeg,spike,run,PrintBase,gPC,gIN,overwrite,PLOT,PRINTFIG,'.spectCA3',EegRate,SampleRate);
  %[goodPC,goodIN,overwrite,PLOT,PRINTFIG,FileOut,EegRate,SampleRate] = DefaultArgs(varargin,{[],[],0,0,0,'.spect',1250,20000});
...
return;
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %% compute the spectra -- Long
  GG = spike.good;
  spike.good = spike.good.*WithinRanges(spike.t/SampleRate*states.rate,states.itv(1,:));
  idx = find(spike.good);
  
  ALL(f).spectL = SVMUspect(FileBase,Eeg,spike,run,PrintBase,goodPC,goodIN,overwrite,PLOT1,PRINTFIG,'.spectLong',EegRate,SampleRate);
  %[goodPC,goodIN,overwrite,PLOT,PRINTFIG,FileOut,EegRate,SampleRate] = DefaultArgs(varargin,{[],[],0,0,0,'.spect',1250,20000});
  spike.good = GG;
  %plot(spike.t(idx)/SampleRate,spike.ind(idx),'.')

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %% compute the spectra -- Short
  GG = spike.good;
  spike.good = spike.good.*WithinRanges(spike.t/SampleRate*states.rate,states.itv(2,:));
  idx = find(spike.good);
 
  ALL(f).spectS = SVMUspect(FileBase,Eeg,spike,run,PrintBase,goodPC,goodIN,overwrite,PLOT1,PRINTFIG,'.spectShort',EegRate,SampleRate);
  %[goodPC,goodIN,overwrite,PLOT,PRINTFIG,FileOut,EegRate,SampleRate] = DefaultArgs(varargin,{[],[],0,0,0,'.spect',1250,20000});
  spike.good = GG;
  %hold on
  %plot(spike.t(idx)/SampleRate,spike.ind(idx),'r.')
  %hold off
