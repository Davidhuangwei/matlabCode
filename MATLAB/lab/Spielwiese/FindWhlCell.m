function WhlCell=FindWhlCell(FileBase, spike, whl, trial, Eeg, varargin)
...
if ~FileExists([FileBase '.WhlCell']) | overwrite
  ...
  %keyboard
  %% speed - select segments of 1s (with overlap?)
  ...
  for n=1:length(neurons)
    ...
    title('place map including all spikes')
	...
	plotidx = idx;
      ...
      subplot(4,3,dire)
      plot(whl.itv([1:50:end],1),whl.itv([1:50:end],2),'o','markersize',5,'markerfacecolor',[1 1 1]*0.7,'markeredgecolor',[1 1 1]*0.7) 
      hold on
	...
	waitforbuttonpress
      ...
      subplot(313)
      plot(f,mean(yallEeg)/max(mean(yallEeg)))
      hold on
      plot(f,mean(yallCell)/max(mean(yallCell)))
