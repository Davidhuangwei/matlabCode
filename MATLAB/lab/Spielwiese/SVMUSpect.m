function spect = SVMUspect(FileBase,Eeg,spike,run,PrintBase,varargin)
[goodPC,goodIN,overwrite,PLOT,PRINTFIG,FileOut,EegRate,SampleRate,allPC,CluSubset,overwriteMUA] = DefaultArgs(varargin,{[],[],0,0,0,'.spect',1250,20000,[],[],0});
...
if ~FileExists([FileBase FileOut]) | overwrite
  ...
  if length(CluSubset)<50
    %[SP,f]=mtptcsd(Res,Clu,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,FreqRange,CluSubset,[],[],1);
    [SP,f,phi,err]=mtptchd(Res,Clu,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,FreqRange,CluSubset,[],[],1);
    if length(CluSubset)> size(SP,2)
      ...
      %eSP(:,:,1) = zeros(size(f,1),length(CluSubset)+1);
      %eSP(:,ii,1) = GetDiagonal(sq(err(:,:,:,1)));
      %eSP(:,:,2) = zeros(size(f,1),length(CluSubset)+1);
      %eSP(:,ii,2) = GetDiagonal(sq(err(:,:,:,2)));
    else
      cSP = GetDiagonal(SP);
    end
	...
	%[SP,f,phi,err]=mtptcsd(Res(idx),Clu(idx),nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,FreqRange,[],[],[],1);
	[SP,f,phi,err]=mtptchd(Res(idx),Clu(idx),nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,FreqRange,[],[],[],1);
	cSP(:,n) = SP;
	%eSP(:,n,1) = err(:,1,1,1);
	%eSP(:,n,2) = err(:,1,1,2);
      end
  ...
  %% x-corr between cells and eeg
  pEeg = SelectPeriods(Eeg,run,'c',1,1);
  [mSP,f]=mtptcsd(pEeg,Res,ones(size(Res)),nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,FreqRange,[],[],[],1);
  for n=1:size(ySP,2)
    spect.xcorr(:,n) = xcorr(ySP(:,n),mSP(:,1,1));
  end
...
if ~isfield(spect,'mua') | overwriteMUA
