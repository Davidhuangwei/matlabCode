function PlaceCell=FindPlaceFieldsAllOLD(FileBase, spike, whl, trial, states, varargin)
...
if ~FileExists([FileBase '.PlaceCellAll']) | overwrite
  ...
  while sts<length(states.ind)
	...
	goodwhl = [round(states.itv(min(goodsegs),1)/EegWhlRate):round(states.itv(max(goodsegs),2)/EegWhlRate)];
	ggwhl = find(whl.itv(goodwhl,1)>1); 
	xindx = find(spike.ind==neu & spike.good & spike.pos(:,1)>0 & WithinRanges(round(spike.t/20000*1250),states.itv(goodsegs,:)));
	if length(xindx) > 5000
	  indx = xindx(1:100:end);
	else
	  indx = xindx;
	end
      
	subplot(221)
	plot(whl.ctr(goodwhl(ggwhl(1:10:end)),1),whl.ctr(goodwhl(ggwhl(1:10:end)),2),'o', ...
	     'markersize',5,'markeredgecolor','none','markerfacecolor',[0.9 0.9 0.9]);
	hold on;
	scatter(spike.pos(indx,1),spike.pos(indx,2),30,(spike.ph(indx))*180/pi);
	hold off;
	%axis tight;
	grid on
	xlim([0 max(whl.ctr(goodwhl,1))]);
	ylim([0 max(whl.ctr(goodwhl,2))]);
	
	%% plot 2D rate
	subplot(222)
	%[PlaceMap, OccupancyMap] = myPlaceField(spike.t(xindx),whl.itv(goodwhl(ggwhl),:),300,5,50);
	[Rate, Occupancy, Spk, BinX, BinY] = PlaceMap(whl.ctr,spike.t(xindx),[min(goodwhl(ggwhl)) max(goodwhl(ggwhl))]);
	imagesc(BinX,BinY,Rate')
	axis xy
	grid on
	colorbar
	...
	%% plot autocorrelogram
	subplot(223)
	[ccg, t] = CCG(spike.t(xindx),1,20,50,20000);
	bar(t,ccg);
	xlim([min(t) max(t)]);
	grid on
