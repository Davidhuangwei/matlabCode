function SVMUPlotSpectGrp2(ALL,varargin)
[arg] = DefaultArgs(varargin,{[]});

GoPrint = 0;

%%% maze and wheel
CatMW = CatStruct(ALL(1:16));

%%% CA1/CA3
CatCA = CatStruct(ALL(17:30));
...
for n=1:16
  if isempty(ALL(n).compr)
    continue
  end
  if isfield(ALL(n).compr,'Ldt')
    mdl = ALL(n).compr.Ldt;
    mdt = ALL(n).compr.Sdt;
  
    [b stats] = robustfit(mdl,mdt);
    mc(n) = b(2);
    mcp(n) = stats.p(2);
    
    good = find(ALL(n).spectM.good);
    mf0ft(n) = median(ALL(n).spectPhM.maxfu(good));
  
  else
    %% maze
    mdl = ALL(n).compr.Mrunt;
    mdt = ALL(n).compr.Mtheta;
    ...
    %% wheel
    wdl = ALL(n).compr.Trunt;
    wdt = ALL(n).compr.Ttheta;
    
    [b stats] = robustfit(wdl,wdt);
    wc(n) = b(2);
    wcp(n) = stats.p(2);

    good = find(ALL(n).spectW.good);
    wf0ft(n) = median(ALL(n).spectPhW.maxfu(good));
  end
end
...
for n=17:30
  ...
  mdl = ALL(n).compr.runt;
  mdt = ALL(n).compr.theta;
...
ggm = find(mf0ft & mcp<0.05);
ggw = find(wf0ft & wcp<0.05);
%ggm = find(mf0ft & mcp<0.5);
%ggw = [];%find(wf0ft & wcp<0.01);

%% commented out to make subplot
figure(346);clf
%set(gcf,'position',[1922         528         426         348]);


subplot(131)
a1 = [(1-mc(ggm)) (1-wc(ggw))];
b1 = [mf0ft(ggm) wf0ft(ggw)];
a2 = [(1-mc(ggm))];
b2 = [mf0ft(ggm) ];
a3 = [(1-wc(ggw))];
b3 = [wf0ft(ggw) ];
%
%a = [(1-mc(ggm)) (1-wc(ggw))];
%b = [1./mf0ft(ggm) 1./wf0ft(ggw)];
gg = (a1.*b1)>0.6;
x1=a1(gg).*b1(gg)-1;
bin = [-0.1:0.03:0.1];
h=histcI(x1,bin);
bb=bar(bin(1:end-1)+0.01,h,1);
set(bb,'FaceColor',[1 1 1]*0.7)
meanx = mean(x1)
mean_YY = meanx
Lines(meanx,[],'r','--');
...
box off
set(gca,'FontSize',16,'TickDir','out','XTick',[-0.2:0.1:0.2])
xlabel('f_\theta/f_0 - (1-c)','FontSize',16)
ylabel('count','FontSize',16)
axis tight
xlim([-0.1 0.1])

PrintFig(['SpectExplGrp3'],0)
