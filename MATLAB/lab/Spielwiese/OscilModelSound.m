...
yya=0;
for n=1:length(Ta)
  ya=(1+cos(2*pi*440*(t-c*Ta(n))))*0.5.* exp(-(t-Ta(n)).^2/(sig^2));
  yya = yya+ya;
end
yya = yya/max(yya);
...
yyb=0;
for n=1:length(Tb)
  yb=(1+cos(2*pi*440*(t-c*Tb(n))))*0.5.* exp(-(t-Tb(n)).^2/(sig^2));
  yyb = yyb+yb;
end
yyb = yyb/max(yyb);
...
yy=0;
for n=1:3
  y=(1+cos(2*pi*440*(t-c*T(n))))*0.5.* exp(-(t-T(n)).^2/(0.25^2));
  yy = yy+y;
end
yy = yy/max(yy);


y1 = (1+cos(2*pi*440*t))*0.5.* exp(-(t-2.5).^2/(0.5.^2));
yp=cos(2*pi*440*(1-c)*t)/2;

%sound(yy,10000)

sound([yy yya+yyb],10000);


%WY = [yy-1 yya+yyb-1];
%fname = ['PopSound_c' num2str(c) '_fpop' num2str(ft) '.wav']
