...
plot([1:length(gtR)]/1252,MUA.rate(gtR),'color',cc1,'LineWidth',2)
axis tight
xlim([0 3])
set(gca,'TickDir','out','FontSize',16)
box off
...
plot([1:length(gtR)]/1252,MUA.eeg(gtR),'color',cc2,'LineWidth',2)
axis tight
%xlim([0 3])
set(gca,'TickDir','out','FontSize',16)
box off
...
imagesc(MUA.spT(gtSP)-a1,MUA.spF(gf),(MUA.spectgreeg(gtSP,gf)'))
axis xy
hold on
plot(MUA.spT(gtSP)-a1,MUA.mxeegF(gtSP),'o','markersize',5,'markerfacecolor',[1 1 1]*0,'markeredgecolor',[1 1 1]*0)
xlim([0 3])
...
fGSP = find(GSP);
[msp mi] = max(SP.spunit(ff,GSP));
[ma mai] = sort(msp);
%imagesc(SP.spunit(gf,fGSP(mai)))
pos = [0.8 0.65 0.5];cc = [73:75];
for n=1:3
  subplot('position',[0.75 pos(n) 0.2 0.15])
  %plot(MUA.spF(gf),x/max(x),'k--','LineWidth',2)
  hold on
  plot(SP.f(ff),SP.spunit(ff,fGSP(mai(cc(n))))/msp(mai(cc(n))),'k','LineWidth',2)
  [mx mt] = MaxPeak(SP.f(ff),SP.spunit(ff,fGSP(mai(cc(n)))));
