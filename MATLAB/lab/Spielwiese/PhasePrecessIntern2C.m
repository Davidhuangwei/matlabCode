function InterPhase = PhasePrecessIntern2C(FileBase,PlaceCell,spike,whl,trial,Tune,Rate,Eeg,ThRpPh,varargin)
...
for dire=unique(PlaceCell.ind(find(IN),4))';
  ...
  for n=1%1:length(fields)
    ...
    text(60,15,'P1','FontSize',16,'Color',ColPC(1,:))
    text(110,10,'P2','FontSize',16,'Color',ColPC(2,:))
    text(25,30,'IN','FontSize',16)
	...
	xlabel('time [ms]','FontSize',16)
      ...
      xlim([-80 80]);
      set(h1,'FontSize',16,'TickDir','out','YTick',[0 round(TextPos(m,2))])
    ...
    text(-795,14.75,'A','FontSize',20);
    text(-795,12,'B','FontSize',20);
    text(-170,14.75,'C','FontSize',20);
    text(-795,4.5,'D','FontSize',20);
    ...
    annotation('arrow',[0.43 0.4],[0.705 0.675],'HeadStyle','vback1','HeadLength',10,'LineWidth',2);
    annotation('arrow',[0.43 0.4],[0.705 0.675]-0.17,'HeadStyle','vback1','HeadLength',10,'LineWidth',2);
