...
%for n=1:length(T)
%  fc = 440;
%  sig = 1/(3*sqrt(2)*c(n)*f0);
%  fc = 350+n*10;
%  y=(1+cos(2*pi*f0*(t-c(n)*T(n))))*0.5.* exp(-(t-T(n)).^2/(sig^2)).*[1+cos(2*pi*fc*t)]/0.5*1/(sqrt(pi)*sig);
%  yy = yy+y;
%end
%yy = yy/max(yy);
...
for n=1:length(c)
  ...
  for m=1:length(T)
    fc = 440;
    sig = 1/(3*sqrt(2)*c(n)*f0);
    y1=(1+cos(2*pi*f0*(t-c(n)*T(m))))*0.5.* exp(-(t-T(m)).^2/(sig^2)).*[1+cos(2*pi*fc*t)]/0.5*1/(sqrt(pi)*sig);
    %y1=(1+cos(2*pi*f0*t))*0.5.* exp(-(t-T(n)).^2/(sig^2)).*[1+cos(2*pi*fc*t)]/0.5;
    yy1 = yy1+y1;
  end
  yy1 = yy1/max(yy1);
  yy = [yy yy1(gt)];
...
%y1 = (1+cos(2*pi*f0*t))*0.5.* exp(-(t-2.5).^2/(sig.^2)).*cos(2*pi*440*t);
y2 = (1+cos(2*pi*f0*t))*0.5;
y3 = repmat(y2(gt),1,length(c));

ft = f0.*(1-c)


%y8 = (1+cos(2*pi*8*t))*0.5.*[1+cos(2*pi*400*t)]*0.5;
%y9 = (1+cos(2*pi*9*t))*0.5.*[1+cos(2*pi*400*t)]*0.5;
%keyboard
...
WY = [yy-1];
fname = ['SoundDelay.wav']
wavwrite(WY,10000,32,fname)
...
%sig = 0.32; % [sec]
f0 = 240;    % [Hz]
cf = 0.2;%1/(3*sqrt(2)*sig*f0); % dimension less 
sig =1/(3*sqrt(2)*cf*f0);
