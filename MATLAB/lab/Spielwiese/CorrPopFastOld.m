function C = CorrPopFastOld(PlaceCell,spike,whl,trial,Rate,Tune,varargin)
...
for dire=unique(PlaceCell.ind(find(PC),4))';
  ...
  for neu=1:length(neurons)
	...
	continue;
      ...
      tune(:,1) = mean(Tune.rate(ttr(goodtr(slowP)),:,find(tneu==neurons(neu))),1)';
      tune(:,2) = mean(Tune.rate(ttr(goodtr(slowP)),:,find(tneu==neurons(neu2))),1)';
      [xtune xtunelag] = xcorr(tune(:,1),tune(:,2));
      [dummy xtunemax] = max(xtune);
      xDist(1) = xtunelag(xtunemax);
      tune(:,1) = mean(Tune.rate(ttr(goodtr(fastP)),:,find(tneu==neurons(neu))),1)';
      tune(:,2) = mean(Tune.rate(ttr(goodtr(fastP)),:,find(tneu==neurons(neu2))),1)';
      [xtune xtunelag] = xcorr(tune(:,1),tune(:,2));
      [dummy xtunemax] = max(xtune);
	...
	figure(286428);clf
	subplot(311)
	TuneE(:,1) = mean(Tune.rate(ttr(goodtr),:,find(tneu==neurons(neu))),1)';
	TuneE(:,2) = mean(Tune.rate(ttr(goodtr),:,find(tneu==neurons(neu2))),1)';
	plot(TuneE(:,1));
	hold on
	plot(TuneE(:,2),'r');
