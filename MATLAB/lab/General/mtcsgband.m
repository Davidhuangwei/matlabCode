...
y=complex(zeros(nFFTChunks, nChannels, nChannels)); % output array
    ...
    for Ch1 = 1:nChannels
        for Ch2 = 1:nChannels % don't compute cross-spectra twice
            ...
            if (Ch1 == Ch2)
                ...
                y(:,Ch1, Ch2) = mean(sq(tmpy(:,:,Ch1, Ch2)));
            else
                % for off-diagonal elements, scale
                y(:,Ch1, Ch2) = mean(abs(sq(tmpy(:,:,Ch1, Ch2)).^2) ...
                    ./ (sq(tmpy(:,:,Ch1,Ch1)) .* sq(tmpy(:,:,Ch2,Ch2))));
            end
        end
    end
