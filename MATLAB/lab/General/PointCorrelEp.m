% PointCorrelEp(T1, T2, BinSize, HalfBins, isauto, SampleRate, Normalization,fr2)
%
% Plots a cross-covarioram of 2 series (sshuflle corrected)
% 'scale' is modified to allow to use discontinuous T2 spike trains. 
% fr2 of T2 should be passed then as last argument
...
function [Out, t] = PointCorrelEp(T1, T2, BinSize, HalfBins, isauto, SampleRate, Normalization,fr2);
...
if (nargin<8 | isempty(fr2))
    fr2 = length(T2)/Trange;
end
switch Normalization
	...
	case 'count';
		AxisUnit = '(Spikes)';
	case 'scale'
		Histo = Histo *SampleRate / (BinSize * length(T1) * fr2);
		AxisUnit = '(Scaled)';
	otherwise
