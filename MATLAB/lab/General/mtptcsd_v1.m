...
BigPeriodogram = complex(zeros(nFFTChunks, nFreqBins, nTapers, nChannelsAll)); % ALL FFTs!!! this is new
...
for j=1:nFFTChunks
    ...
    if nChannels > 0
        ...
        if length(Segi_in)>length(Segi)/2
            ...
            BigPeriodogram(j,:,:,1:nChannels) = fftOut(select,:,:)*sqrt(2/nFFT);
    ...
    if ~isempty(SegmentId)
        ...
        BigPeriodogram(j, : , : ,  nChannels+1:nChannelsAll) = fftOut(select, : , :)*sqrt(2/nFFT);
        ...
        for Ch2 = Ch1:nChannelsAll % don't compute cross-spectra twice
            if ComputeCsd(Ch1)  & ComputeCsd(Ch2) % don't do csd calculation if both signals don't fall into Segments
               ...
               % also need to accumulate the power for cross-spectra
                ...
                %phase locking index
                phloc(:,Ch1,Ch2) = phloc(:,Ch1,Ch2) + sum(exp(i*(angle(Temp1)+angle(Temp2))),2);
