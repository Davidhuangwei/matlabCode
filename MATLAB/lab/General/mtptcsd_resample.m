function [y, f, normmat, pow4coh, phloc, y_shuffle]=mtptcsd(varargin)
...
y_shuffle=complex(zeros(nFreqBins, nChannelsAll, nChannelsAll, 100)); % output array Added by Nils
    ...
    for Ch1 = 1:nChannelsAll
        for Ch2 = Ch1:nChannelsAll % don't compute cross-spectra twice
            if ComputeCsd(Ch1)  & ComputeCsd(Ch2) % don't do csd calculation if both signals don't fall into Segments
                ...
                for shuffle = 1:size(y_shuffle,4) %Added by Nils
                    eJ = exp(1i*2*pi*rand(size(eJ))).*eJ;%Added by Nils
                    y_shuffle(:,Ch1,Ch2,shuffle) = y_shuffle(:,Ch1,Ch2,shuffle) + eJ;%Added by Nils
                end  %Added by Nils
                normmat(Ch1,Ch2) = normmat(Ch1,Ch2) + nTapers;
               % also need to accumulate the power for cross-spectra
                % normalization for unit-field and unit-unit case - to take into account
...
for Ch1 = 1:nChannelsAll
    for Ch2 = (Ch1+1):nChannelsAll % don't compute cross-spectra twice
        ...
        y_shuffle(:, Ch2, Ch1,:) = conj(y_shuffle(:,Ch1,Ch2,:)); %Added by Nils
...
normp_shuffle = repmat(normp,[1 1 1 size(y_shuffle,4)]);%Added by Nils
nz_shuffle = normp_shuffle>0;%Added by Nils
y_shuffle(nz_shuffle) = y_shuffle(nz_shuffle) ./ normp_shuffle(nz_shuffle); %Added by Nils
