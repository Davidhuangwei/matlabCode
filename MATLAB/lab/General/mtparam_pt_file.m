...
function [x, Res, Clu, nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,nChannels, nClu, nSamples,...
    nFFTChunks,winstep,select,nFreqBins,f,t,FreqRange,CluSubset,Segments,pval,MinSpikes] = mtparam_pt_file(FileBase,P)
...
elseif isempty(P{1}) & length(P{2}) == length(P{3})
   % have only point process
    ...
    nSamples = max(Res)+1;
    nInputs = 3;  
elseif ~isempty(P{1}) & (length(P{2}) == length(P{3})) & ~isempty(P{2})
    Res = P{2};
    Clu = P{3};
    x = P{1};
...
if (nargs<12 | isempty(P{12})) MinSpikes=2*nTapers; else MinSpikes = P{11}; end
