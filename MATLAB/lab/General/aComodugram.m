% Co = Comodugram2(x, nFFT, SampleRate, FreqRange, WinLength, NW, Detrend)
%
...
function Co = aComodugram(x, nFFT, SampleRate, FreqRange,WinLength, NW, Detrend)
...
if (nargin<5 | isempty (WinLength)) WinLength = nFFT; end;
if (nargin<6) NW = 3; end;
if (nargin<7) Detrend = []; end;
...
%	Spex = zeros(1 + nFFT/2, nTimeBins, nChannels);
	...
	for i=1:nChannels
		[Spex(:,:,i), f, t] = mtcsg(x(:,i), nFFT, SampleRate, WinLength);
	end;
else
	Spex = x;
	f = nFFT;
...
% 		if (nargout<1)
    %			subplot(nChannels, nChannels, j + (i-1) * nChannels);
 			imagesc(f(FreqBins), f(FreqBins), C);
% 		end;
