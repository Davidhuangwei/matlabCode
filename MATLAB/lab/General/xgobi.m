...
function [colorsOut, glyphsOut, OriginalRow] = xgobi(data, colors, glyphs, labels)
...
if isstruct(data)
	...
	for i=1:length(FieldNames)	
        ...
        if ~isnumeric(d) | ndims(d)>2 | min(size(d))>1
            continue
        end
        nPoints(i) = length(d);
		d2(1:nPoints(i),i) = d(:);
...
%	ColorIndex = 1+colors-min(colors);
	...
	for i=1:length(ColName)
        if i>nColors
            colind = mod(i,nColors);
            if colind==0 colind=nColors; end
        else
            colind = i;
        end
        if isnumeric(ColName(i))
    		fprintf('Color %d -> %s\n', ColName(i), ColorNames(colind,:));
        else
    		fprintf('Color %s -> %s\n', ColName(i), ColorNames(colind,:));
        end
	end
	ColorIndex(ColorIndex>nColors) = mod(ColorIndex(ColorIndex>nColors),nColors);
    ColorIndex(ColorIndex ==0) = nColors;
...
eval(['! /u12/antsiro/bin/xgobi ', FileBase, '&'])
...
if nargout>=1
	...
	Map(i) = find(strcmp(UniqueNames(i), ColorNamesUnsort));
    end
