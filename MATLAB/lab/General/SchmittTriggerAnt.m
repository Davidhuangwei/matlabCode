...
% originally Ken's ..  potentially usefull additions by Anton
function [UpCross DownCross]= SchmittTrigger(Signal,UpThresh,DownThresh)
...
UpCross = UpDownSort(find(PrevType==-1 & TypeSort==1));
DownCross = UpDownSort(find(PrevType==-1 & TypeSort==1)+1);


%PrevType = [-1; TypeSort(1:length(TypeSort)-1)];