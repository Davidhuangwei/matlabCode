...
function [histres,histerr,t] = PointCorrelS(T1, T2, BinSize, HalfBins, isauto, SampleRate, Normalization, RepeatNum,color,type);
...
% figure
 if nargout==0
     if type=='l'
     plot(t,histres,color);
 else
    bar(t,histres,color);
 end
     hold on
     plot(t,histerr,strcat('g:'));
     plot(t,-histerr,strcat('g:'));
     plot(t,0,'k');
     axis tight
