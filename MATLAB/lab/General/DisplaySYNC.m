...
while 1,
    ...
    while 1,
        ...
        % now plot AF deleted and added spots for video
        finddeleted = find(AFdeletedspots1>sync1(first) & AFdeletedspots1<sync1(last1));
        if ~isempty(finddeleted),
            plot(AFdeletedspots1(finddeleted), zeros(length(finddeleted),1),'x','markersize',10,'linestyle','none','color',[0 0 0]);
        end    
        findadded = find(AFaddedspots1>sync1(first) & AFaddedspots1<sync1(last1));
        if ~isempty(findadded),
            plot(AFaddedspots1(findadded), zeros(length(findadded),1),'+','markersize',10,'linestyle','none','color',[0 1 1]);
        end    
        ...
        plot(sync2(first:last2)*ratio+shift,0.1*ones(last2-first+1,1),...
            ...
            % now plot AF deleted and added spots for eeg
        finddeleted = find(AFdeletedspots2>sync2(first) & AFdeletedspots2<sync2(last2));
        if ~isempty(finddeleted),
            plot(AFdeletedspots2(finddeleted)*ratio+shift, 0.1+zeros(length(finddeleted),1),'x','markersize',10,'linestyle','none','color',[0 0 0]);
        end    
        findadded = find(AFaddedspots2>sync2(first) & AFaddedspots2<sync2(last2));
        if ~isempty(findadded),
            plot(AFaddedspots2(findadded)*ratio+shift, 0.1+zeros(length(findadded),1),'+','markersize',10,'linestyle','none','color',[0 1 1]);
        end    
        ...
        elseif action == 'a' | action == 'A',
            ...
            if (action == 'a' & y < 0.05) | (action == 'A' & ~isempty(syncToAdd)),
                ...
                p = plot(x,0.1,'o','color',[1 0 0],'markersize',16);
        ...
        elseif action == 'F',
            ...
            fprintf('\n\nAUTOFIX iteratively deletes and adds pulses when adjacent pulses are closer or farther apart than the median.');
            fprintf('Deleted points are displayed in black. Added points are displayed in cyan.\n');
            AFinput = [];
            while ~strcmp(AFinput,'v') & ~strcmp(AFinput,'e')
                AFinput = input('Would you like to run AUTOFIX on the video (''v'') or the eeg pulse (''e'')?','s');
            end
            if AFinput =='v'
                    ...
                    while (j < m1)
                        if (((sync1(j+1)-sync1(j)) >= (MedSync1Dif*tolerance)) & ((sync1(j+1)-sync1(j)) <= (MedSync1Dif/tolerance)))
                            ...
                            while ((j < m1) & ((sync1(j+1)-sync1(j)) < (MedSync1Dif*tolerance)))
                                m1=m1-1;
                                AFdeletedspots1 = [AFdeletedspots1 sync1(j+1)];
                                sync1(j+1) = [];
                            end
                            while ((j < m1) & ((sync1(j+1)-sync1(j)) > (MedSync1Dif/tolerance)))
                                ...
                                m1=m1+1;
                                m2 = m1;
                                AFaddedspots1 = [AFaddedspots1 x];
                            end
                        end
                    end
                    fprintf('\nAdded %i points\nDeleted %i points\n\n',length(AFaddedspots1),length(AFdeletedspots1));
                ...
                MedSync2Dif = median(diff(sync2(1:end)));
                tolerance = 0.95 ;
                if (median(diff(sync2(2:5))) < median(diff(sync2(1:end)))*tolerance |  median(diff(sync2(2:5))) > median(diff(sync2(1:end)))/tolerance)
                    fprintf('\n\n******** The median number of frame between the 2nd-5th spot (%d) is not similar to the median number of frames between all the spots in the file (%d)...',median(diff(sync2(2:5))),median(diff(sync2(1:end))));
                    fprintf('i.e there is a problem and you should attempt some manual cleaing before running autofix\n');
                    while 1,
                        goforit = input('do you want to continue? yes/no: ', 's');
                        if strcmp(goforit,'yes') | strcmp(goforit,'no'), break; end
                ...
                if goforit(1) == 'y'
                    ...
                    j=2; % skip the first point because it's usually off
                    while (j < m1)
                        if (((sync2(j+1)-sync2(j)) >= (MedSync2Dif*tolerance)) & ((sync2(j+1)-sync2(j)) <= (MedSync2Dif/tolerance)))
                            j=j+1;
                        else
                            while ((j < m1) & ((sync2(j+1)-sync2(j)) < (MedSync2Dif*tolerance)))
                                m1=m1-1;
                                AFdeletedspots2 = [AFdeletedspots2 sync2(j+1)];
                                sync2(j+1) = [];
                            end
                            while ((j < m1) & ((sync2(j+1)-sync2(j)) > (MedSync2Dif/tolerance)))
                                x = sync2(j) + MedSync2Dif;
                                sync2 = [sync2(1:j);x;sync2(j+1:end)];
                                j=j+1;
                                m1=m1+1;
                                m2 = m1;
                                AFaddedspots2 = [AFaddedspots2 x];
                            end
                        end
                    end
                    fprintf('\nAdded %i points\nDeleted %i points\n\n',length(AFaddedspots2),length(AFdeletedspots2));
                    % Update automatic preselections so hitting A or D doesn't confuse the user
                    [syncToDelete,syncToAdd] = GetSuggestions(sync2,max([j-1 1]));
                end
            end
