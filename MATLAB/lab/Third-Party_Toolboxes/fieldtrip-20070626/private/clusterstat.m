...
elseif strcmp(cfg.clusterthreshold, 'nonparametric_individual')
  ...
  % each individual voxel will get its own threshold
  ...
  for i=1:Nsample
    postailobs(i) = (sum(statrnd(i,:)>statobs(i))/Nrand) <= cfg.clusteralpha/correcttail;
    negtailobs(i) = (sum(statrnd(i,:)<statobs(i))/Nrand) <= cfg.clusteralpha/correcttail;
    tmp = statrnd(i,:);
    [srt,ind] = sort(tmp);
    postailrnd(i,ind) = (1:-1/Nrand:1/Nrand) <= cfg.clusteralpha/correcttail;
    negtailrnd(i,ind) = (1/Nrand:1/Nrand:1)  <= cfg.clusteralpha/correcttail;
  end

elseif strcmp(cfg.clusterthreshold, 'nonparametric_common')
  % threshold based on bootstrap using all other randomizations
  % all voxel will get a common threshold
