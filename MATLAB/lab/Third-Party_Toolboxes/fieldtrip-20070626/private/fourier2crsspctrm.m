...
for j = 1:length(chn)
  chnindx(j,1) = strmatch(chn{j}, freq.label);
end
...
for j = 1:size(chncmb,1)
  cmbindx(j,1) = strmatch(chncmb{j,1}, freq.label);
  cmbindx(j,2) = strmatch(chncmb{j,2}, freq.label);
end
...
progress('init', 'textbar', 'computing single-trial power-spectral densities');
for j = 1:Nrpt
...
progress('init', 'textbar', 'computing single-trial cross-spectral densities');
for j = 1:Nrpt
  progress(j/Nrpt, 'trial %d/%d\n', j, Nrpt);
...
cfg.version.id = '$Id: fourier2crsspctrm.m,v 1.11 2006/06/23 10:49:33 jansch Exp $';
