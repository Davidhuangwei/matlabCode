...
% construct the randomized design
...
if strcmp(cfg.correctm, 'cluster')
  ...
  elseif strcmp(cfg.clusterthreshold, 'parametric') && isempty(cfg.clustercritval)
    ...
    tmpcfg.computecritval = 'yes';  % explicitely request the computation of the crtitical value
...
for i=1:Nrand
  ...
  randdesign = squeeze(res(i,:,:,:,:,:));
  if size(randdesign,1)==size(dat,2)
    randdesign = transpose(randdesign);
  end
  if strcmp(cfg.correctm, 'cluster')
    % keep each randomization in memory for cluster postprocessing
    dum = statfun(cfg, dat, randdesign);
    if isstruct(dum)
    ...
    statrand = statfun(cfg, dat, randdesign);
