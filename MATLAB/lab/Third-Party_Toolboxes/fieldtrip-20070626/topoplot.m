...
% cfg.highlight       = 'off' or the electrodenumbers you want to highlight (default = 'off') 
...
% try to detect EEGLAB-style input and give an informative error message
if mod(length(varargin),2)==0 && length(varargin)>1 && isvector(varargin{1}) && isstruct(varargin{2}) && isfield(varargin{2}, 'labels')
  % the input resembles the input of the EEGLAB topoplot function
  error('Unrecognized input, please look at "help topoplot", "which topoplot" and "path". The input looks as if you expect the EEGLAB topoplot function. Your path settings may be incorrect and the FieldTrip and EEGLAB version of the "topoplot" function may be confused.')
end

% deal with the different types of input syntax that this function can get
if rem(nargin,2)==1 && isnumeric(varargin{1}) && isstr(varargin{2})
