...
if ischar(cfg.pad)
  if strcmp(cfg.pad,'maxperlen')
    ...
    for i = 1: length(data.trial)
      dat_length(i)=length(data.trial{i});
    end
    max_smp = max(dat_length+data.offset);
...
if strcmp(cfg.keeptrials,'no') &&  strcmp(cfg.keeptapers,'yes')
...
for foilop = 1:numfoi
  ...
  elseif strcmp(cfg.taper, 'sine')
    ...
    tap = tap./norm(tap);
    % freqanalysis_mtmfft always throws away the last taper of the Slepian
    % sequence, so add a dummy taper
    tap(:,2) = nan;
  end
  numtap(foilop) = size(tap,2)-1;
  if (numtap(foilop) < 1)
    error(sprintf(...
      '%.3f Hz : datalength to short for specified smoothing\ndatalength: %.3f s, smoothing: %.3f Hz, minimum smoothing: %.3f Hz',...
      cfg.foi(foilop), acttapnumsmp/data.fsample, cfg.tapsmofrq(foilop), data.fsample/acttapnumsmp));
  elseif (numtap(foilop) < 2)
  ...
  for taplop = 1:numtap(foilop)
    try
      knlspctrmstr{foilop}(taplop,:) = ...
        fft(complex(vertcat(prezer,tap(:,taplop).*cos(ind),pstzer), ...
        vertcat(prezer,tap(:,taplop).*sin(ind),pstzer)),[],1)';
    end
  end
end
if keep == 1
...
for perlop = 1:numper
  ...
  % avoid the use of a 3rd input argument to facilitate compatibility with starp
  ...
  for foilop = 1:numfoi
    ...
    for taplop = 1:numtap(foilop)
      ...
      if fftflg
        ...
        if keep == 1 && numacttimboi > 0
          fourierspctrm(:,foilop,acttimboiind) = ...
            fourierspctrm(:,foilop,acttimboiind) + ...
            reshape((fourierdum ./ numtap(foilop)),[numsgn,1,numacttimboi]);
        elseif keep == 2 && numacttimboi > 0
          fourierspctrm(cnt,:,foilop,acttimboiind) = ...
            fourierspctrm(cnt,:,foilop,acttimboiind) + ...
            reshape(fourierdum ./ numtap(foilop),[1,numsgn,1,numacttimboi]);
          fourierspctrm(cnt,:,foilop,nonacttimboiind) = nan;
        elseif keep == 4 && numacttimboi > 0
          fourierspctrm(cnt,:,foilop,acttimboiind) = ...
            reshape(fourierdum,[1,numsgn,1,numacttimboi]);
          fourierspctrm(cnt,:,foilop,nonacttimboiind) = nan;
        elseif (keep == 4 || keep == 2) && numacttimboi == 0
          fourierspctrm(cnt,:,foilop,nonacttimboiind) = nan;
        end
      ...
      if csdflg
        ...
        if keep == 1 && numacttimboi > 0
          crsspctrm(:,foilop,acttimboiind) = ...
            crsspctrm(:,foilop,acttimboiind) + ...
            reshape((csddum ./ numtap(foilop)),[numsgncmb,1,numacttimboi]);
        elseif keep == 2 && numacttimboi > 0
          crsspctrm(cnt,:,foilop,acttimboiind) = ...
            crsspctrm(cnt,:,foilop,acttimboiind) + ...
            reshape(csddum ./ numtap(foilop),[1,numsgncmb,1,numacttimboi]);
...
cfg.version.id = '$Id: freqanalysis_mtmconvol.m,v 1.37 2007/03/27 11:00:28 ingnie Exp $';
