function outStruct = LoadGlmResults01(depVar,chanLocVersion,fileExt,inNameNote,animalDirs,varargin)

[dirname selChanBool saveDir reportFigBool] = DefaultArgs(varargin,{'GlmWholeModel07',0,'MazePaper/new/',1});
...
for k=1:length(animalDirs)
    for m=1:length(animalDirs{k})
        ...
        if  selChanBool == 0
            ...
            selChanNames = fieldnames(selChanStruct);
            for n=1:length(selChanNames)
                selChan{n} = ['.ch' num2str(selChanStruct.(selChanNames{n}))];
                selChanNums = selChanStruct.(selChanNames{n});
            end
        end
        for n=1:length(selChan)
            fprintf('\nLoading %s',[dirname '/' inNameNote '/' fileExt '/' depVar selChan{n} '.mat'])
            load([dirname '/' inNameNote '/' fileExt '/' depVar selChan{n} '.mat']);
...
%                 end
                for q=1:length(chanLoc.(anatFields{p})) % num shanks
                    ...
                    if ~isempty(goodChans)
                        meanObs{k}(m,n,p,q) = mean(numObs(goodChans));
                    else
                        meanObs{k}(m,n,p,q) = NaN;
                    end
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.coeffs.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.rSqs.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.pVals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.residNormPs.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.residMeanPs.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                mean(numObs(goodChans) .* tempData(r,goodChans));
                        else
                            outStruct.residMeanPs.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                NaN;
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.contDwPvals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                mean(numObs(goodChans) .* tempData(r,goodChans));
                        else
                            outStruct.contDwPvals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                NaN;
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.categDwPvals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                mean(numObs(goodChans) .* tempData(r,goodChans));
                        else
                            outStruct.categDwPvals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                NaN;
                    ...
                    for r=1:length(varNames);
                        if ~isempty(goodChans)
                            outStruct.prllPvals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                mean(numObs(goodChans) .* tempData(r,goodChans));
                        else
                            outStruct.prllPvals.(GenFieldName(varNames{r})){k}{n,p}(q,m) = ...
                                NaN;
