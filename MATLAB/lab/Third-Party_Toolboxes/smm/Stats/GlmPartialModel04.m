function GlmPartialModel04(dataDescription,analRoutine,outNameNote,fileBaseMat,fileExt,depVar,varargin)
%function GlmPartialModel04(description,fileBaseMat,fileExt,midPointsBool,minSpeed,winLength,thetaNW,gammaNW,w0,depVar,contIndepCell,varargin)
%[nChan,freqBool,maxFreq,nonParamBool,adjDayMedBool,adjDayZbool] = DefaultArgs(varargin,{96,0,150,0,0,0});
[nChan,freqBool,maxFreq,nonParamBool,midPointsBool,minSpeed,winLength] ...
    = DefaultArgs(varargin,{96,0,150,0,1,0,626});
...
for ch=1:nChan
    ...
    for f=find(fs<=maxFreq)
        ...
        yVar = depData;
        if trialMeanBool
            contXvarNames = [{'ones'}, {'trialMean'}, contIndepCell];
            contXvar = [ones(size(contData,1),1),trialMeanDep,contData];
        else
        ...
        if length(partialModelSpec) == 1
            modelSize = 0;
            for j=1:partialModelSpec
                modelSize = modelSize + nchoosek(length(anovaCategData),j);
            end
        else
            modelSize = size(partialModelSpec,1);
        end
        ...
        if ~nonParamBool
            ...
            [model.pVals(size(contXvar,2)+1:size(contXvar,2)+modelSize,ch,f), model.anovaTable{ch,f}, categStats(ch,f)] = ...
                anovan(contStats(ch,f).resid,anovaCategData,'varnames', categNames,'sstype',ssType,'model',partialModelSpec,'display','off');
        else
        ...
        % ssqTotal = sum((yVar-mean(yVar)).^2);
        % for j=1:length(contXvarNames)
        % ssqModel(j,1) = sum((2.*contStats(ch,f).beta(j).*contXvar(:,j)-contStats.yhat).^2);
        % end
        ...
        for j=2:size(table,1)-2
            ...
            model.rSqNames{j-1} = table{j,find(strcmp(table(1,:),'Source'))};
        end
        model.varNames(size(contXvar,2)+1:size(contXvar,2)+length(model.rSqNames)) = model.rSqNames;
...
infoStruct.modelSpec = partialModelSpec;
