function PlotGlmScat08(chanLocVersion,depVarCell,fileExt,inNameNote,animalDirs,varargin)
...
for jj=1:length(depVarCell)
    switch depVarCell{jj}
        ...
        case 'gammaPowIntg40-120Hz'
            depVar = 'gammaPowIntg40-120Hz'; categMeansYlim = [-2 2]; selChanNums = 0;
        case 'gammaPowIntg40-100Hz'
            depVar = 'gammaPowIntg40-100Hz'; categMeansYlim = [-2 2]; selChanNums = 0;
        case 'gammaPowIntg50-120Hz'
            depVar = 'gammaPowIntg50-120Hz'; categMeansYlim = [-2 2]; selChanNums = 0;
        case 'gammaPowIntg50-100Hz'
            depVar = 'gammaPowIntg50-100Hz'; categMeansYlim = [-2 2]; selChanNums = 0;
...
for j=1:length(plotNames)
    ...
    for k=1:length(anatNames)%length(anatNames):-1:1
        ...
        allTtestData = cat(1,allTtestData, ttestData);
        groupNames = cat(1,groupNames,repmat(anatNames(k),size(ttestData)));
    end
    set(gca,'fontsize',5,'ytick',[-length(anatNames):-1],'yticklabel',flipud(anatNames))
    %     for k=1:length(anatNames)
    %         get(gca,'ylim')
    %          end
    if ttestBool & ~strcmp(plotNames{j},'Constant')
%         [anovaP,AT,stats] = anova1(allTtestData,groupNames,0);
