function PlotGlmCoh02(chanLocVersion,fileExt,inNameNote,dirname,reportFigBool)
    ...
    for j=1:length(selChanNums)
        for k=1:length(animalDirs)
            ...
            for n=1:length(anatFields)
                %%%%%%% Betas %%%%%%%%
                numMeas = length(setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))));
                varNames = model.coeffNames;
                for m=1:length(varNames);
                    %varNames{m}
                    %try 
                        coeffs.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                            model.coeffs(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                    %catch keyboard
                    %end
                    if strcmp(varNames{m},'Constant')
                        constantCoeff.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                            model.coeffs(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                    end
                end
                %%%%%% categMeans %%%%%%%%
                if ~isempty(model.categMeans)
                    for q = 1:length(model.categMeans)
                        ...
                        for m=1:length(varNames);
                            ...
                            %                                 squeeze(model.categMeans{q}(m,1,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))-...
                            %                                 mean(model.categMeans{q}(:,1,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))));
                            categMeans{q}.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                                squeeze(model.categMeans{q}(m,1,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j)))));
                            %- mean(model.categMeans{q}(:,1,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))));
                        end
                    end
                ...
                for m=1:length(varNames);
                    assumPlot.residNormPs.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                        tempData(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                    %                 if m==1
                    %                 size(tempData(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))')
                    %                 j
                    %                 end
                end
                %%%% residMeanPs %%%%%
                ...
                for m=1:length(varNames);
                    assumPlot.residMeanPs.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                        tempData(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                end
                %                 %%%%%% residNormPs %%%%%%%%
                ...
                %                     residNormPs.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                %                         temp.(varNames{m})(setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))));
                %                 end
                %                 %%%%%% residMeanPs %%%%%%%%
                %                 varNames = fieldnames(assumTest.residMeanPs);
                %                 for m=1:length(varNames);
                %                     temp = MatStruct2StructMat2(assumTest.residMeanPs);
                %                     residMeanPs.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                %                         temp.(varNames{m})(setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))));
                %                end
                %%%% contDwPvals %%%%%
                ...
                for m=1:length(varNames);
                    assumPlot.contDwPvals.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                        tempData(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                end
                %%%% categDwPvals %%%%%
                ...
                for m=1:length(varNames);
                    assumPlot.categDwPvals.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                        tempData(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                end
                %%%% prllPvals %%%%%
                ...
                for m=1:length(varNames);
                    assumPlot.prllPvals.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                        tempData(m,setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))))';
                end
                %%%%%% prllPvals %%%%%%%%
                ...
                %                         else
                %                             assumPlot.prllPvals.(GenFieldName(varNames{m})).(anatFields{n}).(animalDirs{k}(13:18))(1:numMeas,j) = ...
                %                                 assumTest.prllPvals(setdiff(chanLoc.(anatFields{n}),union(badChans,selChans(selChanNums<j))));
                %                         end
            end
        end
    end
    varType = 'phase';
    [mu.coeffs p.coeffs] = CalcMuP(coeffs,0,0,varType);
    if ~isempty(model.categMeans)
        for q = 1:length(model.categMeans)
            [mu.categMeans{q}] = CalcMuP(categMeans{q},1,0,varType,constantCoeff.Constant);
        end
    end
%     [subtjunk] = CalcMuP(categMeans{1},1,0,varType,constantCoeff.Constant);
%     [junk] = CalcMuP(categMeans{1},1,0,varType);
%     keyboard
%    % function [mu p] = CalcMuP(analData,transMuBool,log10Bool,depVarType,constantCoeff)
% junkConst = CalcMuP(constantCoeff,1,0,varType);

    coeffNormP = CalcNormP(coeffs);
    mu.residNormPs = CalcMuP(assumPlot.residNormPs,0,1,varType);
    mu.residMeanPs = CalcMuP(assumPlot.residMeanPs,0,1,varType);
    mu.contDwPvals = CalcMuP(assumPlot.contDwPvals,0,1,varType);
    mu.categDwPvals = CalcMuP(assumPlot.categDwPvals,0,1,varType);
    mu.prllPvals = CalcMuP(assumPlot.prllPvals,0,1,varType);
