function GlmWholeModel05(dataDescription,analRoutine,outNameNote,fileExt,depVar,varargin)
%function GlmWholeModel05(description,fileBaseMat,fileExt,midPointsBool,minSpeed,winLength,thetaNW,gammaNW,w0,depVar,contIndepCell,varargin)
%[nChan,freqBool,maxFreq,nonParamBool,adjDayMedBool,adjDayZbool] = DefaultArgs(varargin,{96,0,150,0,0,0});
[phaseMeanBool,nChan,freqBool,maxFreq,midPointsBool,minSpeed,winLength] ...
    = DefaultArgs(varargin,{0,96,0,150,1,0,626});
if midPointsBool
...
if equalNbool
    ...
    for j=1:length(fields)
        nTrials(j) = size(depStruct.(fields{j}),1);
    end
    shortestLen = min(nTrials);
    for j=1:length(fields)
        indexes = randsample(size(depStruct.(fields{j}),1),shortestLen);
        depStruct.(fields{j}) = depStruct.(fields{j})(indexes,:,:);
        dayStruct.(fields{j}) = dayStruct.(fields{j})(indexes,:,:);
    ...
    for j=1:length(contIndepCell)
        if ~strcmp(contVarSub{j},'jibe')
            for k=1:length(contVarSub{j})
                ...
                elseif strncmp(contVarSub{j}{k},'selCh',5)
...
for ch=1:nChan
    ...
    for f=find(fs<=maxFreq)
        ...
        if adjDayMedBool
            ...
            goodDepStruct = DayNormalize(goodDepStruct,goodDayStruct,adjDayZbool);
            catch
                fprintf('\nproblem with DayNormalize(goodDepStruct,goodDayStruct,adjDayZbool)\n')
                keyboard
            end
        end
        keepContCellStruct{:,ch,f} = goodContCellStruct;
        keepDepStruct(ch,f) = goodDepStruct;
        keepDayStruct(ch,f) = goodDayStruct;
...
save(outName,SaveAsV6,'infoStruct','dayStruct','depStruct','contCellStruct', ...
    'keepContCellStruct','keepDepStruct','keepDayStruct','modelStats', ...
