function PlotGlmCoh11(depVarCell,spectAnalDir,fileExt,chanLocVersion,analRoutine,varargin)
%function PlotGlmCoh11(depVarCell,fileExt,chanLocVersion,analRoutine,varargin)
% [analDirs betaClim reportFigBool printFigBool statsDir saveDir bonfFactor statAlpha nonSigSat nonSigVal] = ...
...
[analDirs betaClimCell reportFigBool printFigBool statsDir saveDir bonfFactor statAlpha nonSigSat nonSigVal] = ...
    DefaultArgs(varargin,{analDirs,[],0,0,'GlmWholeModel08','REMPaper/',1,0.001,0.35,0.35});
...
%     [mu stDev pVal] = GlmResultsCalcMuStdP(plotData,[],bonfFactor);
    [mu stDev pVal] = GlmResultsCalcMedC95P(plotData,bonfFactor);
...
%     mu = GlmResultsAddConst(transMu,-0.5);
    ...
    %%%%%%% AddConstTransBeta %%%%%%%%
    plotData = dataStruct.coeffs;
    titlesExt = 'AddConstTransBeta';
    colorLimits = [betaClimCell];
    [mu stDev pVal] = GlmResultsCalcMedC95P(plotData);
    tempMu = GlmResultsAddConst(mu,mu.Constant);
    tempMu.Constant = mu.Constant;
    transMu = GlmResultsUnTransform(tempMu,depVarType);
    mu = GlmResultsSubtractConst(transMu,transMu.Constant);
    mu.Constant = transMu.Constant;
    nextFig = LocalImageHelper(nextFig,mu,betaPval,fileName,titlesExt,anatNames,selChanNames,colorLimits,statAlpha,nonSigSat,nonSigVal);
...
%%%%% rSqs %%%%%%
    ...
    if printFigBool
        if length(fNames) == 2
            PrintSaveFig([depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_subTransMu'],4);
%             print(4,'-depsc2',[depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_subTransMu.eps']);
%             print(4,'-dpng',[depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_subTransMu.png']);
        else
            PrintSaveFig([depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_transMu'],3);
%             print(3,'-depsc2',[depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_transMu.eps']);
%             print(3,'-dpng',[depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_transMu.png']);
        end
        PrintSaveFig([depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_pVal'],2);
%         print(2,'-depsc2',[depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_pVal.eps']);
%         print(2,'-dpng',[depVarCell{a} '_' analRoutine Dot2Underscore(fileExt) '_bf' Dot2Underscore(num2str(bonfFactor)) '_pVal.png']);
    end
...
%      keyboard
     lower = tril(ones(size(plotTemp)));
     lower(~lower) = NaN;
%     ImageScPvalDots(plotTemp.*lower,log10(valueData.(analNames{j})),cLimits,-5)
    if isempty(valueData) | strcmp(analNames{j},'Constant')
        ...
        ImageScHSV(plotTemp,satMask,valMask,[cLimits]);
        title(SaveTheUnderscores([titleBase, ' ' analNames{j}]),'fontsize',3);
        set(gca,'ylim',[0.5 length(yTickLabels)+0.5])
...
%         keyboard
%     set(gca,'ylim',[0.5 length(yTickLabels)+2.5])
    end
    
    set(gca,'xlim',[0.5 length(xTickLabels)+0.5])
    set(gca,'fontsize',3,'ytick',[1:length(yTickLabels)],'yticklabel',yTickLabels)
    set(gca,'fontsize',3,'xtick',[1:length(xTickLabels)],'xticklabel',xTickLabels)
    set(gca,'fontsize',3,'ytick',[1:length(yTickLabels)],'yticklabel',yTickLabels)
    set(gca,'fontsize',3,'xtick',[1:length(xTickLabels)],'xticklabel',xTickLabels)
% %     clf
%     maxPval = 5
%     c = rsmak('circle');
%     hold on
%     for m=1:size(valueData.(analNames{j}),1)
%         for n=1:size(valueData.(analNames{j}),2)
%             if n<=m
%                 if ~isnan(valueData.(analNames{j})(m,n))
%                 radius = 0.10/maxPval*floor(-1*log10(valueData.(analNames{j})(m,n))) + 0.05;
%                 sc = fncmb(c,radius);
%                 fnplt(fncmb(sc,[m; n]),'color','w');
%                 end
%             end
%         end
%     end
