function PlotGlmCoh08(depVarCell,spectAnalDir,fileExt,chanLocVersion,analRoutine,varargin)
%function PlotGlmCoh08(depVarCell,fileExt,chanLocVersion,analRoutine,varargin)
% [analDirs betaClim reportFigBool dirname saveDir statAlpha nonSigSat nonSigVal] = ...
%     DefaultArgs(varargin,{analDirs,[],1,'GlmWholeModel08','REMPaper/',0.001,0.3,0.3});
analDirs = {...
...
[analDirs betaClimCell reportFigBool dirname saveDir statAlpha nonSigSat nonSigVal] = ...
    DefaultArgs(varargin,{analDirs,[],1,'GlmWholeModel08','REMPaper/',0.001,0.3,0.3});
...
%     alpha = baseAlpha/(nLayers*(nSelChan+1)/2);
    ...
    [mu stDev pVal] = GlmResultsCalcMuStdP(plotData,log10Bool,depVarType);
    nextFig = LocalImageHelper(nextFig,GlmResultsCalcLog10(pVal),[],fileName,titlesExt,anatNames,selChanNames,colorLimits,statAlpha,nonSigSat,nonSigVal);
    %%%%%%% categMeans %%%%%%%%
    plotData = dataStruct.categMeans;
    titlesExt = 'categMeans';
    log10Bool = 0;
    colorLimits = [];
    [mu stDev pVal] = GlmResultsCalcMuStdP(plotData,log10Bool,depVarType,1);
