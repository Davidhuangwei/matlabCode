function GlmWholeModel08(spectAnalDir,analRoutine,outNameNote,fileExt,depVar,varargin)
% function GlmWholeModel08(spectAnalDir,analRoutine,outNameNote,fileExt,depVar,varargin)
% [phaseMeanBool,nChan,freqBool,maxFreq,saveInputData] ...
[phaseMeanBool,nChan,freqBool,maxFreq,saveInputData] ...
    = DefaultArgs(varargin,{0,96,0,150,0});

warnState(1) = warning('OFF', 'stats:regress:RankDefDesignMat');
warnState(2) = warning('OFF', 'MATLAB:divideByZero');
...
outDir = [mfilename '/' analRoutine '_' outNameNote '/' spectAnalDir fileExt '/'];
outName = [mfilename '/' analRoutine '_' outNameNote '/' spectAnalDir fileExt '/' depVar '.mat'];
...
for j=1:length(warnState)
    warning(warnState(j).state,warnState(j).identifier);
end
