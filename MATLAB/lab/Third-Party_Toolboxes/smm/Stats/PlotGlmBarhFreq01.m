function PlotGlmBarFreq01(chanLocVersion,depVarCell,fileExt,inNameNote,animalDirs,varargin)
...
for analBattery=1:length(depVarCell)
    switch depVarCell{analBattery}
        ...
        case 'powSpec.yo'
            depVar = 'powSpec.yo'; categMeansYlim = [-4 4]; selChan = 0;
...
%                     permute(tempData(setdiff(chanLoc.(anatFields{j}),badChans),:),[2,3,1]);
%             end
            %%%% residCategVarZs %%%%%
    ...
    ttestBool = 1;
    nextFig = ScatPlotHelper(nextFig,plotData,titlesExt,fileExt,log10Bool,yLimits,chanLocVersion,plotColors,fileName,ttestBool,selChanLoc);
    %%%%%%%% plot categMeans %%%%%%%%
    if 0
...
function nextFig = ScatPlotHelper(nextFig,plotData,titlesExt,fileExt,log10Bool,xLimits,chanLocVersion,plotColors,fileName,ttestBool,selChanLoc)
resizeWinBool = 1;
baseAlpha = 0.01;
% chanInfoDir = 'ChanInfo/';
% chanMat = LoadVar([chanInfoDir 'ChanMat' fileExt '.mat']);
...
%plotNames=fieldnames(plotData);
fo = LoadField('sm9603m2_212_s1_255/CalcRunningSpectra9_noExp_MidPoints_MinSpeed0Win626_LinNear.eeg/powSpec.fo');
...
%     end
        ...
        for m=1:length(animalNames)
            if ~isempty(plotData.(plotNames{j}).(anatNames{k}).(animalNames{m}))
                ...
                if exist('selChanLoc','var') & ~isempty(selChanLoc)
                    ...
                    hold on
                    if log10Bool
                        plot(fo(1:size(plotData.(plotNames{j}).(selChanLoc).(animalNames{m}),2)),...
    ...
    if ~exist('selChanLoc','var') | isempty(selChanLoc)
        ...
        xTickLabels(xTickLabels>fo(size(meanTemp,2))) = [];
        xTicks = [];
        for n=1:length(xTickLabels)
            xTicks(n) = find(abs(fo-xTickLabels(n)) == min(abs(fo-xTickLabels(n))),1);
        end
        ...
        if strcmp(plotNames{j},'Constant')
            ...
            plot(get(gca,'xlim'),zeros(size(get(gca,'xlim'))),'--','markersize',20,'color',[0.5 0.5 0.5])
            if ttestBool
                hPlotTemp = yLimits(2)*ones(1,size(h,2))-0.01;
                plot(fo(find(h(strcmp(anatNames,selChanLoc),:))),hPlotTemp(logical(h(strcmp(anatNames,selChanLoc),:))),'r*','markersize',5 )
            end
        end
    end
end
