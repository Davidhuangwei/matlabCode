function [categPvalStruct contPvalStruct] = CalcDurbinWatson(yVar,residuals,categData,contData,contNames)
...
%%%% contPvalStruct %%%%%
%%% calc dw sorted by size of y
[sortedCategData sortIndexes] = sort(yVar);
data = residuals(sortIndexes);
sigu = data'*data;
ediff = data(2:end) - data(1:end-1);
contDWStruct.depVar = (ediff'*ediff)/sigu; % durbin-watson
shuffDW = [];
for k=1:nRandSamp
...
contPvalStruct.depVar = ...
    (1+length(find(shuffDW>=contDWStruct.depVar)))/length(shuffDW);
