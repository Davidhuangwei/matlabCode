function [pValStruct betas] = TestParallelGroup2(depStruct,IndepStruct,varargin)
...
[indepNames, nRandSamp] = DefaultArgs(varargin,...
    {{'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p'},1000});
...
keyboard
for i=1:nGroups
    results = ols(depCell{i,end},indepCell{i,end});
    aovMat(i,:) = [results.nobs results.beta results.bstd];
 end
...
for i=1:size(varShuffBetas,2)
    pValStruct.(Dot2Underscore(indepNames{i})) = (1+length(find(varShuffBetas(:,i)>=varBetas(i))))/length(varShuffBetas);
end
