...
for j=1:length(stims)
    datSegs(:,:,j) = bload([filebase '/' filebase '.dat'],[nchannels timewindow*datsampl],stims(j)*nchannels*bps);
...
segs = datSegs;
save([filebase '/' 'StimTrigSegs_' notes '.dat.mat'],SaveAsV6,'segs','time','infoStruct');
...
infoStruct.extrapEegBool = extrapEegBool;
segs = interp1CSD;
save([filebase '/' 'StimTrigSegs_' notes '_NearAveCSD1.csd.mat'],SaveAsV6,'segs','time','infoStruct');
...
infoStruct.extrapEegBool = extrapEegBool;
segs = interp121CSD;
save([filebase '/' 'StimTrigSegs_' notes '_LinNearCSD121.csd.mat'],SaveAsV6,'segs','time','infoStruct');
else
end
return
