function [outSpec fo to coi outSeg] = LoadTrigStdSpec(animalDirs,filesNameCell,...
...
for k=1:length(animalDirs)
    for m=1:length(animalDirs{k})
        ...
        for j=1:length(filesNameCell)
            sleepFiles = cat(1,sleepFiles,LoadVar(['FileInfo/' filesNameCell{j}]));
        end
...
try        % Sum Files w/in a day
        for j=1:length(sleepFiles)
             inFile = [sleepFiles{j} '/' trigFileBase fileExt '.mat'];
...
%                 tempSpec = tempSpec + 10.^((inSpec.yo)/10) * inSpec.numSeg / safetyFactor;
                tempSpec = tempSpec + (inSpec.yo).^2 * inSpec.numSeg ;
                numSegFiles = numSegFiles + inSpec.numSeg ;
            end
