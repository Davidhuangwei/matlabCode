function PlotRippPowerChangeDist(fileBaseCell,rippFileName,normFileName,eegExt,varargin)
...
[firstFig,plotSize,offset,colorLimits] = DefaultArgs(varargin,...
    {1,[10 8],offset,[]});
        ...
        for j=1:length(fileBaseCell)
            ...
            inName = [fileBase '/' rippFileName eegExt '.mat'];
            fprintf('Loading: %s\n',inName)
            load(inName);
...
for j=1:size(selectedSegs,1)
    rippPow(j) = 10.*log10(mean(mean(filter0(firfiltb,squeeze(selectedSegs(j,:,:))).^2,1),2));
        ...
        for j=1:length(fileBaseCell)
            fileBase = fileBaseCell{j};
            inName = [fileBase '/' normFileName eegExt '.mat'];
            fprintf('Loading: %s\n',inName)
            load(inName);
...
% end
for j=1:size(selectedSegs,1)
    normPow(j) = 10.*log10(mean(mean(filter0(firfiltb,squeeze(selectedSegs(j,:,:))).^2,1),2));
end

rippPow = normPow - rippPow;

figure(firstFig)
clf
set(gcf,'name',['RippPower_' rippFileName '-' normFileName eegExt])
% keyboard
...
imagesc(1:size(chanMat,2)',-1:-1:-size(chanMat,1)',Make2DPlotMat(interpData,chanMat))
% shading faceted
