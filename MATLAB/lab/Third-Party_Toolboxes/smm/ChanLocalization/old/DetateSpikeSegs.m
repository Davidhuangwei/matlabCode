function DetateSpikeSegs(fileBaseMat,trigExt,trigNChan,molTrigChan,hilTrigChan,fileExt,nChan,varargin)
...
for k=1:size(fileBaseMat,1)
    ...
    for j=1:length(combIndexes)
        try segs = bload([fileBase '/' fileBase fileExt], [nChan winLen*sampRate],...
            (combIndexes(j)-round(winLen*sampRate/2))*nChan*bps);
        catch
        end   
    end
    time = combIndexes/sampRate;
    infoStruct.fileBase = fileBase;
    outName = [fileBase '/' 'RippTrigSegs_RippTrigChan' num2str(rippTrigChan) '_SwTrigChan' num2str(swTrigChan) fileExt '.mat'];
    fprintf('Saving: %s\n',outName);
...
%         try molSegs(:,:,j) = bload([fileBase '/' fileBase trigExt], [trigNChan winLen*sampRate],...
%             (molIndexes(j)-winLen*sampRate/2)*trigNChan*bps);
%         catch
%         end
%     end
%     hilSegs =[];
%     for j=1:length(hilIndexes)
%         try hilSegs(:,:,j) = bload([fileBase '/' fileBase trigExt], [trigNChan winLen*sampRate],...
%                 (hilIndexes(j)-round(winLen*sampRate/2))*trigNChan*bps);
%         catch
%         end
%     end
