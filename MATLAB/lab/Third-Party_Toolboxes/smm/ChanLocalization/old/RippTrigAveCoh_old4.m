            ...
            for p=1:length(segRestrictionCell)
                ...
                for r=1:size(selChan,1)
                    ...
                    for n=1:size(ripp.segs,3)
                        for ch=1:size(ripp.segs,1) %% calculate the wavelet spectrum
                            [wave(ch,:,:) period scale] = wavelet(squeeze(ripp.segs(ch,:,n)),dt,pad,DJ,S0,J1,mother,wavParam);
                        end
                        sinv=1./(scale');
                        smoothWave = zeros(size(wave));
                        for ch=1:size(ripp.segs,1) % smooth wavelet spectrum for coherence calculation
                            temp = abs(squeeze(wave(ch,:,:))).^2;
                            smoothWave(ch,:,:) = smoothwavelet(sinv(:,ones(1,size(wave,3)))...
