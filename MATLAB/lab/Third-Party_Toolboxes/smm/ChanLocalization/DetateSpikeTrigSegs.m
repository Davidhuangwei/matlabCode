function DetateSpikeTrigSegs(fileBaseCell,trigExt,trigNChan,molTrigChan,hilTrigChan,fileExt,nChan,varargin)
...
for k=1:length(fileBaseCell)
    ...
    for j=1:length(combIndexes)
        try segs(:,:,j) = bload([fileBase '/' fileBase fileExt], [nChan segLen],...
            (combIndexes(j)-round(segLen/2))*nChan*bps);
        catch
        end   
    end
