function PlotRippTrigAveSpec(animalDirs,fileExt,segLen,segRestriction,varargin)
chanLocVersion = DefaultArgs(varargin,'Min');

[outSpec fo to coi outSeg] = LoadRippTrigAveSpec(animalDirs,fileExt,segLen,segRestriction,chanLocVersion);
[stdSpec fo to coi outSeg2] = LoadRippTrigStdSpec(animalDirs,fileExt,segLen,segRestriction,chanLocVersion);
...
figTitleBase = ['RippTrigAveSpec_' segRestriction num2str(segLen) fileExt];
...
%     set(gca,'clim',[-5 5])
    ...
    colorbar
    if j==1
        title(cat(2,titleBase,'meanPowSpec'))
    end
end
