...
[x,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers,nChannels,nSamples,nFFTChunks,winstep,select,nFreqBins,f,t] = mtparam_sm(varargin);

% allocate memory now to avoid nasty surprises later
y=complex(zeros(nFreqBins, nFFTChunks, nChannels)); % output array

for Ch=1:nChannels
	[y(:,:,Ch) f t] = mtcsglong(x(:,Ch),nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers);
end
