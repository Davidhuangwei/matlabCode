function [y, f]=CalcMazeRegionSpectrum(fileBaseMat,fileext,nChannels,Channels,Fs,nFFT,WinLength,NW,plotBool,removeExp,trialTypesBool,mazeLocationsBool)
    ...
    while (firstIndex < whlm),
        ...
        if whldat(windowCenter,1)~=-1,
            ...
            for j=1:length(Channels)
                ...
                y(:,Channels(j)) = (y(:,Channels(j))*t + y1(:,1)*WinLength)/(t+WinLength); % average spectrum from current segment with previous segments
...
if removeExp,
    for i=1:length(Channels)
        [beta,y(:,Channels(i))] = hajexpfit(f,log(y(:,(i))));
    end
end
