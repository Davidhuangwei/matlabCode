...
for m=1:size(spatialComodData,1)
    for n=1:size(spatialComodData,2)
        if size(spatialComodData{m,n})>1 %| size(spatialComodData{m,n},3)==1
            %fprintf('m=%i,n=%i; ',m,n)
            for j=1:length(channels)
                ...
                try
                    %keyboard
                    [b,bint,r,rint,stats] = regress(squeeze(spatialComodData{m,n}(1,channels(j),:)),...
                        [ones(size(squeeze(spatialComodData{m,n}(2,channels(j),:)))) squeeze(spatialComodData{m,n}(2,channels(j),:))],0.05);
