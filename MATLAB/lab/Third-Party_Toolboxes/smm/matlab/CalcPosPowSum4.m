...
function [sumPerPosStruct,fo,channelNums] = CalcPosPowSum4(taskType,fileBaseCell,fileExt,fileNameFormat,Channels,...
    winLength,nOverlap,NW,freqRange,peakBool,autoSave,trialTypesBool,mazeLocationsBool)
...
for h=1:length(fileBaseCell);
    cd(fileBaseCell{h})
    if iscell(trialTypesBool)
        for j=1:length(trialTypesBool)
            tempWhlDat = LoadMazeTrialTypes(fileBaseCell{h},trialTypesBool{j},mazeLocationsBool{j},1);
            if j==1
                whlDat = tempWhlDat;
            else
                notMinusOnes = find(tempWhlDat(:,1) ~= -1);
                whlDat(notMinusOnes,:) = tempWhlDat(notMinusOnes,:);
            end
        end
    else
