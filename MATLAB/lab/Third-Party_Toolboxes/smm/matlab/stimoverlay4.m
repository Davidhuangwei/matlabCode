function stimoverlay4(filebase,begintime,endtime,channels,autosave,notes)
...
datsampl=20000;
eegsampl = 1250;
data = readmultiSM([filebase '.dat'],nchannels,channels(1));
...
set(gca,'xlim',[0 2*xdisp], 'ylim', [-traceOffset*(length(channels)+1) 0]);
set(gca, 'xtick', [0*xdisp 0.25*xdisp 0.5*xdisp 0.75*xdisp 1*xdisp 1.25*xdisp 1.5*xdisp 1.75*xdisp 2*xdisp], 'xticklabel', [-1*xdisp/eegsampl -0.75*xdisp/eegsampl -0.5*xdisp/eegsampl -0.25*xdisp/eegsampl 0*xdisp/eegsampl 0.25*xdisp/eegsampl 0.5*xdisp/eegsampl 0.75*xdisp/eegsampl 1*xdisp/eegsampl]);
grid on;


datstims = datstims + begintime*datsampl-1; % convert values to correspond to full file
stims = round(datstims*eegsampl/datsampl); 
figure(3)
clf;
xdisp = 50*datsampl/1000; 
traceOffset = 10000;
for i=1:length(datstims)
    dataseg = bload([filebase '.dat'],[nchannels, xdisp],datstims(i)*2*nchannels,'int16');
    for j=1:length(channels)
        hold on; 
        plot(dataseg(channels(j),:)-j*traceOffset,'color', [mod(j,2) 0 0]);
    end
end
title([filebase ': channels ' num2str(channels(1)) '-'  num2str(channels(end)) ' - ' notes]);
set(gca,'xlim',[1 xdisp], 'ylim', [-traceOffset*(length(channels)+1.5) 0.5*traceOffset]);
set(gca, 'xtick', [0*xdisp+1  0.25*xdisp+1 0.5*xdisp+1 0.75*xdisp+1 xdisp+1], 'xticklabel', [0*xdisp*1000/datsampl 0.25*xdisp*1000/datsampl 0.5*xdisp*1000/datsampl 0.75*xdisp*1000/datsampl 1*xdisp*1000/datsampl]);
grid on;
