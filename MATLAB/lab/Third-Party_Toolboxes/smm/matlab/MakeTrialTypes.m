function MakeTrialTypes(varargin)
analDirs = {...
...
% spectAnalDir = 'RemVsRun04_noExp_MinSpeed5wavParam6Win6250';
spectAnalDir = 'RemVsRun04_allTheta_MinSpeed5wavParam6Win1250';
[taskTypeName filesName analDirs fileExtCell spectAnalDir] = DefaultArgs(varargin,...
    {'','',analDirs,fileExtCell,spectAnalDir});

for j=1:length(analDirs)
    cd(analDirs{j})
    files = LoadVar(['FileInfo/' filesName]);
...
%             end           
            ...
            if strcmp(in,'y')
                taskType = repmat({taskTypeName},length(times),1);
