...
spectraFileName = [spectrogramDir fileBaseMat(1,:) fileExt 'Win' num2str(winLength) 'Ovrlp' num2str(nOverlap) 'NW' num2str(NW) '_mtpsg_' num2str(Channels(1)) '.mat'];
        ...
        for m=1:size(mazeRegionNames,1)
            ...
            if isempty(notMinusOnes)
                ...
                for k=1:length(Channels)
                    ...
                    for j = 1:length(fo)
                        if dbBool
                            yFreqPos(j,:,k) = Accumulate(linearRLaverage(notMinusOnes),10.*log10(yo(j,floor((notMinusOnes-1)/whlSamp*toSamp+1))),maxMazeX);
                        else
                            yFreqPos(j,:,k) = Accumulate(linearRLaverage(notMinusOnes),yo(j,floor((notMinusOnes-1)/whlSamp*toSamp+1)),maxMazeX);
                        end
