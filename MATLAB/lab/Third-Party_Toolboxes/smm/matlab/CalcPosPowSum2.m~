% function [sumPerPosStruct,fo,channelNums] = CalcPosPowSum2(taskType,fileBaseMat,fileExt,fileNameFormat,Channels,...
...
for h=1:size(fileBaseMat,1);
    ...
    if isempty(notMinusOnes)
        ...
        for k=1:length(Channels)
            spectraFileName = [spectrogramDir fileBaseMat(h,:) fileExt 'Win' num2str(winLength) ...
...
if i(1) == 'n'
    ...
    fileNamesInfo = GetFileNamesInfo(fileBaseMat,fileNameFormat);
    peak = [];
    if peakBool
        peak = '_peak';
    end
    ...
    sumPerPosStruct = setfield(sumPerPosStruct,'info','fileBaseMat',fileBaseMat);
