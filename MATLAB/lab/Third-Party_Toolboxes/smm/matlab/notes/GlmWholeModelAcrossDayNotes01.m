function GlmWholeModelAcrossDayNotes01(analDirs,spectAnalBase,analRoutine,outNameNote,fileExt,depVar,varargin)
...
[phaseMeanBool,nChan,freqBool,maxFreq,midPointsBool,minSpeed,winLength,saveInputData,chanLocVersion] ...
    = DefaultArgs(varargin,{0,96,0,150,1,0,626,0,'Full'});
if midPointsBool
...
cwd = pwd;
for numAnal=1:length(analDirs)
    cd(analDirs{numAnal});
    %%%%%% load TrialDesig %%%%%%
    %    load(['TrialDesig/' mfilename '/' analRoutine '.mat'])
    load(['TrialDesig/' 'GlmWholeModel07' '/' analRoutine '.mat'])
    ...
    for k=1:length(contCellStruct)
        contCellCell{k} = Struct2CellArray(contCellStruct{k},[],1);
    end
    if length(analDirs)>1
        oldChanLoc = LoadVar([analDirs{numAnal} 'ChanInfo/ChanLoc_' chanLocVersion fileExt '.mat']);
        oldSelChan = LoadVar([analDirs{numAnal} 'ChanInfo/SelChan' fileExt '.mat']);
        oldBadChan = load([analDirs{numAnal} 'ChanInfo/BadChan' fileExt '.txt']);
        anatLayers = fieldnames(oldChanLoc);
        for m=1:length(anatLayers)
            nShanks = length(oldChanLoc.(anatLayers{m}));
            for n=1:nShanks
                %newChanNum = (m-1)*nShanks+n;
                chans = setdiff(oldChanLoc.(anatLayers{m}){n},union(oldSelChan.(anatLayers{m}),oldBadChan));
                if ~isempty(chans)
                    for j=1:size(depCell,1)
                        if exist('mappedDepCell','var') & size(mappedDepCell,1)>=m &...
                                size(mappedDepCell,2)>=n & length(mappedDepCell{m,n})>=j
                            mappedDepCell{m,n}{j}= cat(1,mappedDepCell{m,n}{j},...
                                squeeze(mean(depCell{j,end}(:,chans,:),2)));
                            for k=1:length(contCellStruct)
                                mappedContCell{k}{m,n}{j} = cat(1,mappedContCell{k}{m,n}{j},...
                                    contCellCell{k}{j,end});
                            end
                        else
                            mappedDepCell{m,n}{j}= squeeze(mean(depCell{j,end}(:,chans,:),2));
                            for k=1:length(contCellStruct)
                                mappedContCell{k}{m,n}{j} = contCellCell{k}{j,end};
                            end
                        end
                        if ~isempty(mappedDepCell{m,n}{j})
                            goodChans(m,n,j) = 1;
                        end
                    end
                end
...
newChan = 1;
for m=1:size(mappedDepCell,1)
    selChan.(anatLayers{m}) = size(mappedDepCell,1)*size(mappedDepCell,2)+m;
    for n=1:size(mappedDepCell,2)
        if goodChans(m,n,:)
            depCellStruct{newChan} = CellArray2Struct(cat(2,depCell(:,1:end-1), mappedDepCell{m,n}'));
            for k=1:length(contCellStruct)
                contCellCellStruct{k}{m,n} = CellArray2Struct(cat(2,contCellCell{k}(:,1:end-1),mappedContCell{k}{m,n}'));
            end
            chanLoc.(anatLayers{m}){n} = newChan;
            newChan = newChan+1;
        else
...
if ~exist(['ChanInfo/ChanLoc_' chanLocVersion fileExt '.mat'],'file')
    save(['ChanInfo/ChanLoc_' chanLocVersion fileExt '.mat'],SaveAsV6,'chanLoc')
end
if ~exist(['ChanInfo/SelChan' fileExt '.mat'],'file')
    save(['ChanInfo/SelChan' fileExt '.mat'],SaveAsV6,'selChan')
end
if ~exist(['ChanInfo/BadChan' fileExt '.txt'],'file')
    save(['ChanInfo/BadChan' fileExt '.txt'],'-ascii','badChan')
end
...
if exist('equalNbool','var') & equalNbool==1
    for j=1:length(depCellStruct)
            data = EqualizeN(cat(2,depCellStruct(j),contCellStruct),outlierDepth);
            depCellStruct{j} = data{1};
    end
end
...
for ch=1:nChan
    ...
    for f=1:length(fo)
        ...
        try depStructSub = GetStructMatSub(depCellStruct{ch},[2,f]);
        catch
...
infoStruct.chanLoc = chanLoc;
infoStruct.selChan = selChan;
infoStruct.badChan = badChan;
infoStruct.nChan = nChan
infoStruct.spectAnalBase = spectAnalBase;
