function CheckCohereMean_6_26_06(animal, allFreqBool)
...
for j=1:length(selChans)
    for k=1:length(fields)
        selChanNames{j} = ['ch' num2str(selChans(j))];
        for n=1:max(chanMat(:))
            [p(n) th0(n) r(n)] = RayleighTest(gammaPhase.(selChanNames{j}).(fields{k})(:,n));
        end
        subplot(length(selChans),length(fields),(j-1)*length(fields)+k)
        imagesc(Make2DPlotMat(log10(p'),chanMat,badChans))
        %set(gca,'clim',[0 1.5]);
        colorbar
