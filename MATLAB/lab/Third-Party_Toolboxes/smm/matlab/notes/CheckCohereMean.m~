...
for j=1:length(selChans)
    ...
    data.(selChanNames{j}) = LoadDesigVar(fileBaseMat,analDir,['cohSpec.yo.' selChanNames{j}] ,trialDesig);
end

nextFig = k;
fields = fieldnames(data.(selChanNames{6}));
plotData = [];
for j=1:length(fields)
    plotData(j,:,:) = ((tanh(squeeze(mean(data.(selChanNames{6}).(fields{j}))))./1.999)+0.5);
end
log10Bool = 0;
colorLimits = [0 1];
commonCLim = 2;
...
resizeWinBool = 1;
maxFreq = 150;
filename = GenFieldName(fileExt);
interpFunc = [];
PlotMultiFreqHelper(nextFig,plotData,fileExt,log10Bool,colorLimits,commonCLim,cCenter,invCscaleBool,yLabels,figTitle,resizeWinBool,fs,maxFreq,filename,interpFunc);
end
...
data = LoadDesigVar(fileBaseMat,analDir,'powSpec.yo.' ,trialDesig);

nextFig = k;
fields = fieldnames(data);
plotData = [];
for j=1:length(fields)
    plotData(j,:,:) = squeeze(mean(data.(fields{j})));
end
log10Bool = 0;
colorLimits = [];
commonCLim = 2;
...
offset = load(['ChanInfo/OffSet' fileExt '.txt']);
normBool = 1;
fs = LoadField([fileBaseMat(1,:) '/' analDir '/' depVar '.fo']);
maxFreq = 150;
thetaFreqRange = [4 12];
gammaFreqRange = [65 100];
...
for j=1:length(selChans)
    selChanNames{j} = ['ch' num2str(selChans(j))];
    data.(selChanNames{j}) = LoadDesigVar(fileBaseMat,analDir,[depVar '.yo.' selChanNames{j}] ,trialDesig);
end
...
cohere.(selChanNames{6}) = LoadDesigVar(fileBaseMat,analDir,['cohSpec.yo.' selChanNames{6}] ,trialDesig);
phase.(selChanNames{6}) = LoadDesigVar(fileBaseMat,analDir,['phaseSpec.yo.' selChanNames{6}] ,trialDesig);
fields = fieldnames(cohere.(selChanNames{6}));
for j=1:length(fields)
    figure(k)
    subplot(length(fields),1,j)
plotPhase = angle(squeeze(phase.(selChanNames{6}).(fields{j})(:,selChans(2),:)));
plotPhase = floor(plotPhase.*100);
plotCoh = squeeze(cohere.(selChanNames{6}).(fields{j})(:,selChans(2),:));
test = Accumulate(plotPhase,plotCoh);
plot(plotPhase(:),plotCoh(:),'.');
title(fields{j})
xlabel('phase')
ylabel('coh')
