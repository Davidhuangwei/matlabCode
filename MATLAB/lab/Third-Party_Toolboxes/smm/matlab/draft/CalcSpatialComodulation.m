function CalcSpatialComodulation(fileBaseMat,nchannels,channels,chanMat,badchan)
...
for i=1:size(fileBaseMat,1)
    ...
    if onePointBool
        ...
        while ~isempty(trialbegin),
            ...
            for m=1:max(whldat(:,1))
                for n=1:max(whldat(:,2))
                    indexes = trialbegin(1)-1+find(binnedWhlDat(trialbegin(1):trialend(1),1)==m & binnedWhlDat(trialbegin(1):trialend(1),2)==n);
                    if ~isempty(indexes)
                        %if exist('spatialComodData','var') & ~isempty(spatialComodData)
                        spatialComodData{m,n} = cat(3, spatialComodData{m,n}(:,:,:), [powerdat1(indexes(1),:); powerdat2(indexes(1),:)]);
                        %else
                            %spatialComodData{m,n}(:,:,:) = [powerdat1(indexes(1),:); powerdat2(indexes(1),:)];
                        %moend
...
save(['spatialComodData_grid' num2str(binSize)],'spatialComodData');
