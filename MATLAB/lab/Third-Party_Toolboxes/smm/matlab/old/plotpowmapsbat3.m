...
%[psm psn] = size(pos_sum);
pos_sum(zerosum)= NaN;
for z=1:chan_z
    figure(z)
    for y=1:chan_y
        for x=1:chan_x
            mina = max(0,min(fa)-(numbinsmooth-1));
            %maxa = min(psm,max(fa)+
            minb = max(0,min(fb)-(numbinsmooth-1));
            %maxb = min(
            mazepow(:,:,channels(y,x,z)) = pow_sum(mina:max(fa),minb:max(fb),channels(y,x,z))./pos_sum(mina:max(fa),minb:max(fb)); 
