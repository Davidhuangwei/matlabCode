...
[syncToDelete,syncToAdd] = GetSuggestions(sync1,1,tolerance);
...
while 1,
    ...
    while 1,
        ...
        while ~isempty(action) & action ~= 'd' & action ~= 'D' & action ~= 'a' & action ~= 'A' & action ~= 'F',
            string3 = ' ''F''+ENTER to Fix the syncronization automatically,';
        ...
        if action == 'd' | (action == 'D' & ~isempty(syncToDelete)),
            ...
            if (action == 'd' & y < 0.05) | action == 'D',
                ...
                [syncToDelete,syncToAdd] = GetSuggestions(sync1,max([j-1 1]),tolerance),
            else
        ...
        elseif action == 'a' | action == 'A',
            ...
            if (action == 'a' & y < 0.05) | (action == 'A' & ~isempty(syncToAdd)),
                ...
                elseif isempty(j),
                    ...
                    x = (sync2(j-1) + median(diff(sync1(1:end))))/2 *ratio+shift;
                end
        ...
        elseif action == 'F',
            % first fix video flashes
            % add points as necesary beginning with point 2 because point 1 is usually a bit off
            
            MedSync1Dif = median(diff(sync1(1:end)));
            
            if (~isempty(find(diff(sync1(2:5)) < median(diff(sync1(1:end)))*tolerance)) | ~isempty(find(diff(sync1(2:5)) > median(diff(sync1(1:end)))/tolerance)))
                fprintf('\n\nthe median time difference between the 2nd-5th spot is not similar to the median time difference between all the spots in the file... i.e there is a problem and you should attempt some manual cleaing before running autofix');    
            else
                [m1,n] = size(sync1);
                % Correct sync 1
            ...
            % first fix eeg flashes
            % add points as necesary beginning with point 2 because point 1 is usually a bit off
            MedSync2Dif = median(diff(sync2(1:end)));
            
            if (~isempty(find(diff(sync2(2:5)) < median(diff(sync2(1:end)))*tolerance)) | ~isempty(find(diff(sync2(2:5)) > median(diff(sync2(1:end)))/tolerance)))
                fprintf('\n\nthe median time difference between the 2nd-5th spot is not similar to the median time difference between all the spots in the file... i.e there is a problem and you should attempt some manual cleaing before running autofix');    
            else
                [m2,n] = size(sync2);
                % Correct sync 1
                ...
                while (j < m2)
                    if (((sync2(j+1)-sync2(j)) >= (MedSync2Dif*tolerance)) & ((sync2(j+1)-sync2(j)) <= (MedSync2Dif/tolerance)))
                        ...
                        while ((j < m2) & ((sync2(j+1)-sync2(j)) < (MedSync2Dif*tolerance)))
                            m2=m2-1;              
                            sync2(j+1) = [];              
                        end
                        while ((j < m2) & ((sync2(j+1)-sync2(j)) > (MedSync2Dif/tolerance)))
                ...
                % Update automatic preselections so hitting A or D doesn't confuse the user
                [syncToDelete,syncToAdd] = GetSuggestions(sync2,max([j-1 1]),tolerance),
            end
  %          [m2,n] = size(sync2);
  %          while (m2 < m2)
...
function [syncToDelete,syncToAdd] = GetSuggestions(sync,start,tolerance)

syncIntervals = diff(sync(start:end));
smallSyncIntervals = find(syncIntervals < tolerance*median(syncIntervals));
syncToDelete = smallSyncIntervals + start;
largeSyncIntervals = find(syncIntervals > median(syncIntervals)/tolerance);
