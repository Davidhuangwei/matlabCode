function varargout=wtcm(x,y,varargin)
% Wavelet comodulation
% Algorithm:
% notation: c = centered, sm = smoothwavelet
% wx = log10(abs(wavelet(x)).^2); % data must be normally dist for r to be reliable
% wy = log10(abs(wavelet(y)).^2); % data must be normally dist for r to be reliable
% cX = x-sm(wx);
% cY = x-sm(wy);
% R = sm(cX.*cY) ./ sm(abs(cX) .* abs(cY))
%
% USAGE: [R,period,scale,coi,sig95]=wtc(x,y,[,settings])
%
% 
...
BROKEN_THIS_FUNCTION_DOES_NOT_WORK
%-----------:::::::::::::--------- ANALYZE ----------::::::::::::------------
...
% ----- center data -----
X = X - smoothwavelet(sinv(:,ones(1,nx)).*X,dt,period,Args.Dj,scale);
Y = Y - smoothwavelet(sinv(:,ones(1,ny)).*Y,dt,period,Args.Dj,scale);
...
% ---- calculate corr coef ----
R = smoothwavelet(sinv(:,ones(1,nx)).*(X.*Y),dt,period,Args.Dj,scale) ...
    ./ smoothwavelet(sinv(:,ones(1,nx)).*(abs(X).*abs(Y)),dt,period,Args.Dj,scale);
keyboard
% --- plotting not developed for comod ----
