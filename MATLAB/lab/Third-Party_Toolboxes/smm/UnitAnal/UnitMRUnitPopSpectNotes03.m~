...
for a=1:length(analDirs)
    ...
    for m=1:length(chanLocNames)
        for n=1:length(cellTypeLabels)
            rateTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopPowSpecNW' num2str(NW) '.' chanLocNames{m} '.' cellTypeLabels{n} '.rate'],trialDesig);
            spectTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopPowSpecNW' num2str(NW) '.' chanLocNames{m} '.' cellTypeLabels{n} '.yo'],trialDesig);
            %elCluList = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopPowSpec.elClu']);
            fo = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopPowSpecNW' num2str(NW) '.' chanLocNames{m} '.' cellTypeLabels{n} '.fo']);
            nCells = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopPowSpecNW' num2str(NW) '.' chanLocNames{m} '.' cellTypeLabels{n} '.nCells']);
...
for m=1:length(selAnatRegions)
    for k=1:length(cellTypeLabels)
        ...
        for j=1:length(mazeLocFieldNames)
            ...
            hold on
            %% sum(spect) / sum(rate) %%
            plot(fo,squeeze(sum(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}))./...
                repmat(sum(firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})),...
                [1,1,size(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3)]))...
                ,'color',plotColors(j));
            %% sum(sum(spect) ./ sum(rate))
            plot(fo,squeeze(sum(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})./...
                repmat(sum(firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})),...
                [size(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),1)...
                ,1,size(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3)])))...
                ,'color',plotColors(j));
            
%             plot(fo,squeeze(mean(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})./...
%                 repmat(mean(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3),...
%                 [1,1,size(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3)])))...
%                 ,'color',plotColors(j));
...
figure
clf
set(gcf,'name','UnitPopThetaSpect')
set(gcf,'unit','inches')
set(gcf,'position',[0.5 0.5 3*length(cellTypeLabels) 1.1*length(selAnatRegions) ])
set(gcf,'paperposition',[0.5 0.5 3*length(cellTypeLabels) 1.1*length(selAnatRegions)])
plotColors = 'bgrcmk'
        ...
        for j=1:length(mazeLocFieldNames)
            if length(firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}))
                catSpect = cat(1,catSpect,squeeze(mean(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})...
                    (:,:,fo>=thetaFreqRange(1) & fo<=thetaFreqRange(2))./...
                    repmat(mean(firingSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3),...
...
%                     [1,1,sum(fo>=thetaFreqRange(1) & fo<=thetaFreqRange(2))]),3)));
        ...
        ylabel(selAnatRegions{m})
        if m==1 & k==2
            titleText = {['thetaFreqRange=' num2str(thetaFreqRange) ', NW=' num2str(NW) ', rateThresh=' num2str(rateThresh) ]; titleText};
        end
        title(titleText)
    end
end
...
figure
clf
set(gcf,'name','UnitPopGammaSpect')
set(gcf,'unit','inches')
set(gcf,'position',[0.5 0.5 3*length(cellTypeLabels) 1.1*length(selAnatRegions) ])
set(gcf,'paperposition',[0.5 0.5 3*length(cellTypeLabels) 1.1*length(selAnatRegions)])
plotColors = 'bgrcmk'
...
%                     [1,1,sum(fo>=gammaFreqRange(1) & fo<=gammaFreqRange(2))]),3)));
        ...
        ylabel(selAnatRegions{m})
        if m==1 & k==2
            titleText = {['gammaFreqRange=' num2str(gammaFreqRange) ', NW=' num2str(NW) ', rateThresh=' num2str(rateThresh) ]; titleText};
        end
        title(titleText)
    end
end
