...
for m=1:length(behavCond)
    ...
    for j=1:length(analDirs)
        ...
        for k=1:length(fileBaseCell)
            ...
            tempCCG.yo(isnan(tempCCG.yo(:))) = 0;
            if sum(isnan(tempCCG.yo(:)))>0
                keyboard
            end
            if isempty(sumCCG)
                sumCCG = sum(tempCCG.yo);
                sumRate = sum(tempCCG.rate);
            else
                sumCCG = sumCCG + sum(tempCCG.yo);
                sumRate = sumRate + sum(tempCCG.rate);
            end
            catN = catN + size(tempCCG.rate,1);
            if ~isempty(to) & to~=tempCCG.to
                ERROR_to~=to
            else
                to = tempCCG.to;
            end
        ...
        cellLayers = cat(1,cellLayers,tempCCG.cellLayer);
        cellTypes = cat(1,cellTypes,tempCCG.cellType);
    end
end
