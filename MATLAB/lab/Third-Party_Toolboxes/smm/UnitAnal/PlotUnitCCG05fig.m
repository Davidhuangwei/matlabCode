...
for j=1:length(analDirs)
    ...
    switch normalization
            ...
            for k=1:size(tempCCG,1)
                for m=1:size(tempRate{k,end},2)
                    for n=1:size(tempRate{k,end},2)
                        ...
                        scaleFactor = 1 / (numSec*tempRate{k,end}(1,m)); % percentage of spikes
        ...
        case 'scale',
            ...
            winLen = LoadField([fileBaseCell{1} '/' spectAnalDir '/' 'infoStruct.winLength']);
            eegSamp = LoadField([fileBaseCell{1} '/' spectAnalDir '/' 'infoStruct.eegSamp']);
            winSec = winLen/eegSamp;
            binSec = binSize/1000;
            for k=1:size(tempCCG,1)
                tempCCG{k,end} = tempCCG{k,end} * binSec / winSec;
            end
            
        case 'hz',
            binSec = binSize/1000;
            for k=1:size(tempCCG,1)
                tempCCG{k,end} = tempCCG{k,end} * binSec;
            end
...
colNamesCell{5} = colNamesCell{5}([3,1,2])
...
UnitSpectrumPlotHelper(dataCell,to,[0 timeLim],infoText,figName,colNamesCell,reportFigBool,outDir)
UnitSpectrumPlotHelper(dataCell,to,[0 timeLim],infoText,figName,colNamesCell,reportFigBool,outDir,[],[],[],1)
UnitACGPlotHelper(dataCell,to,[0 timeLim],infoText,figName,colNamesCell,reportFigBool,outDir,[],[],[],0)
...
% tempPvals = UnitSignRankHelper(tempDataCeUnitLineShapePlotHelper(tempDataCell,[-0.35 .35],infoText,figName,colNamesCell,reportFigBool,outDir,0.05,bonFfactor,2,1.1)ll)
bonFfactor = 1/2;
bonFfactor = 1;
UnitBarPlotHelper(tempDataCell,[-0.6 0.6],infoText,figName,tempColNamesCell,reportFigBool,outDir,0.05,bonFfactor,2,1.1)
UnitLineShapePlotHelper(tempDataCell,[-20 20],infoText,figName,colNamesCell,reportFigBool,outDir,0.05,bonFfactor,2,1.1)

                %%%%%%%%%%%%%%%%%% w/in cell kw + post hoc printing %%%%%%%%%%%%%%%%
        tempDataCell = UnitMeanHelper(dataCell,2,to,timeRange);
...
UnitLineShapePlotHelper(tempDataCell,[],infoText,figName,colNamesCell,reportFigBool,outDir,0.05,bonFfactor,2,1.1)
UnitBarPlotHelper(tempDataCell,[],infoText,figName,colNamesCell,reportFigBool,outDir,0.05,bonFfactor,2,1.1)

 tempDataCell = UnitMedianHelper(tempDataCell,1);


goodInd = ...
    strcmp(tempDataCell(:,5),'Tonic')  | ...
     strcmp(tempDataCell(:,5),'Phasic') ;
