...
for a=1:length(analDirs)
    ...
    for m=1:length(chanLocNames)
        for n=1:length(cellTypeLabels)
            rateTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopACG.' chanLocNames{m} '.' cellTypeLabels{n} '.rate'],trialDesig);
            spectTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopACG.' chanLocNames{m} '.' cellTypeLabels{n} '.yo'],trialDesig);
            to = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopACG.' chanLocNames{m} '.' cellTypeLabels{n} '.to']);
            nCells = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopACG.' chanLocNames{m} '.' cellTypeLabels{n} '.nCells']);
