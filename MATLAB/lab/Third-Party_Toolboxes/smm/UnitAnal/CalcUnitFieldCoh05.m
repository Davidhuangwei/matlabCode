...
% [selChan,fpass,tapers,newSamp,padBool] = DefaultArgs(varargin,{[selChanCell{:,end}],[0 250],[3 5],eegSamp,0});
function CalcUnitFieldCoh05(fileBaseCell,fileExt,winLen,spectAnalDir,varargin)
eegSamp = 1250;
datSamp = 20000;
timeWin = winLen/eegSamp;
selChanCell = Struct2CellArray(LoadVar(['ChanInfo/SelChan' fileExt '.mat']));
...
for j=1:length(fileBaseCell)
    ...
    save([fileBase '/' spectAnalDir 'unitFieldCoh_orig_tapers' num2str(paramb.tapers(2)) '.mat'],SaveAsV6,'unitFieldCoh');
    unitFieldCoh.yo = ATanCoh(unitFieldCoh.yo);
    save([fileBase '/' spectAnalDir 'unitFieldCoh_ATan_tapers' num2str(paramb.tapers(2)) '.mat'],SaveAsV6,'unitFieldCoh');
