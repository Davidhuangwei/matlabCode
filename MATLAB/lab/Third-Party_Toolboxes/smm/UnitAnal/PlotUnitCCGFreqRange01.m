function PlotUnitCCG01(analDirs,winLen,spectAnalDir,varargin)
...
for m=1:length(cellLayers)
    for n=1:length(cellTypes)
        ...
        if isfield(acgs{1,end},cellLayers{m}) & isfield(acgs{1,end}.(cellLayers{m}),cellTypes{n})
            plot(acgs{1,end}.(cellLayers{m}).(cellTypes{n}),...
                ccgs{2,end}.(cellLayers{m}).(cellTypes{n}),'.')
            minVal = min([ccgs{1,end}.(cellLayers{m}).(cellTypes{n});...
                ccgs{2,end}.(cellLayers{m}).(cellTypes{n})]);
            maxVal = max([ccgs{1,end}.(cellLayers{m}).(cellTypes{n});...
                ccgs{2,end}.(cellLayers{m}).(cellTypes{n})]);
            minVal = minVal-.1*(maxVal-minVal);
            maxVal = maxVal+.1*(maxVal-minVal);
            
            plot([minVal maxVal],[minVal maxVal])
            title(['n=' num2str(size(ccgs{1,end}.(cellLayers{m}).(cellTypes{n}),1))])
        end 
        if n==1 & m==1
...
rates = SortRates2LayerTypes(squeeze(tempCCG.maze),cellLayers,cellTypes);
