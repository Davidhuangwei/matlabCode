...
%         keepInd = tempRate{k,end} >= cellRateMin;
        ...
        if ~isempty(idLoc)
            tempAnimalID{k,end} = repmat(str2num(analDirs{j}...
                ([idLoc+length(idText):idLoc+length(idText)+1])),size(cellLayers,1),1);
        else
            tempAnimalID{k,end} = zeros(size(cellLayers,1),1);    
        end
%         % sort cells by layer/type
        if isempty(ccgs)
            ...
            if ~isempty(
            animalID = repmat(str2num(analDirs{j}(strfind(analDirs{j},'sm96')
        end
