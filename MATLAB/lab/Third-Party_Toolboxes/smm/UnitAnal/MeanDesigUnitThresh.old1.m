...
function outStruct = MeanDesigUnitThresh(fileBaseCell,spectAnalDir,depVar,unitRateVar,rateMin,trialDesig)
        ...
        if ~isempty(temp{m,end})
            for n=1:size(unitRate{m,end},2)
                ...
                if length(sumTemp)>=m & ~isempty(sumTemp{m}) & size(sumTemp{m},1)>=n
                    sumTemp{m}(n,:,:) = sumTemp{m}(n,:,:) + permute(sum(temp{m,end}(goodRate & notNaN,n,:,:),1),[2,3,4,1]);
                    num(m,n) = num(m,n) + size(temp{m,end}(goodRate & notNaN,n,:,:),1);
                else
                    if ndims(permute(sum(temp{m,end}(goodRate & notNaN,n,:,:),1),[2,3,4,1])) < 3
                        sumTemp{m}(n,:) = permute(sum(temp{m,end}(goodRate & notNaN,n,:,:),1),[2,3,4,1]);
                    end
                    sumTemp{m}(n,:,:) = permute(sum(temp{m,end}(goodRate & notNaN,n,:,:),1),[2,3,4,1]);
                    num(m,n) = size(temp{m,end}(goodRate & notNaN,n,:,:),1);
                end
            end
        end
    end
