function PlotUnitRatesScat02(analDirs,fileBaseCell,winLen,spectAnalDir,varargin)
...
for m=1:length(cellLayers)
    for n=1:length(cellTypes)
        for j=1:size(acgs,1)
            ...
            hold on
            title
            if isfield(acgs{j,end},cellLayers{m}) & isfield(acgs{j,end}.(cellLayers{m}),cellTypes{n})
                plot(acgs{j,end}.(cellLayers{m}).(cellTypes{n}),[plotColors(j) '.']);
                title(['n=' num2str(size(rates{1,end}.(cellLayers{m}).(cellTypes{n}),1))])
            end
            if n==1 & m==1
                title('Unit Firing Rates')
