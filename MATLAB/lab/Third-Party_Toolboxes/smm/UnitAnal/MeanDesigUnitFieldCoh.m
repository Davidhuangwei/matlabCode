...
function outStruct = MeanDesigUnitFieldCoh(fileBaseCell,spectAnalDir,depVar,unitRateVar,rateMin,trialDesig)
        ...
        if ~isempty(temp{m,end})
            for n=1:size(unitRate{m,end},2)
                ...
                if length(sumTemp)>=m & ~isempty(sumTemp{m}) & size(sumTemp{m},1)>=n
                    sumTemp{m}(n,:,:) = sumTemp{m}(n,:,:) + shiftdim(sum(temp{m,end}(unitRate{m,end}(:,n)>=rateMin,n,:,:),1),1);
                    num(m,n) = num(m,n) + size(temp{m,end}(unitRate{m,end}(:,n)>=rateMin,n,:,:),1);
                else
                    sumTemp{m}(n,:,:) = shiftdim(sum(temp{m,end}(unitRate{m,end}(:,n)>=rateMin,n,:,:),1),1);
