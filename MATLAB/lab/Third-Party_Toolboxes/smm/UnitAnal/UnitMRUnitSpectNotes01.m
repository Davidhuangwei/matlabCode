...
for a=1:length(analDirs)
    ...
    for f=1:length(fileBaseCell)
        ...
        unitPowSpec = LoadField([fileBase '/' spectAnalDir 'unitPowSpec.yo']);
        elCluList = LoadField([fileBase '/' spectAnalDir 'unitPowSpec.elClu']);
        fo = LoadField([fileBase '/' spectAnalDir 'unitPowSpec.fo']);
        if size(cluLoc,1)~=size(elCluList,1)
            problem
        end
        rateTrialsByCells = cat(1,rateTrialsByCells,firingRate);
        spectTrialsByCells = cat(1,spectTrialsByCells,unitPowSpec);
        mazeLocVec = cat(1,mazeLocVec,mazeLocName);
    end
    mazeLocFieldNames = unique(mazeLocName);
    for j=1:length(mazeLocFieldNames)
        if a==1
            firingRateStruct.(mazeLocFieldNames{j}) = mean(rateTrialsByCells(strcmp(mazeLocVec,mazeLocFieldNames{j}),:,:));
            firingSpectStruct.(mazeLocFieldNames{j}) = mean(spectTrialsByCells(strcmp(mazeLocVec,mazeLocFieldNames{j}),:,:));
        else
            firingRateStruct.(mazeLocFieldNames{j}) = cat(2,firingRateStruct.(mazeLocFieldNames{j}),...
                mean(rateTrialsByCells(strcmp(mazeLocVec,mazeLocFieldNames{j}),:,:)));
            firingSpectStruct.(mazeLocFieldNames{j}) = cat(2,firingSpectStruct.(mazeLocFieldNames{j}),...
                mean(spectTrialsByCells(strcmp(mazeLocVec,mazeLocFieldNames{j}),:,:)));
        end
...
for m=1:length(selAnatRegions)
    for j=1:length(mazeLocFieldNames)
        for k=1:length(cellTypeLabels)
            ...
            if sum(chanInd)>0
                plot(fo,squeeze(mean(firingSpectStruct.(mazeLocFieldNames{j})...
                    (:,chanInd,:)./...
                    repmat(firingRateStruct.(mazeLocFieldNames{j})...
                    (:,chanInd,...
