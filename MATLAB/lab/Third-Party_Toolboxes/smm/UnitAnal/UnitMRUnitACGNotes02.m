...
for a=1:length(analDirs)
    ...
    rateTrialsByCells = LoadDesigVar(fileBaseCell,dirName,'unitACG.rate',trialDesig);
    spectTrialsByCells = LoadDesigVar(fileBaseCell,dirName,'unitACG.yo',trialDesig);
    elCluList = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitACG.elClu']);
    to = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitACG.to']);
    
    mazeLocFieldNames = fieldnames(rateTrialsByCells);
    for j=1:length(mazeLocFieldNames)
        for k=1:size(rateTrialsByCells.(mazeLocFieldNames{j}))
            goodTrials = rateTrialsByCells.(mazeLocFieldNames{j})(:,k)>rateCutoff;
            if a==1
                if sum(goodTrials)==0
                firingRateStruct.(mazeLocFieldNames{j})(1,k) = ...
                    mean(rateTrialsByCells.(mazeLocFieldNames{j})(goodTrials,k));
                firingSpectStruct.(mazeLocFieldNames{j})(1,k) = ...
                    mean(spectTrialsByCells.(mazeLocFieldNames{j})(goodTrials,k));
            else
