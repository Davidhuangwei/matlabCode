...
%                 keyboard
                ...
                hold on
                errBarLen = 0.08;
                offset = 0.1*(length(col{3})/2 - m);
                plotColor = colorOrder(mod(m,length(colorOrder))+1,:);

                
                plot(median(catData,1),[1:size(catData,2)]+offset,'color',plotColor,'linewidth',3)
...
%                 end
                ...
                text(textX,textY,[col{3}{m} ',' nText],'color',plotColor)

%                 try 
%                     barh(catData,catGroup);
%                end
                title(SaveTheUnderscores(cat(1,titleText)))
                if m==length(col{3})
