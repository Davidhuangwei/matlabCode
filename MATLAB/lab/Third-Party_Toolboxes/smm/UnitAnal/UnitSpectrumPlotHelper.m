...
[xLimits yLimits infoText figName colNamesCell reportFigBool saveDir plotColors screenHeight xyFactor,errBarBool] = ...
    DefaultArgs(varargin,{[],[],{'noInfo'},[],{},0,'./','rgbkcym',11,1.5,0});
...
%                         temp1 = ExpSmooth(temp1,ones(3,1)/3,1);
                        plot(xVal,temp1,'color',plotColors(p))
                        if errBarBool
                            expInd = p*2:length(col{5})*2:size(tempData{1},2);
...
%                                                     plot([xVal(errBarSamp)],errTemp([3],:),':','color',plotColors(p))
                            plot([xVal(errBarSamp); xVal(errBarSamp)],errTemp([2 3],:),'color',plotColors(p))
                        end
                   end
                    if isempty(xLimits)
