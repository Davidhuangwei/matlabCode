    ...
    for p=1:length(selChanNames)
        for m=1:length(chanLocNames)
            for n=1:length(cellTypeLabels)
                ...
                crossSpectTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopCrossSpecNW' num2str(NW) '.' selChanNames{p} '.' chanLocNames{m} '.' cellTypeLabels{n} '.yo'],trialDesig);
                eegPowTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopEegPowSpecNW' num2str(NW) '.' selChanNames{p} '.' chanLocNames{m} '.' cellTypeLabels{n} '.yo'],trialDesig);
                unitPowTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopUnitPowSpecNW' num2str(NW) '.' selChanNames{p} '.' chanLocNames{m} '.' cellTypeLabels{n} '.yo'],trialDesig);
                ...
                for j=1:length(mazeLocFieldNames)
                    if a==1
                        ...
                        crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            (crossSpectTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh,:,:));
                        eegPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            (eegPowTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh,:,:));
                        unitPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            (unitPowTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh,:,:));
                        ...
                        firingRateStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            ...
                            (rateTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh)));
                        crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            cat(1,crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}),...
                            (crossSpectTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh,:,:)));
                        eegPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            cat(1,eegPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}),...
                            (eegPowTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh,:,:)));
                        unitPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}) = ...
                            cat(1,unitPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(chanLocNames{m}).(cellTypeLabels{n}),...
                            (unitPowTrialsByCells.(mazeLocFieldNames{j})(rateTrialsByCells.(mazeLocFieldNames{j})>rateThresh,:,:)));
...
for p=1:length(selChanNames)
    figure
    clf
    set(gcf,'name',['UnitPopMedianCohSpectSum'])
    set(gcf,'unit','inches')
    ...
    for m=1:length(selAnatRegions)
        for k=1:length(cellTypeLabels)
            ...
            for j=1:length(mazeLocFieldNames)
                ...
                hold on
                cohTemp = sqrt(squeeze(sum(...
                    crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}).*...
                    conj(crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}))))./...
                    squeeze(sum(eegPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}).*...
                    unitPowSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}))));
                plot(fo,cohTemp,'color',plotColors(j));
%                 atanh((sqrt(squeeze(xSpec(1,k,m,:).*conj(xSpec(1,k,m,:)))./...
%                     squeeze(pow(1,selectedChannels(k),:).*pow(1,m,:)))-0.5)*1.999)
%                 
%                 plot(fo,squeeze(mean(crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k})./...
%                     repmat(mean(crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}),3),...
%                     [1,1,size(crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}),3)])))...
%                     ,'color',plotColors(j));
        ...
        for k=1:length(cellTypeLabels)
            ...
            for j=1:length(mazeLocFieldNames)
                if length(firingRateStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}))
                    catSpect = cat(1,catSpect,median(crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k})...
...
for m=1:length(selAnatRegions)
    for k=1:length(cellTypeLabels)
        ...
        for j=1:length(mazeLocFieldNames)
            if length(firingRateStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k}))
                catSpect = cat(1,catSpect,squeeze(mean(crossSpectStruct.(mazeLocFieldNames{j}).selChanNames{p}.(selAnatRegions{m}).(cellTypeLabels{k})...
...
for m=1:length(selAnatRegions)
    for k=1:length(cellTypeLabels)
        ...
        for j=1:length(mazeLocFieldNames)
            subplot(length(selAnatRegions),length(cellTypeLabels),(m-1)*length(cellTypeLabels)+k)
            hold on
             plot(median(crossSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3),...
                firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),...
                'o','color',plotColors(j));
%             plot(fo,squeeze(mean(crossSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})./...
%                 repmat(firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),...
%                 [1,1,size(crossSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),3)])))...
%                 ,'color',plotColors(j));
              if j==1
...
for m=1:length(selAnatRegions)
    for k=1:length(cellTypeLabels)
        ...
        for j=1:length(mazeLocFieldNames)
            subplot(length(selAnatRegions),length(cellTypeLabels),(m-1)*length(cellTypeLabels)+k)
            hold on
            meanSpect = squeeze(mean(mean(crossSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})...
                (:,:,fo>=gammaFreqRange(1) & fo<=gammaFreqRange(2))./...
                repmat(firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),...
                [1,1,sum(fo>=gammaFreqRange(1) & fo<=gammaFreqRange(2))]),3),1));
            stdSpect = squeeze(std(mean(crossSpectStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k})...
                (:,:,fo>=gammaFreqRange(1) & fo<=gammaFreqRange(2))./...
                repmat(firingRateStruct.(mazeLocFieldNames{j}).(selAnatRegions{m}).(cellTypeLabels{k}),...
    ...
    for k=1:length(cellTypeLabels)
        ...
        for j=1:length(mazeLocFieldNames)
            ...
            chanInd = strcmp(chanLayerVecAll,selAnatRegions{m})...
                    ...
                    & firingRateStruct.(mazeLocFieldNames{j})(1,:)>0;
            if sum(chanInd)>0
                plot(fo,squeeze(mean(crossSpectStruct.(mazeLocFieldNames{j})(:,chanInd,:)./...
                    repmat(firingRateStruct.(mazeLocFieldNames{j})(:,chanInd),...
                    [1,1,size(crossSpectStruct.(mazeLocFieldNames{j}),3)]),2)),'color',plotColors(j));
            end
            titleText = [titleText ',' cellTypeLabels{k} '=' num2str(sum(chanInd))];
            ylabel(selAnatRegions{m})
            if m==1 & k==1
    ...
    for k=1:length(cellTypeLabels)
        ...
        for j=1:length(mazeLocFieldNames)
            ...
            chanInd = strcmp(chanLayerVecAll,selAnatRegions{m})...
                    & strcmp(cellTypesVecAll,cellTypeLabels{k});
            if sum(chanInd)>0
                plot(fo,squeeze(mean(crossSpectStruct.(mazeLocFieldNames{j})(:,chanInd,:),2)),'color',plotColors(j));
            end
            titleText = [titleText ',' cellTypeLabels{k} '=' num2str(sum(chanInd))];
            ylabel(selAnatRegions{m})
            if m==1 & k==1
