...
for a=1:length(analDirs)
    ...
    for m=1:length(chanLocNames)
        for n=1:length(cellTypeLabels)
            rateTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopPowSpec.' chanLocNames{m} '.' cellTypeLabels{n} '.rate'],trialDesig);
            spectTrialsByCells = LoadDesigVar(fileBaseCell,dirName,['unitPopPowSpec.' chanLocNames{m} '.' cellTypeLabels{n} '.yo'],trialDesig);
            %elCluList = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopPowSpec.elClu']);
            fo = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopPowSpec.' chanLocNames{m} '.' cellTypeLabels{n} '.fo']);
            nCells = LoadField([fileBaseCell{1} '/' spectAnalDir 'unitPopPowSpec.' chanLocNames{m} '.' cellTypeLabels{n} '.nCells']);
            ...
            for j=1:length(mazeLocFieldNames)
                if a==1 & m==1 & n==1
                    ...
                    nCellsStruct.(mazeLocFieldNames{j}) = nCells;
                else
                    ...
                    nCellsStruct.(mazeLocFieldNames{j}) = cat(2,nCellsStruct.(mazeLocFieldNames{j}),nCells);
                end
