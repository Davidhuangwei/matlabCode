function PlotBadChanAnal2(animal,behaviorCell,depMeasureCell,shanks,allChans,frequencies,saveFigBool)
% function PlotBadChanAnal2(animal,behaviorCell,depMeasureCell,fileNamesCell,shanks,allChans,frequencies,saveFigBool)
%
...
% fileNamesCell: a cell array containing a text string in each cell that is the
% basename of a .dat file.
%
% A .mat file with the name [behaviorCell{i} '_' depMeasFile '_' firstFile '-' lastFile '.mat']
% created by CalcBadChanAnal2
...
for z=1:length(depMeasureCell)
    ...
    %keyboard
    cLimits = [min(min(min(plotData(plotData~=1 & plotData~=0)))) max(max(max(plotData(plotData~=1 & plotData~=0))))];
    %cLimits = [min(min(min(aveDepVar))) 0.7];
    %cLimits = [0.3 0.7];
    try set(gca,'clim',cLimits); end
    %set(gca,'clim',[0 1]);
    colorbar;
    if saveFigBool
        print([depMeasure '_' behavior num2str(allChans(1)) '-' num2str(allChans(end)) '_all_chans'], '-dpng', '-r125');
    ...
    for k=1:length(shanks)
        ...
        imagesc(plotData);
        title(['ave ' depMeasure ': ' behavior ' files ' firstFile '-' lastFile ', ch ' num2str(shanks{k}(1)) '-' num2str(shanks{k}(end)) ],'fontsize',7);
        ...
        if saveFigBool
            print([depMeasure '_' behavior num2str(allChans(1)) '-' num2str(allChans(end)) '_each_shank' ], '-dpng', '-r125');
            %print(['SFNposter2005/' depMeasure '_' behavior '_ch' num2str(allChans(1)) '-' num2str(allChans(end)) '_Ave' num2str(fo(1)) '-' num2str(fo(end)) 'Hz'], '-depsc', '-adobecset')
        else
            keyboard
        end
