function CalcRunningSpectraSpeedAccel01(saveDir,fileBaseCell,fileExt,winLength,varargin)
...
%                             end                            
                            ...
                            for m=1:nChan 
                                ...
                                pow(1,m,:) = mean(temp'.*normEegHannWin,1);
                            ...
                            for k=1:length(selectedChannels)
                                for m=1:nChan
                                    ...
                                    temp = Wxy;
                                    xSpec(1,k,m,:) = mean(temp'.*normEegHannWin,1);
                                    % ----------------------- phase ---------------------------------
                                    temp = temp./abs(temp); % normalize (convert to complex phase)
                                    phase(1,k,m,:) = mean(temp'.*normEegHannWin,1);
                                    ...
                                    coh(1,k,m,:) = mean(temp'.*normEegHannWin,1);
                                end
                            end
                            wave = [];
                            smoothWave = [];
                            ...
                            if exist('powSpec','var') & fo ~= powSpec.fo
                                ERROR_powSpec.fo_is_changing
                            end
