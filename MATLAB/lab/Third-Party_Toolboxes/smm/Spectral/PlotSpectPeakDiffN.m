...
for c=1:length(chans)
    ...
    for k=1:length(dtN)
        ...
        [n{k}{c} x{k}{c}] = BsErrBars(@median,percConf,resamps,@hist,1,...
            (dtN{k}(clu{k}==c)/datSamp*1000),...
            [-xLimit:xLimit*2/nBins:xLimit]);
        n{k}{c} = n{k}{c}/sum(clu{k}==c);
        plot(x{k}{c}(1,:),n{k}{c}(1,:),plotColors(k));
        hold on
        plot(repmat(x{k}{c}(1,:),[2 1]),n{k}{c}(2:3,:),...
            plotColors(k));
...
% stats testing
goodInd = (x{1}{1}(1,:) > -75 & x{1}{1}(1,:) < -25) | (x{1}{1}(1,:) > 25 & x{1}{1}(1,:) < 75);
data = [];
group = {};
chan = 5;
for j=1:length(n)
    data = cat(1,data,n{j}{chan}(1,goodInd)');
    group = cat(1,group,repmat(time(j,1),[sum(goodInd) 1]));
end
[p atab stats] = kruskalwallis(data,group);
multcompare(stats)

data = [];
group = {};
for j=1:length(n)
    goodDtN = abs(dtN{j})/datSamp*1000<250;
data = cat(1,data,abs(dtN{j}(goodDtN))/datSamp*1000);
group = cat(1,group,repmat(time(j,1),size(dtN{j})));
end
[p atab stats] = kruskalwallis(data,group);
figure
