function CalcPhasePeakDyn01(fileBaseCell,fileExt,nChan,refChan,peakChan,freqRange,winLength,aveLength,varargin)
...
%     peakIndex = [];
    ...
    for j=1:size(whl,1)
        time = (j-1)/whlSamp;
%         round((time-winLength/2)*eegSamp)
%         numSamp-round(eegSamp*winLength)
...
%         [fileBase '/' fileBase fileExt]
        ...
        [yo fo] = mtspectrumc(eeg(peakChan,:)',params);
        [peakF tempPeakIndex] = FindSpectPeak01(permute(yo,[3,2,1]),fo,params.fpass);
...
%         coh = (coh);
        ...
        gammaPhase(j,:) = phase(tempPeakIndex,:);
%         peakIndex(j,1) = tempPeakIndex;
    end
%     keyboard
    figure
    subplot(1,2,1)
...
%     set(gca,'ylim',[0 1]);
    ...
    smoothGammaPhase = angle(Filter0(avgfiltb,gammaPhase));
    outCohFileName = [fileBase '/' fileBase '_' num2str(freqRange(1)) '-' num2str(freqRange(2)) 'Hz'...
        '_Win' num2str(winLength) '_Ave' num2str(aveLength) fileExt '_ref' num2str(refChan) '.10000peakPhase'];
    fprintf('\nSaving %s\n', outCohFileName);
    bsave(outCohFileName,10000*smoothGammaPhase','int16');
end
