...
function CalcSpectraTime01(saveDir,fileBaseCell,fileExt,winLength,epochsExt,varargin)
[maxNTimes,batchModeBool] = ...
    DefaultArgs(varargin,{NaN,1});
...
%         try
            ...
            remTimes = load([fileBase epochsExt]);
            for k=1:size(remTimes,1)
                eegSegTime = [eegSegTime; [ceil(remTimes(k,1)/winLength)*winLength:...
                    winLength:floor(remTimes(k,2)/winLength)*winLength-winLength]'];
            end
            length(eegSegTime)
            %              try
