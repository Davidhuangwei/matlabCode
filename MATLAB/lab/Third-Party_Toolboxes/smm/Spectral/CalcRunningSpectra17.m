...
spectAnalBase = [description midPtext '_MinSpeed' num2str(minSpeed)...
    'wavParam' num2str(wavParam) 'Win' num2str(winLength)];
% spectAnalBase = [description midPtext '_MinSpeed' num2str(minSpeed)...
%      'Win' num2str(winLength) 'wavParam' num2str(wavParam)];
saveDir = [spectAnalBase fileExt];
...
for j=1:length(fileBaseCell)
    try
        ...
        if exist([dirName '/infoStruct.mat'],'file')
            infoStruct = MergeStructs(LoadVar([SC(dirName) 'infoStruct.mat']),infoStruct,1);
        end
            ...
            switch funcs2run{k}
                ...
                case 'CalcRunningSpectraSpeedAccel02'
                    CalcRunningSpectraSpeedAccel02(saveDir,fileBaseCell(j),fileExt,winLength,...
                        [],1,{gcf},[],batchModeBool)
                case 'CalcRunningSpectraBehav03'
                    CalcRunningSpectraBehav03(saveDir,fileBaseCell(j),fileExt,winLength,...
                        [],1,{gcf},[],batchModeBool)
                ...
                case 'ExtractThetaFreqLM'
                    ExtractThetaFreqLM(fileBaseCell(j),thetaFreqRange,spectAnalBase,{fileExt});
