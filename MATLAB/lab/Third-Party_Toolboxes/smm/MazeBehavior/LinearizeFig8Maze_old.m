...
linearRLaverage(find(exploration(:,1) ~= -1)) = abs(exploration(find(exploration(:,1) ~= -1),1)-max(exploration(:,1))) + explorationOffset;
...
if i(1) == 'y'
    ...
    if str2num(matlabVersion(1)) > 6
      save([filebase '_LinearizedWhl.mat'],'-V6','linearRLaverage');
    else
      save([filebase '_LinearizedWhl.mat'],'-V6','linearRLaverage'); 
    end
end
