function DetectPhasicRem03(fileExt,varargin)
try
...
[fileBaseCell,nChan,chan,chanGroupName,plotChan,winLen,phasicRefrac,defSD,preWhitenBool,recalcBool,fRange] = ...
    DefaultArgs(varargin,{LoadVar('FileInfo/RemFiles'),nChan,chan,'Mol',plotChan,1024,2,1.5,1,0,[0 250]});
...
%     integPow = squeeze(mean(mean(y(times,f>fRange(end,1) & f<fRange(end,2),:),2),3));
    ...
    set(gcf,'unit','inches')
    set(gcf,'position',[0 0.5 16 5*(length(plotNum)+1)])
...
%          pcolor(t(times),f,log10(Conv2Trim(h1,h2,y(:,:,plotNum(j)))'))
         pcolor(t(times),sqrt(f),log10(Conv2Trim(h1,h2,y(:,:,plotNum(j)))'))
         shading interp
%         imagesc(t(times),f,log10(y(:,:,plotNum(j)))')
        % imagesc(t(times),f,log10(y(:,:,j)'))
        plot(t(times),sqrt(ConvTrim(thetaFreq(times,plotNum(j)),ones(filtLen,1)./filtLen)));
