...
while (setnewtriggers)
    ...
    while (moreboxes)
        figure(1)
        zoom on;
        input('Select the top delay area.\n   Then click back in this window and hit ENTER...','s');
        lpx = xlim;
        lpy = ylim; 
        if xlim==xlimits & ylim==ylimits,
            fprintf('*********\nThe whole maze area is not a reasonable selection\n*********\n');
    ...
    figure(1)
    zoom on;
    input('Select the top maze corner.\n   Then click back in this window and hit ENTER...','s');
    rcx = xlim;
    rcy = ylim;
    if xlim==xlimits & ylim==ylimits,
        fprintf('*********\nThe whole maze area is not a reasonable selection\n*********\n');
    ...
    figure(1)
    zoom on;
    input('Select the bottom maze corner.\n   Then click back in this window and hit ENTER...','s');
    lcx = xlim;
    lcy = ylim;
    if xlim==xlimits & ylim==ylimits,
        fprintf('*********\nThe whole maze area is not a reasonable selection\n*********\n');
...
while ~donewithfile
    while (~isempty(intrigzone) & ~isempty(notintrigzone))
        ...
        plot(xlimits(2)-whldat(newtrialtrigger(1)-1+find(rWaterPort(newtrialtrigger(1):endoftrial(end))),1),whldat(newtrialtrigger(1)-1+find(rWaterPort(newtrialtrigger(1):endoftrial(end))),2),'.','color',[1 0 1],'markersize',7,'linestyle','none');
        plot(xlimits(2)-whldat(newtrialtrigger(1)-1+find(lWaterPort(newtrialtrigger(1):endoftrial(end))),1),whldat(newtrialtrigger(1)-1+find(lWaterPort(newtrialtrigger(1):endoftrial(end))),2),'.','color',[1 0 1],'markersize',7,'linestyle','none');
    ...
    if ~isempty(find(RightRight)),
        subplot(3,4,3)
        hold on
        plot(xlimits(2)-whldat(:,1),whldat(:,2),whldat(:,1),'.','color',[1 1 0],'markersize',7,'linestyle','none');
    ...
    if i(1) == 'y'
        ...
        if i(1) == 'y'
            ...
            clf;
            subplot(2,1,1);
            plot(xlimits(2)-whldat(:,1),whldat(:,2),'.','color',[1 0 0],'markersize',7,'linestyle','none');
            set(gca,'xlim',xlimits,'ylim',ylimits);
            subplot(2,1,2);
