...
% MakeWhlFile_3spots_fixhd(fileBase,nSamplesPerScreen)
%
...
function MakeWhlFile_3spots_fixhd(fileBase,nSamplesPerScreen,videoRes)
if nargin<1
    fprintf('Usage: MakeWhlFile_3spots(fileBase[,nSamplesPerScreen, videoRes([x, y])])\n');
    return;
end
if nargin < 2,
...
syncEEG = SchmittTrigger_sm(eeg,MeanEeg+StdEeg/4,MeanEeg-StdEeg/4);
...
if ~isempty(spots),
  ...
  syncVideo = SchmittTrigger_sm(PixCnt,Thresh,0);
% keyboard
  ...
  while 1,
      ...
      if strcmp(i,'done'),
          ...
          [syncVideo,syncEEG] = DisplaySYNC_sm(syncVideo,syncEEG,nSamplesPerScreen);
      end
      if 0
      figure(1);
      subplot(2,2,1);
...
if ~isempty(events),
  ...
  while 1,
      ...
      if strcmp(i,'done'),
          ...
          [syncEvents,syncEEG] = DisplaySYNC_sm(syncEvents,syncEEG,nSamplesPerScreen);
      end
      figure(1);
      subplot(2,2,1);
      plot(diff(syncVideo),'.-')
...
if ~isempty(spots),
  ...
  clf;
 % keyboard
  %rgb = ycbcr2rgb(spots(GoodSpots,7:9)); % Gives worse discrimination
  rgb = (spots(GoodSpots,7:9));
