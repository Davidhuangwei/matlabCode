function DetectPhasicBursts04(fileBaseCell,fileExt,varargin)
try
...
for j=1:length(fileBaseCell)
    ...
    if exist([fileBase '/' fileBase '.sts.REM'],'file')
        epochs = load([fileBase '/' fileBase '.sts.REM']);
        behav = 'rem';
    elseif exist([fileBase '/' fileBase '.sts.RUN'],'file')
        epochs = load([fileBase '/' fileBase '.sts.RUN']);
        behav = 'run';
        runFiles = cat(1,runFiles,j);
    end
    if ~isempty(epochs)
        if preWhitenBool
...
%                 if isempty(arModel)
%                     [eeg arModel] = WhitenSignal(eeg,[],1);
%                 else
%                  [eeg] = WhitenSignal(eeg,[],1,arModel);
%                 end
            end
            winLen = 1;
            params.tapers = [1 1];
...
%             catT{j,1} = cat(1,catT{j,1},t>=epochs(k,1)/eegSamp & t<=epochs(k,2)/eegSamp);
%             catTo{j,1} = cat(1,catTo{j,1},t(t>=epochs(k,1)/eegSamp & t<=epochs(k,2)/eegSamp));
            selInd = t>=epochs(k,1)/eegSamp & t<=epochs(k,2)/eegSamp;
            catTo = cat(1,catTo,t(selInd)');
%             if strcmp(behav,'run')
%                 runTimes = cat(1,runTimes,[t(find(selInd,1,'first')) t(find(selInd,1,'last'))]);
%             end
...
%          pcolor(t(times),(f).^e,log10(y(:,:,plotNum(j))'))
 if preWhitenBool
        ...
        pcolor(t(times),(f).^e,log10(Conv2Trim(h1,h2,...
            y(:,:,plotNum(j))./repmat(mean(y(:,:,plotNum(j)),1),[size(y,1),1,1]))'))
    end
         shading interp
%         imagesc(t(times),f,log10(y(:,:,plotNum(j)))')
...
%         set(gca,'yticklabel',num2str(sqrt(str2num(get(gca,'yticklabel')))))
    ...
    threshVal = medianPow+numSD*sdPow;
    plot(t(times),repmat(threshVal,size(t(times))),'k')
    runTimes = [];
    for j=1:length(runFiles)
        runTimes = cat(1,runTimes,t(fileNum == runFiles(j))');
    end
    plot(runTimes',repmat(threshVal,size(runTimes')),'r')
    % plot(t([lmin2 lmin2])',repmat(yLimits,[length(lmin2) 1])','--r')
