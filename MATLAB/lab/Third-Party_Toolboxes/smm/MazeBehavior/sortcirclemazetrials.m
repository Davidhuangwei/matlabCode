...
if exist([filebase '_whl_indexes.mat'],'file')
        while (1)
        i = input('Would you like to load the trigger zones from file? (yes/no)', 's');
...
while (setnewtriggers)
    ...
    while (moreboxes)
        ...
        if xlim==xlimits & ylim==ylimits,
            ...
            set(gca,'xlim',xlimits,'ylim',ylimits);
            if isfield(triggerZones,'rWaterPort')
                nBoxes = size(getfield(triggerZones,'rWaterPort'),1) + 1;
            else
                nBoxes = 1;
            end
            triggerZones = setfield(triggerZones,'rWaterPort',{nBoxes,1:4}, ...
                [rpx(1) rpy(1) rpx(2) rpy(2)]);
    ...
    while (moreboxes)
        ...
        if xlim==xlimits & ylim==ylimits,
            ...
            set(gca,'xlim',xlimits,'ylim',ylimits);
            if isfield(triggerZones,'lWaterPort')
                nBoxes = size(getfield(triggerZones,'lWaterPort'),1) + 1;
            else
                nBoxes = 1;
            end
            triggerZones = setfield(triggerZones,'lWaterPort',{nBoxes,1:4}, ...
                [lpx(1) lpy(1) lpx(2) lpy(2)]);
