function  ExampleSpectAll_04(filebasemat,fileext,nchannels,channel,lowband,highband)
...
figure(1)
set(gcf,'name',['GammaSpectExamplesSelected_' num2str(lowband) '-' num2str(highband) 'Hz_01'])
...
keyboard
        enterDelay = beginRawWhl-1+find(whlMazeLoc(beginRawWhl:endRawWhl,3)==1,1,'first');
        exitDelay = beginRawWhl-1+find(whlMazeLoc(beginRawWhl:endRawWhl,5)==1,1,'first');
        goodWhlInd = [beginRawWhl:enterDelay-1 exitDelay:endRawWhl];
...
%             integPow(k,:) = 10*log10(sum(filt(beginEEG:endEEG,:).^2));
            for m=1:length(chans)
                [wave period scale coi] = xwt(eeg(beginEEG-winLength:endEEG+winLength,m),eeg(beginEEG-winLength:endEEG+winLength,m),'Pad',1,'DJ',DJ,'S0',S0,'J1',J1);
                fo = 1./period.*eegSamp;
                logWave = 10.*log10(wave(fo>=lowband & fo<=highband,:));
                for n=1:size(logWave,1)
                    smoothWave(n,:) = smooth(logWave(n,:),500);
                end
                cutWave = smoothWave(:,winLength+1:end-winLength);
               figure(1)
                subplot(nSubplots,nHorzSubplots,(mazePlotSize+m)*nHorzSubplots-tracePlotWidth:(mazePlotSize+m)*nHorzSubplots-1)
                hold on
               pcolor(plotStart:plotStart+endEEG-beginEEG,fo(fo>=lowband & fo<=highband),cutWave-repmat(mean(cutWave,2),1,size(cutWave,2)))
...
%                 set(gca,'ylim',[65 85])
                ...
                plotStart = plotStart+endEEG-beginEEG+1;
%                 keyboard
        end
%         keyboard
%         reportFigBool = [];
