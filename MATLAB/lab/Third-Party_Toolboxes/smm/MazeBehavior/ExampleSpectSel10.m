function  ExampleSpectSel10(fileBaseCell,fileext,nchannels,chans,lowband,highband)
%Fs = 1250;
% lowband = 40;
% highband = 120;
forder = 312;
...
% plot limits
        ...
        for m=1:length(chans)
                ...
                smoothWave = [];
                gaussianX = gausswin(winLength)/sum(gausswin(winLength));
                gaussianY = gausswin(length(find(fo>=lowband & fo<=highband)))/sum(gausswin(length(find(fo>=lowband & fo<=highband))));
...
%                  cla
                ...
                shading interp
                set(gca,'clim',[-3 3])
                plot((saveCutFilt(m,:)-mean(saveCutFilt(m,:)))/std(saveCutFilt(m,:))*8+mean([lowband highband]),'color',[0.35 0.35 0.35])
...
%                 plot([plotStart+endEEG-beginEEG plotStart+endEEG-beginEEG],freqLim,['--' plotColors(k)])
                ...
                if k==length(fields)
                    text(endEEG-exitDelayEEG+2+enterDelayEEG-beginRawEEG + 300,mean(freqLim),selChanNames{m});
                end
                ylabel('Freq');
%                 shading interp
...
%         end
        ...
        if reportFigBool(1) %& sum(reportFigBool)>=3
          ReportFigSM(1,[pwd '/NewFigs/GammaTrialExamples2/'],[],[],{{[fileBase], ['time=' num2str(time)],num2str(reportFigBool)}});
        end
