function DetectPhasicRem04_test(fileExt,varargin)
try
...
%     chanLoc = LoadVar(['ChanInfo/ChanLoc_Min' fileExt]);
    ...
    chan = setdiff(chan,badChan);
    nChan = load(['ChanInfo/NChan' fileExt '.txt']);
    plotChan = chan;
%     plotChan = chan([2,4,5]);
end
[fileBaseCell,nChan,chan,chanGroupName,plotChan,winLen,phasicRefrac,defSD,preWhitenBool,recalcBool,fRange] = ...
    DefaultArgs(varargin,{LoadVar('FileInfo/RemFiles'),nChan,chan,'SelChan',plotChan,1024,2,1.5,1,0,[0 250]});
...
while 1
   ...
   integPow = ConvTrim(thetaFreq(times,plotNum(4)),ones(filtLen,1)./filtLen);
    filtLen = 3;
    integPow2 = 10*log10(squeeze(mean(mean(y(times,f>fRange(end,1) & f<fRange(end,2),:),2),3)));
