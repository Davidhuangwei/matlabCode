function  ExampleSpectSel07(filebasemat,fileext,nchannels,channel,lowband,highband)
%Fs = 1250;
lowband = 40;
highband = 120;
forder = 626;
...
analDir = ['CalcRunningSpectra9_noExp_MidPoints_MinSpeed0Win' num2str(winLength) fileext];
...
%                 plot([plotStart+endEEG-beginEEG plotStart+endEEG-beginEEG],freqLim,['--' plotColors(k)])
                ...
                if k==length(fields)
                    text(endEEG-exitDelayEEG+2+enterDelayEEG-beginRawEEG + 150,mean(freqLim),selChanNames{m});
                end
                ylabel('Freq');
%                 shading interp
