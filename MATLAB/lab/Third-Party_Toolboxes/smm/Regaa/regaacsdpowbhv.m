function regaacsdpowbhv(filebase, nchannels, channel, powdata)
    
eegchdata = readmulti([filebase '.csd' ], nchannels, channel);
...
while ((j<whlm) & (j/whlsamp*eegsamp+(timewindow)*eegsamp/2)<eegm)
    ...
    hold on;
    for i=1:9
        line([i*eegsamp*timewindow/10 i*eegsamp*timewindow/10], [-eegwinheight eegwinheight], 'color' , [0 0 0]);
    end
    ...
    line([eegsamp*timewindow/2 eegsamp*timewindow/2],[-eegwinheight eegwinheight],'color',[1 0 0]);
    set(gca,'xlim',[0 timewindow*eegsamp],'ylim', [-2 5]);
    set(gca, 'xtick', [0 eegsamp/2 eegsamp], 'xticklabel', [j/whlsamp-timewindow/2 j/whlsamp j/whlsamp+timewindow/2]);
