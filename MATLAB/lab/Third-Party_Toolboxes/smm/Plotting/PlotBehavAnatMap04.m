...
function PlotBehavAnatMap04(depVar,fileExt,spectAnalDir,analRoutine,varargin)
[colorLimits,interpFunc,glmVersion,subtractMeanBool] ...
    = DefaultArgs(varargin,{[],'linear','GlmWholeModel08',1});
...
%  keyboard
    ...
    try
%         ImageScRmNaN(Make2DPlotMat(grandMean,chanMat,badChan,interpFunc),cLimits)
        imagesc(Make2DPlotMat(grandMean,chanMat,badChan,interpFunc),cLimits)
    catch
%         ImageScRmNaN(Make2DPlotMat(grandMean,chanMat,badChan),cLimits)
        imagesc(Make2DPlotMat(grandMean,chanMat,badChan),cLimits)
    end
...
%             ImageScRmNaN(Make2DPlotMat(temp,chanMat,badChan,interpFunc),cLimits);
            imagesc(Make2DPlotMat(temp,chanMat,badChan,interpFunc),cLimits);
       catch
%             ImageScRmNaN(Make2DPlotMat(temp,chanMat,badChan),cLimits);
            imagesc(Make2DPlotMat(temp,chanMat,badChan),cLimits);
        end
