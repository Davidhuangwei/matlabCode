function plotchanspectrum(F,pspec,chanmat,badchan,samescale,dbscale,removeexp,color,lineWidth)
    ...
    for y=1:chan_y
        for x=1:chan_x
            if isempty(find(badchan==chanmat(y,x,z))), % if the channel isn't bad
                ...
                hold on;
                if dbscale,
                    plot(F(1:findfreq(1)),10*log10(pspec(1:findfreq(1),chanmat(y,x,z))),'color',color,'LineWidth',lineWidth);
                    if samescale,
                        ymin = min(min(ymin,min(min(10*log10(pspec(1:findfreq(1),chanmat(y,x,z)))))+headRoom),ylim(1));
                        ymax = max(max(ymax,max(max(10*log10(pspec(1:findfreq(1),chanmat(y,x,z)))))+headRoom),ylim(2));
                    end
                else
                    if removeexp,
                        [beta,pspec(:,chanmat(y,x,z))] = hajexpfit(F,log(pspec(:,chanmat(y,x,z))));
                    end
                    plot(F(1:findfreq(1)),pspec(1:findfreq(1),chanmat(y,x,z)),'color',color,'LineWidth',lineWidth);
                    if samescale, 
                        ymin = min(min(ymin,min(min(pspec(1:findfreq(1),chanmat(y,x,z))))+headRoom),ylim(1));
                        ymax = max(max(ymax,max(max(pspec(1:findfreq(1),chanmat(y,x,z))))+headRoom),ylim(2));                     
                    end
                end
            end
        end
