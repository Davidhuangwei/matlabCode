function PlotBehavHist01(animalDirs,depVar,fileExt,spectAnalDir,analRoutine,varargin)
[catMethod,saveDir,chanLocVersion,glmVersion] ...
    = DefaultArgs(varargin,{'trial','PhasePaper','Min','GlmWholeModel08'});
chanDir = 'ChanInfo/';
...
chanLocVersion = 'Min';
depCell = LoadSpectAnalResults01(animalDirs,spectAnalDir,fileExt,depVar,glmVersion,...
...
for j=1:size(selChansCell,1)
    for m=1:size(depCell,1)
        ...
        hold on
        [n,x] = hist(depCell{m,end}{j});
        plot(x,n,'color',plotColors(m))
    end
