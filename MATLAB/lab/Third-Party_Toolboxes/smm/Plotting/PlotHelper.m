...
if resizeWinBool
    ...
    set(gcf, 'Position', [figHorzOffset,figVertOffset,figXYFactor*figSizeFactor*(size(plotData,1)),figSizeFactor])
end
...
try
    h = ImageScRmNaN_old(Make2DPlotMat(squeeze(a),chanMat,badChans,interpFunc),colorLimits);
catch
    fprintf('\nBAD VALUES: %s\n',[titlesBase{j} titlesExt])
    h = ImageScRmNaN_old(Make2DPlotMat(squeeze(a),chanMat,badChans),colorLimits);
end
    if plotAnatBool
