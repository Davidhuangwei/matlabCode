%
%function ReportFigSM(FigHandleMat, UserPath, FileNameCell, plotSize, Comment, Resolution, Preview )
...
if ~exist('UserPath','var') | isempty(UserPath)
    UserPath = pwd;
end
...
for j=1:length(FigHandleMat)
    ...
    LongFileName = [UserPath '/NewFigs/' FileName '.html'];
    DirName = [UserPath '/NewFigs/' FileName];
    if exist([UserPath '/NewFigs'])~=7
        mkdir(UserPath,'NewFigs');
    end
    if FileExists(LongFileName)
        ...
        mkdir([UserPath '/NewFigs'],FileName);
        FigIndex = 1;
    end
