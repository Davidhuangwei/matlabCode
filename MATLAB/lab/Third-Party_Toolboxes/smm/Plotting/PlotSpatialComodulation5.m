function PlotSpatialComodulation5(taskType,fileExt,chanMat,badchan,winLength,NW,thetaFreq,gammaFreq,binSize,smoothRad)
...
spatialCorrCoefs = getfield(spatialComodStruct,'spatialCorrCoefs');
...
figureCell{i,j,k} = SmoothSkipNaN(TrimBorderNaNs(newSpatialCorrCoefs(:,:,chanMat(i,j,k),2,1),2),smoothRad);
...
figureCell2{i,j,k} = log10(SmoothSkipNaN(TrimBorderNaNs(newSpatialPvalues(:,:,chanMat(i,j,k),2,1),2),smoothRad));
    %figure(1)
