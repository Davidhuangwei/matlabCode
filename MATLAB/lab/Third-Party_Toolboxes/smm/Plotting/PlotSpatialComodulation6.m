...
figure(2)
subplot(1,3,3)
imagesc(SmoothSkipNaN(SmoothSkipNaN(newNPoints,smoothRad+1,0),smoothRad,1))
set(gca,'clim',[0 cLimits(2)]);
colorbar
...
figureCell{i,j,k} = SmoothSkipNaN(SmoothSkipNaN(TrimBorderNaNs(newSpatialSlopes(:,:,chanMat(i,j,k)),2),smoothRad+1,0),smoothRad,1);
figureCell2{i,j,k} = SmoothSkipNaN(SmoothSkipNaN(TrimBorderNaNs(newSpatialRSquare(:,:,chanMat(i,j,k)),2),smoothRad+1,0),smoothRad,1);
...
figureCell3{i,j,k} = SmoothSkipNaN(SmoothSkipNaN(log10(TrimBorderNaNs(newSpatialPvalues(:,:,chanMat(i,j,k)),2)),smoothRad+1,0),smoothRad,1);
    %figure(1)
