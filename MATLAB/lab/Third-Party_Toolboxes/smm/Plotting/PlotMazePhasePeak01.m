...
% badchan,saveMazePow,calcZ,normalizeBool,colorLimits,maxscale,textBool,plotRes,plot_low_thresh)
%
...
smoothMazePowFile = [taskType '_' fileNamesInfo ...
    fileExt '_ref' num2str(refChan) '_' num2str(freqRange(1)) '-' num2str(freqRange(2))...
    'Hz_Win' num2str(winLength) '_SmoothMazePhasePeak.mat'];
...
if strcmp(i,'y') | strcmp(i,'')
    ...
    if exist(pos_pow_sumFile,'file')
        ...
        pos_pow_sumFile
        YOU_NEED_TO_RUN_CalcPosPhasePeakSum
    end
...
%                 end
                ...
                elseif normalizeBool
                       plotMazePow(:,:,chanMat(y,x,z)) = angle(plotMazePow(:,:,chanMat(y,x,z))) ...
                           - angle(meanPowthisChan);
                       plotMazePow(:,:,chanMat(y,x,z)) = 180/pi*angle(complex(cos(plotMazePow(:,:,chanMat(y,x,z))),sin(plotMazePow(:,:,chanMat(y,x,z)))));
                    colormap(LoadVar('ColorMapSean'));
                else
                    plotMazePow(:,:,chanMat(y,x,z)) = angle(plotMazePow(:,:,chanMat(y,x,z)));
                    plotMazePow(:,:,chanMat(y,x,z)) = 180/pi*angle(complex(cos(plotMazePow(:,:,chanMat(y,x,z))),sin(plotMazePow(:,:,chanMat(y,x,z)))));
%                     colormap(flipud(colormap('hsv')));
                    
                    temp = colormap('hsv');
                    temp = flipud(temp);
                    temp=[temp(end-18:end,:);temp(1:end-19,:)];
                    colormap(temp)
% 
%                     temp = colormap('hsv');
%                     temp = flipud(temp);
%                     temp=[temp(end-16:end,:);temp(1:end-17,:)];
%                     colormap(temp)

                    
                end
                %[toPlotY toPlotX] = find(plotMazePow(:,:,chanMat(y,x,z)) > -1);
                  ImageScRmNaN(plotMazePow(1:plotRes:end,1:plotRes:end,chanMat(y,x,z)),colorLimits,[1 1 1]);
                  colorbar
%                   keyboard
%   
                ...
                if calcZ | normalizeBool
                    title([num2str(chanMat(y,x,z)) ': ' num2str(180/pi*angle(meanPowthisChan),3)]);
                else
...
outname = [taskType '_' fileNamesInfo ...
    fileExt '_' num2str(freqRange(1)) '-' num2str(freqRange(2)) 'Hz_Win' num2str(winLength) '_NW' num2str(NW) peak '_SmoothMazePhasePeak.mat'];
...
if textBool
    if 1
        for i=1:size(chanMat,3)
            ...
            text(100,50,{[num2str(freqRange(1)) '-' num2str(freqRange(2)) 'Hz PhasePeak'],fileExt,'',...
            ...
            titleText = ['SmoothMazePhasePeak_' taskType '_' fileNamesInfo ...
