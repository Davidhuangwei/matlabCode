function PlotMazePowNormByControl03(expTaskType,expFileBaseCell,controlTaskType,controlFileBaseCell,fileNameFormat,fileExt,freqRange,winLength,NW,chanMat,badchan,dbBool,samescale,colorLimits,textBool,plotRes,plot_low_thresh)
% function plotpowmapsbat9(taskType,fileBaseMat,fileNameFormat,fileExt,freqRange,winLength,NW,chanMat,badchan,saveMazePow,calcZ,percentageBool,DB,samescale,minscale,maxscale,textBool,plotRes,plot_low_thresh)
%
...
%         fileExt '_' num2str(freqRange(1)) '-' num2str(freqRange(2)) 'Hz_SmoothMazePow' DB '.mat'];
% 
%     pos_pow_sumFile = [taskType '_' fileBaseMat(1,[1:7 10:12 14 17:19]) '-' fileBaseMat(end,[7 10:12 14 17:19]) ...
%         fileExt '_' num2str(freqRange(1)) '-' num2str(freqRange(2)) 'Hz_pos_pow_sum' DB '.mat'];
% end
if fileNameFormat == 2,
    expFileNamesInfo = GetFileNamesInfo(expFileBaseCell,2);
    expSmoothMazePowFile = [expTaskType '_' expFileNamesInfo ...
        fileExt '_' num2str(freqRange(1)) '-' num2str(freqRange(2)) ...
        'Hz_Win' num2str(winLength) '_NW' num2str(NW) '_SmoothMazePow' DB '.mat'];
    ...
    for y=1:chan_y
        for x=1:chan_x
            ...
            if isempty(find(badchan==chanMat(y,x,z))), % if the channel isn't bad
                ...
                if dbBool
                    normMazePow(:,:,chanMat(y,x,z)) = 10*log10(normMazePow(:,:,chanMat(y,x,z)));
                end
...
if textBool
    if 1
        for i=1:size(chanMat,3)
            ...
            if fileNameFormat == 0
                NOT_YET_READY
                text(-625,-500,{[num2str(freqRange(1)) '-' num2str(freqRange(2)) 'Hz Power'], [DB],fileExt,'',...
