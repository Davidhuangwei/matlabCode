...
[selChanBool chanLocVersion catMethod indepVarBool subMedBool] = DefaultArgs(varargin,{0,'Min','trial',0,0});
                ...
                for p=1:length(anatFields)
                    if strcmp(anatFields,'')
                        ...
                        for q=1:length(chanLoc.(anatFields{p}))
                            ...
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            %%%% Allows loading of non-chan data (e.g.
                            %%%% speed) for plotting as indepVar
                            if (ndims(measVar{s,end})==1) | ...
                                    (ndims(measVar{s,end})==2 & size(measVar{s,end},2)==1)
                                tempVar = repMat(measVar{s,end},[1 max(goodChans)]);
                            else
                                tempVar = measVar{s,end};
                            end
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%                                         tempVar = squeeze(measVar{s,end}(:,goodChans(t),:,:,:,:));
                                      ...
                                      tempSpec2{s,r,p,q} = cat(1,tempSpec2{s,r,p,q},...
                                            squeeze(tempVar(:,goodChans(t),:,:,:,:)));
                                        %numSegDays(s,r,p,q) = size(measVar{s,end},1);
                                    case 'chan'
                                        tempSpec2{s,r,p,q} = cat(1,tempSpec2{s,r,p,q},...
                                            shiftdim(mean(tempVar(:,goodChans(t),:,:,:,:),1),1));
                                        %                                         squeeze(mean(measVar{s,end}(:,goodChans(t),:,:,:,:),1)));
                                        %numSegDays(s,r,p,q) = size(measVar{s,end},1);
                                    case 'shank'
                                        tempSpec2{s,r,p,q} = cat(1,tempSpec2{s,r,p,q},...
                                            shiftdim(mean(mean(tempVar(:,goodChans,:,:,:,:),1),2),1));
                                        %numSegDays(s,r,p,q) = size(measVar{s,end},1);
                                end
                            end
                        end
