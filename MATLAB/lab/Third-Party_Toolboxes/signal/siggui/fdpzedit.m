...
% Coordinate popup:
    cpop = fdspec('style','popupmenu','string',{'Rectangular' 'Polar'},...
...
case {'Lz_down','Lp_down'}
   ...
   % NOW FIND FILTVIEW and get ready for drag updating
   filtview_fig = findobj('type','figure','tag','filtview');
   global stabVal phaseVal
   global plots magHand phaseHand gdHand zzHand ppHand
   global imp impstem impc impstemc
   global step stepstem stepc stepstemc
   global zz pp bb aa 
   global Nfft dbmode freqPartial freqmode phaseMode
   global gdPartial gdOmega
   global impPartial stepPartial
   ...
   if ~isempty(filtview_fig)
       % create any needed partial responses
       bb = poly(zz)*k;  % bb and aa are the numerator and denom. of
       aa = poly(pp);    % the 'unchanging' part of the filter - the part
                         % that stays constant during the drag operation
                         
       FVud = get(filtview_fig,'userdata');
       FDud = get(gcf,'userdata');
       focusIndex = get(FDud.ht.filtMenu,'value');
       plots = FVud.prefs.plots;
       if plots(1) | plots(2)
           magHand = FVud.lines(focusIndex).mag;
           phaseHand = FVud.lines(focusIndex).phase;
           if plots(1)
               set(magHand,'erasemode','xor')
           end
           if plots(2)
               set(phaseHand,'erasemode','xor')
           end
           Nfft = FVud.prefs.nfft;
           dbmode = strcmp(FVud.prefs.magmode,'decibels');
           %freqlogscale = strcmp(FVud.prefs.freqscale,'log');   
           warnsave = warning;
           warning('off')   % avoid possible divide by zero warnings
           freqPartial = fft(bb(:).',Nfft)./fft(aa(:).',Nfft);
           warning(warnsave)
           freqmode = FVud.prefs.freqrange;
           switch freqmode
           case 1    % [0 Fs/2]
               freqPartial = freqPartial(1:Nfft/2);
           case 3    % [-Fs/2 Fs/2]
               freqPartial = fftshift(freqPartial);
           end
           phaseMode = FVud.prefs.phasemode;
       end       
       if plots(3)
           gdHand = FVud.lines(focusIndex).grpdelay;
           gdOmega = get(gdHand,'xdata')*2*pi/FVud.filt(focusIndex).Fs;
           warnsave = warning; 
           warning('off');  % turn off divide by zero warning
           gdPartial = grpdelay(bb,aa,gdOmega);
           warning(warnsave)
           set(gdHand,'erasemode','xor')
       end
       if plots(4)
           zzHand = FVud.lines(focusIndex).z;
           ppHand = FVud.lines(focusIndex).p;
           set([zzHand ppHand],'erasemode','xor')
       end
       if plots(5)
           imp = FVud.lines(focusIndex).imp;
           impstem = FVud.lines(focusIndex).impstem;
           impc = FVud.lines(focusIndex).impc;
           impstemc = FVud.lines(focusIndex).impstemc;
           Nimp = length(get(imp,'xdata'));
           impPartial = impz(bb,aa,Nimp);
           set([imp impstem ],'erasemode','background')
           if ~isempty(impc) & ishandle(impc) & strcmp(get(impc,'visible'),'on')
               set([impc impstemc],'erasemode','background')
           end
       end
       if plots(6)
           step = FVud.lines(focusIndex).step;
           stepstem = FVud.lines(focusIndex).stepstem;
           stepc = FVud.lines(focusIndex).stepc;
           stepstemc = FVud.lines(focusIndex).stepstemc;
           Nstep = length(get(step,'xdata'));
           stepPartial = filter(bb,aa,ones(1,Nstep));
           set([step stepstem ],'erasemode','background')
           if ~isempty(stepc) & ishandle(stepc) & strcmp(get(stepc,'visible'),'on')
               set([stepc stepstemc],'erasemode','background')
           end
       end
   else
       plots = [0 0 0 0 0 0];
...
case {'Lz_drag','Lp_drag'}
    ...
    if any(plots)
        ...
        if plots(1) | plots(2)
            warnsave = warning;
            warning('off')   % avoid possible divide by zero warnings
            freqSelected = fft(bs(:).',Nfft)./fft(as(:).',Nfft);
            warning(warnsave)
            switch freqmode
            case 1    % [0 Fs/2]
                freqSelected = freqSelected(1:Nfft/2);
            case 3    % [-Fs/2 Fs/2]
                freqSelected = fftshift(freqSelected);
            end
            H = freqSelected.*freqPartial;
            if plots(1)
                magYD = abs(H);
                if dbmode
                    warnsave = warning;
                    warning('off')   % avoid log of zero warnings
                    magYD = 20*log10(magYD);
                    warning(warnsave)
                end
                set(magHand,'ydata',magYD)
            end
            if plots(2)
                if freqmode < 3
                    pha = unwrap(angle(H));
                else
                    pha_neg = unwrap(angle(H(Nfft/2:-1:1)));
                    pha_neg = pha_neg(end:-1:1);
                    pha_pos = unwrap(angle(H((Nfft/2+1):end)));
                    pha = [pha_neg(:); pha_pos(:);];
                end
                if strcmp(phaseMode,'degrees')
                    pha = pha*180/pi;
                end
                set(phaseHand,'ydata',pha)
            end
        end   
        if plots(3)
            gdSelected = gdPartial;
            w = gdOmega;
            warnsave = warning; 
            warning('off');  % turn off divide by zero warning
            for i=1:length(zs)
                r = abs(zs(i));
                theta = angle(zs(i));
                gdSelected = gdSelected + (-r*cos(w-theta)+r^2)./(r^2-2*r*cos(w-theta)+1);
            end
            for i=1:length(ps)
                r = abs(ps(i));
                theta = angle(ps(i));
            ...
            warning(warnsave)
            set(gdHand,'ydata',gdSelected)
        end
        if plots(4)  % zero-pole plot
            set(zzHand,'xdata',real([zz(:); zs(:)]),'ydata',imag([zz(:); zs(:)]))
            set(ppHand,'xdata',real([pp(:); ps(:)]),'ydata',imag([pp(:); ps(:)]))
        end
        if plots(5)  % update impulse response
            impSelected = filter(bs,as,impPartial);
            setstem([imp impstem],get(imp,'xdata'),real(impSelected))
            if ~isempty(impc) & ishandle(impc) & strcmp(get(impc,'visible'),'on')
                setstem([impc impstemc],get(imp,'xdata'),imag(impSelected))
            end
        end
        if plots(6)  % update step response
            stepSelected = filter(bs,as,stepPartial);
            setstem([step stepstem],get(step,'xdata'),real(stepSelected))
            if ~isempty(stepc) & ishandle(stepc) & strcmp(get(stepc,'visible'),'on')
                setstem([stepc stepstemc],get(step,'xdata'),imag(stepSelected))
            end
        end
    end    
...
case {'Lz_up','Lp_up'}
    ...
    if plots(1)
        set(magHand,'erasemode','normal')
    end
    if plots(2)
        set(phaseHand,'erasemode','normal')
    end
    if plots(3)
        set(gdHand,'erasemode','normal')
    end
    if plots(4)
        set([zzHand ppHand],'erasemode','normal')
    end
    if plots(5)
        set([imp impstem],'erasemode','normal')
        if ~isempty(impc) & ishandle(impc) & strcmp(get(impc,'visible'),'on')
            set([impc impstemc],'erasemode','normal')
        end
    end
    if plots(6)
        set([step stepstem],'erasemode','normal')
        if ~isempty(stepc) & ishandle(stepc) & strcmp(get(stepc,'visible'),'on')
            set([stepc stepstemc],'erasemode','normal')
        end
    end
    
    clear global stabVal phaseVal
    clear global plots magHand phaseHand gdHand zzHand ppHand
    clear global imp impstem impc impstemc 
    clear global step stepstem stepc stepstemc
    clear global zz pp bb aa 
    clear global Nfft dbmode freqPartial freqmode phaseMode
    clear global gdPartial gdOmega
    clear global impPartial stepPartial
