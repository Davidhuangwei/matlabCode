...
% revisions
% 31-mar-03 wtext option

function hpol = my_polar(theta,rho,line_style,wtext)

if nargin<4 | isempty(wtext), wtext = 0; end
...
if wtext
    rt = 1.1*rmax;
    for i = 1:length(th)
        text(rt*cst(i),rt*snt(i),num2str(i*pi/6,3),...
             'horizontalalignment','center',...
             'handlevisibility','off');
        if i == length(th)
            loc = int2str(0);
        else
            loc = num2str(pi+i*pi/6,3);
        end
...
if strcmp(line_style,'auto')
    ...
    q = plot([xx; xx(1)],[yy; yy(1)],line_style);     % arrow for resultant - ES
    plot(0,0,'+b');
end
if nargout > 0
    hpol = q;
