function [pcnt, matl,matx,stepsize,fnew,how]=jsearchq(pcnt,fnew,oldx,matl,matx,sd,gdold,stepsize,how)
...
elseif pcnt==2  & fnew< matl(2)
	newstep=jcubici2(gdold,[matl(1);matl(2);fnew],[matx(1);matx(2);stepsize]);
...
%		if abs(newstep)<eps, newstep=rand(1)-0.5; how=[how,'RAND']; end
		...
		newstep=jcubici2(gdold,[matl(1);matl(3);fnew],[matx(1);matx(3);stepsize]);
	end
	matx(3)=stepsize;
...
elseif pcnt==2 & fnew>matl(2)
	matx(3)=stepsize;
	matl(3)=fnew;
	[stepsize]=jcubici2(gdold,matl,matx);
	pcnt=4;
