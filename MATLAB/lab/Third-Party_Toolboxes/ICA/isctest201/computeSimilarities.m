...
if ~testingComponents %Only used in testing mixing matrix
  ...
  if verbose
    fprintf('Computation of stabilized inverse of covariance matrix:\n')
  ...
  maxcondition=10; %maximum condition number allowed
...
%For each components, we store only the maximum similarities to other subjects
...
for subj1=1:subjects
  ...
  for subj2=1:subjects
      ...
      if subj1~=subj2
        ...
        %max(similarities),max(similarities'),pause
