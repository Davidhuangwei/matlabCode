...
%                                       each sensor.
%
%    PRIOR.method :       pos_kurtosis  positive kurtosis sets
%                                       prior.S='heavy_tail'. 
...
alpha=2; % if changed change also in logZ0_heavy_tail
...
sigma2=1; mu=1;
lambda=1./(1+lambda*sigma2);
logZ0terms=0.5*(log(lambda)-mu^2/sigma2-lambda.*(mu^2+gamma.^2*sigma2^2)+log(cosh(sigma2*gamma.*lambda)));
...
alpha=2; % if changed change also in heavy_tail.m
logZ0terms=0.5*gamma.^2./lambda-0.5*log(1+gamma.^2./(2*lambda*alpha));
logZ0=sum(sum(logZ0terms));
...
try 
  ...
  if strcmp(S_prior,'heavy_tail')
    Sigma_rel=100;                                            % default relative noise level 
  else
    Sigma_rel=1;
  end
end 
