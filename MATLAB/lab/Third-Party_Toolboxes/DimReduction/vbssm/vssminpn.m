...
while 1 % note there is a loop break after the F-calculation, of the sort: ~( (it<its) & (dF>1e-14) )
  ...
  AB = SigA*SA*B - SigA*GA*( B'*B + k*SigB );		% < A' B >
  BB = B'*B + k*SigB;					% < B' B >
  ...
  Db = UY - MD'*SigC*SC;
  G = diag( Ydd - SC'*SigC*SC -Db'*SigD*Db );
  ...
  if cFbool|finalit
    ...
    [F] = Fcalc(pa,pb,alpha,beta,gamma,delta,SigA,SigB,SigC,SigD,Bb,Db,SA,SC,GA,MD,G,lnZpn,Tn);
  ...
  if it>=dynhypit
    ...
    alpha = k./diag( k*SigA + SigA*( SA*SA' - 2*GA*B'*SA' + GA*( k*SigB+B'*B )*GA' )*SigA );
    beta  = k./diag( k*SigB + B'*B );
  end
  if it>=outhypit
    % ARD for the output process
    gamma = p./diag( p*SigC + SigC*( SC*rho*SC' - 2*SC*rho*D*MD' + p*MD*SigD*MD' + MD*D'*rho*D*MD' )*SigC );
    delta = p./diag( p*SigD + D'*rho*D );
  end
