...
%PURPOSE
%
%To draw cluster visualization where elements (points) belonging to
%same cluster are presented by "cluster hulls", polygons where the
%edge of the polygon is the convex hull of the points that belong
%to the same cluster.
%
%INPUT []'s mean optional
%
%mode      (string) 'edge' or 'fill'
%coord     (Mx2 matrix) coordinates of points representing the 
%  clustered elements
%partition (Mx1 vector) contains partition of objects into
%  clusters, partition(i)  is the number of cluster that
%  object i belongs to. Cluster numbers must be intergers 1,2,...,K
%  where K is the number of clusters.  
%[color]   (Kx3 matrix) each line is a 1x3 RGB vector that defines
%  color for each cluster. Default color is red for all clusters
%  color(i,:)=[NaN NaN NaN] means that cluster(i) is not drawn at all.
%
%OUTPUT
%
...
%DETAILS
%
%In mode 'edge' each cluster hull is presented by a convex hull, if
%there are more than two points in the cluster. A two point cluster is
%presented by a line segment and a one point cluster by a
%cross. The face of the convex hull is invisible. 
%
%In mode 'fill' the cluster hulls can be filled with the specified
%color. Clusters having less than three members are ignored.
%
%NOTE This functions sets HOLD ON in the graphic axis
%
%SEE ALSO
% icassoShow
% icassoProjection
...
m=repmat(mean(coord(1:end-1,:)),size(coord,1),1);
coord_=coord-m;
coord_=coord_.*S;
coord=coord_+m;
