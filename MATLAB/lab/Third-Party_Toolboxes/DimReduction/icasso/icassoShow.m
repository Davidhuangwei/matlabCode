...
%bounded by a red convex hull. If the average similarity in a
%cluster is above a specified limit 'dense', (input argument pair
%'dense', 0.8 in our example) the graph lines inside the cluster
%are replaced by light red fill color. If the _minimum_
%intra-cluster similarity is above the same limit, the cluster fill
%color is clear red. The graph lines can be suppressed below a
%certain value T of similarity by giving input argument pair
%'limit', ('limit',0.1) in our example. You can suppress the
%similarity graph completely by giving input argument pair 'graph','off'. 
%
%Cluster labels are the same as in Figures 2 and 3.
%
%If you click on a cluster hull or a cluster label (number) the
%cluster label is stored in workspace variable CLUSTER and indices
%for the corresponding estimates are in workspace variable
%INDEX. (The variables are created if necessary). You can retirieve
%the estimates from the Icasso result struct with function
%icassoGet. For example, icassoGet(sR,'source',INDEX) would
%retrieve the estimated independent components in our example, and
%icassoSourceplot(sR,INDEX) would plot them. 
%
%Note that the function will automatically to change this threshold
%to keep the number of lines reasonable if more than 5000 are produced.
%
%Figure 2: Icasso: Cluster quality
%---------------------------------
%Shows cluster ranking. Left panel shows the value of the cluster
%ranking index (x-axis) for each cluster (label on y-axis) the
...
%--------------------------------------
%Shows centrotype estimates (independent component) for each
...
for cluster=1:level,
  ...
  set(h_edge(cluster,1),'userdata',[cluster find(index)], ...
		    'buttondownfcn',ishowButtonDownF);
  set(h_text(cluster,1),'userdata',[cluster find(index)],'buttondownfcn',ishowButtonDownF);
end
