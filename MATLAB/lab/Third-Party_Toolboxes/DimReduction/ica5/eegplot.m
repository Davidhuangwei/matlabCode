...
%           with (optional) gui controls (version 2.1)
% Usage: 
%        >> eegplot(data,srate,spacing,eloc_file,windowlength,title)
%        >> eegplot('noui',data,Fs,spacing,eloc_file,startpoint,color)
%
%    data         : Input data matrix (chans,timepoints) 
%    srate        : Sampling rate in Hz {default: 256 Hz}
%    spacing      : Space between channels (default: max(data)-min(data))
...
%
%    Option 'noui':  Display eeg in current axes without user controls

% 5/98 Colin Humphries, CNL / Salk Institute, La Jolla CA (colin@salk.edu) 
...
function [outvar1] = eegplot(data,p1,p2,p3,p4,p5,p6)
...
SPACING_EYE = 'off';              % spacing I on/off
...
if ~isstr(data)
  ...
  if ~isempty(eloc_file)            % Read in electrode names
    if eloc_file == 0
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'loadelect'
    ...
    timestring = ['[OBJ1,FIGH1] = gcbo;',...
		  ...
		  'FIH0 = get(OBJ1,''UserData'');',...
		  'AXH0 = findobj(''tag'',''eegaxis'',''parent'',FIH0);',...
		  'OUT1 = eegplot(''setelect'',LAB1,AXH0);',...
		  'if (OUT1);',...
		     'delete(FIGH1);',...
		  'end;',...
  ...
  case 'noui'
    ...
    data = p1;
    % usage: eegplot(data,Fs,spacing,eloc_file,startpoint,color)
