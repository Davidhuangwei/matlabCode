...
% Usage:
%    >> topoimage(data,'chan_locs');
% OR >> topoimage(data,[rows cols]);
%    >> topoimage(data,'chan_locs',[nfreqs ntimes],limits,title,...
%                       channels,axsize,colors,ydir) 
%
%   data      = data consisting of nchans images, size (freqs,times*chans) 
%  'chan_locs'= file of channel locations as in >> topoplot('example')
%                 Else: [rows cols] matrix of locations. Example: [6 4]
%  [freqs times] = (rows,cols) per image 
%  [limits]   = [mintime maxtime minfreq maxfreq mincaxis maxcaxis]  (times in ms) 
%                 {0 (or both caxis 0) -> use data limits)
...
% 12-10-99 Scott Makeig, CNL / Salk Institute, La Jolla CA from topoplot.m
% 1-16-00  debugged help msg and improved presentation -sm
% 3-16-00  added axcopy() -sm

function topoimage(data,loc_file,frames,limits,plottitle,channels,axsize,colors,ydr)
...
LINEWIDTH = 2.0;
YVAL = 10;          % plot horizontal lines at 10 Hz
...
if nargin < 1,
  ...
  if frames <=0,
     ...
     return
  elseif frames==1,
    fprintf('topoimage: cannot plot less than 2 frames per trace.\n');
    return
  else
    freqs = frames(1);
    times = frames(2);
...
%
  ...
  if ymax == 0 & ymin == 0,
    ...
    dy = (ymax-ymin)/(times-1);
    y=ymin*ones(1,times)+dy*(0:times-1); % compute y-values
    ymax = ymax*times/times;
  end
  if ymax<=ymin,
    ...
    for I=1:chans,        % for each data channel %%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      %
        ...
        axislcolor = get(gca,'Xcolor');   %%CJH
          ...
          imagesc(x,y,matsel(data,times,0,0,I)); % <---- plot image --------
        ...
        if YVAL>=ymin & YVAL<=ymax
          hp=plot([xmin xmax],[YVAL YVAL],'color',axislcolor);  % draw horizontal axis 
          set(hp,'Linewidth',1.9)
        end
        if xmin<0 & xmax>0 
          plot([0 0],[ymin ymax],'color',axislcolor);  % draw vertical axis 
        end
       ...
       if channels~=0,                               % print channames
          if ymin <= 0 & ymax >= 0,
            ...
            yht = mean(SIGN*data(I,1+P*times:1+P*times+times-1));
          end
          if ~ISRECT    % print before topographically arrayed image 

             % axis('off');
             h=text(xmin-NAME_OFFSET*xdiff,(ymax+ymin)*0.5,[channames(I,:)]); 
      ...
      % end
      if xmin<0 & xmax>0
        hold on; plot([0 0],[ymin ymax],'k','linewidth',2);
      end
      fprintf(' %d',I);
    end; % chan
  fprintf('\n');
...
%
      ...
      axis('off');
      if xmin <=0
          py=plot([0 0],[ymn ymx],'color','k'); % draw vert axis at zero
      else
          py=plot([xmin xmin],[ymn ymx],'color','k'); % vert axis at xmin
      end
      hold on
      if YVAL>=ymin
        px=plot([xmin xmax],[YVAL YVAL],'color',axislcolor); % draw horiz axis at 0
      else
        px=plot([xmin xmax],[ymin ymin],'color',axislcolor); 
      ...
      %
          ...
          axis('off');h=text(textx,(ymin+ymax)/2,ylabel);        % text Hz
            set(h,'Rotation',90);
            set(h,'FontSize',TICKFONTSIZE);         % choose font size
