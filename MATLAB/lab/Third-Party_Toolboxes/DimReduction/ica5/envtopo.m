...
%                                   = envtopo(data,weights,'chan_locs',...
%                                      [limits],[compnums],...
...
%                         {def|0 or both y's 0 -> y data limits}
% [compnums] = vector of components to plot {default|0 -> all}
% 'title'    = plot title {default|0 -> none}
% [plotchans]= data channels to use in computing envelopes {default|0 -> all}
% [voffsets] = vert. line extentions above the data max to disentangle plot
%              lines (left-to-right heads, values in y-axis plot units) {def|0 -> none}
...
%              solid color. Ex: [1] or [1 5] {def|0 -> no fill}
% Outputs:
...
% 3-10-98 Scott Makeig, CNL / Salk Institute, La Jolla from timtopo.m
% Edit History:
...
function [compvarorder,compvars,compframes,comptimes,compsplotted] = envtopo(data,weights,chan_file,limits,compnums,titl,plotchans,voffsets,colorfile,fill_comp_env)
...
if nargin>10
  help envtopo
  fprintf('envtopo(): too many arguments.\n');
  return
end
if nargin <10
...
if size(voffsets) == [1,1] & voffsets(1) == 0
  voffsets = zeros(1,MAXTOPOS); 
end
if nargin < 7
   plotchans=0;
end
if plotchans(1)==0 | isempty(plotchans)
   plotchans = 1:chans;
end
...
if compnums(1) == 0
    compnums = 1:wtcomps; % be default, all components
end
...
for c = 1:ncomps %%% find max variances and their frame indices %%%%%
  ...
  proj = icaproj(data,weights,eye(chans),0,compnums(c));
...
for c = 1:ntopos+1   % plot the computed component envelopes %%%%%%%%%%%%%%%%%%
  ...
  if c==1                                % use colors in original
    set(p,'LineWidth',2);                % component order
  else
    set(p,'LineWidth',1);
  end
  ...
  elseif mapcolors(c)>5                             % dot 6th-> comp. envs.
    set(p,'LineStyle',':','LineWidth',3);
    if all_bold
      set(p,'LineStyle','-','LineWidth',3);
    end
  ...
  if fill_comp_env(1)>0 & find(fill_comp_env==c-1) % plot the n-th component filled 
...
for t=1:ntopos % draw oblique lines from max env vals (or plot top)
  ...
  l1 = plot([(plottimes(t)-xmin)/width  ...
               topoleft+1/pos(3)*(t-1)*6*topowidth/5+topowidth*0.6],...
                 [data_y 0.68], ...
            colors(maporder(t)+1)); % 0.68 is bottom of topo maps
  if maporder(t)>15                        % thin/dot 11th-> comp. envs.
    set(l1,'LineStyle',':','LineWidth',1);
    if all_bold
     set(l1,'LineStyle','-','LineWidth',3);
    end
  elseif maporder(t)>10 
    set(l1,'LineStyle',':','LineWidth',2);
    if all_bold
     set(l1,'LineStyle','-','LineWidth',3);
    end
  elseif maporder(t)>5                     % dot 6th-> comp. envs.
    set(l1,'LineStyle',':','LineWidth',3);
    if all_bold
     set(l1,'LineStyle','-','LineWidth',3);
    end
  elseif maporder(t)>1
    set(l1,'LineStyle',colors(maporder(t)+1,2),'LineWidth',1);
      if colors(maporder(t)+1,2) == ':'
        set(l1,'LineStyle',colors(maporder(t)+1,2),'LineWidth',2);
      end
  end
  hold on
...
for t=1:ntopos % left to right order 
  ...
  topoplot(maxproj(:,t),chan_file,'style','both','emarkersize',3);
                                          % make the topoplot
  %ELSE headplot(winv(:,t),'chan.spline');% make a 3-d headplot
                                          % if available
  if t==1
