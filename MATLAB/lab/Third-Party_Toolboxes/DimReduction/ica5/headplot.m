...
% Usage:
% (Info  )  >> headplot('example') - show an example 'eloc_angles' file
% (Do once) >> headplot('setup',['eloc_angles'],['splinefile'],['comment'])
%     'eloc_angles' - file of electrode locations in spherical coords.
%                       See functions: cart2topo() and topo2sph()
%     'splinefile'  - name of spline file to save splining info into
%     'comment'     - optional string vector containing info for spline file
%
% (Thereafter )  >> headplot(values,'spline_file','Param','Value',...)
%     values       - vector containing value at each electrode position
%     'spline_file' - spline filename computed and saved by running 'setup'
%
% Optional Parameters (any order)  & Values
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Authors: Colin Humphries and Scott Makeig, CNL / Salk Institute, Feb. 1998
...
%
    if nargin<3|nargin>4
       fprintf('headplot(): setup requires 3-4 arguments.\n')
       return
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 'headplot(): spline_file "%s" not found. Run headplot in "setup" mode.\n',...
  ...
  if isstr(View)
    switch lower(View)
      ...
      otherwise
        fprintf('headplot(): Invalid View value %s',View)
        return
    end
  else
    if ~isstr(View)
