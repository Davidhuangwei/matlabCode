...
%           coherent phase difference between the processes. In this panel:
%           A value of -90 degrees (blue) means x leads y by a quarter cycle.
%           A value of  90 degrees (blue) means x lags  y by a quarter cycle.
...
% Usage: 
%      >> [coh,mcoh,times,freqs,cohboot,cohangles] = crossf(xdata,ydata,     ...
%                                              frames,tlimits,titl,          ...
%                                              srate,cycles,winsize,timesout,...
%                                              padratio,maxfreq,alpha);
% Inputs:
...
%       alpha       = Two-tailed bootstrap signif. probability {.03}
%                     Sets n.s. plotted output values to green (0). 
%                     Requires 5 output arguments!
%   Outputs: 
...
function [R,mbase,times,freqs,Rboot,Rangle,Rsignif] = crossf(X,Y,epoch,timelim,ftitle,Fs,varwin,winsize,nwin,oversmp,maxfreq,alpha)
...
DEFAULT_ALPHA   = .03;			% Two-sided significance probability threshold
...
fprintf('Trial timebase is %d ms before to %d ms after the stimulus\n',...
                       timelim(1),timelim(2));
fprintf('The window size is %d samples (%g ms).\n',winsize,2*wintime);
...
fprintf('The maximum frequency displayed is %g Hz.\n',maxfreq);
if nargout>4
...
map=hsv(300); % install cicular color map - green=0, yellow, orng, red, violet = max
