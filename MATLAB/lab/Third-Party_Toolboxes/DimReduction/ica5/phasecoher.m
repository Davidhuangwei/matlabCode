...
%                         [1,halfwin], [frames-halfwin,frames].
% Usage:
%     >> [amps,cohers       ] = phasecoher(data,frames,srate,freq,cycles); 
%     >> [amps,cohers,cohsig,ampsig,allamps] = phasecoher(data,frames,...
%                                                    srate,freq,cycles,...
%                                                       alpha,times,titl);
% Inputs:
...
%   cycles = cycles in Gaussian wavelet window (float)
%   alpha  = (0 0.1] significance probability threshold {default 0.01}
...
% Scott Makeig, CNL / Salk Institute, La Jolla CA 5-5-98
...
function [amps,cohers,cohsig,ampsig,allamps] = phasecoher(data,frames,srate,freq,cycles,alpha,times,titl)
...
fprintf('with a moving %d-frame analysis window.\n',winlength);

if frames < winlength
  fprintf(...
  'Epoch length (%d frames) too short for analysis with %g cycles.\n',...
                frames,                                 cycles);
  help phasecoher
  return
end
...
if ~isnan(alpha)  %%%%%%%%%%%%%% Compute cohsig/ampsig %%%%%%%%%%%%%%
 ...
 for f = 1:COHSIG_REPS %%%%%%%%%%%%%%% Bootstrap replications %%%%%%%%%%%
  ...
  for t = 1:trials %%%%%%%%%%%% trials %%%%%%%%%%%%%%%
    ...
    % amp = sqrt(imagpart.^2+realpart.^2);
    amp = abs(realpart+imagpart*i);
    if amp >= MIN_AMP
      tmpamps = tmpamps + amp;
  ...
  bootamp(f)   = tmpamps;
  bootcoher(f) = abs(realcoh+imagcoh*i);
...
if PLOT_IT %%%%%%%%%%%%%% make two-panel plot of results %%%%%%%%
  ...
  if alpha ~= nan                      % plot coher significance
    plot([times(1) times(winframes)],[cohsig cohsig],'r');
  end                                  
end
