...
%
% Usage:    >> [envdata] = envproj(data,weights,sphere,compnums);
%       or
...
%
% Output: envdata = [2,(frames*length(compnums)+1)] envelopes of data, comps.


% 01-23-97 -Scott Makeig  CNL / Salk Institute as envproject.m from plotproject
...
function [envdata] = envproj(data,weights,sphere,compnums,titl,limits,chanlist,compnames,colors)

FILL = 1; % 1;  % 1 = fill in component envelope unless ncomps>1

if nargin < 4,
    help envproj
    fprintf('envproj(): must have at least four arguments.\n');
    return
end
...
if sc~=chans,
     fprintf(...
  'envproj(): sizes of sphere and data matrices incompatible.\n');
    return
end;
if sr~=sc,
     fprintf('envproj(): sphere must be a square matrix.\n');
    return
end;
if wc~=sr,
     fprintf(...
  'envproj(): sizes of weights and sphere matrices incompatible.\n');
    return
end;
%
...
if max(compnums)>wr| min(compnums)<1,
 fprintf('\nenvproj(): Component index out of bounds (1:%d).\n',wr);
 return
end
...
for c=compnums,            % for each component 
  fprintf('%d ',c);
  % [icaproj] = icaproj(data,weights,sphere,datamean,compindex);
  proj = icaproj(data,weights,sphere,0,c); % let offsets be 
...
for c=1:numcomps+1;
  ...
  if n <= 6
    ...
    H(n)= plot(x,envdata(1,(c-1)*frames+1:c*frames)',...
                                     colors(c),'LineStyle',':','LineWidth',2);
    hold on;
          plot(x,envdata(2,(c-1)*frames+1:c*frames)',...
                                     colors(c),'LineStyle',':','LineWidth',2);
  end
