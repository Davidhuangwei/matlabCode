...
%           If a topo vector and electrode location file are given, the figure 
%           also shows a topoplot() of the specified scalp map.
%           If coher alpha is given, then bootstrap statistics are 
%           computed (from a distribution of NACCU (200) surrogate data epochs)
%           and non-significant features of the output plots are zeroed out 
%           (e.g., plotted in green).  If BASE_BOOT set in script, surrogate
%           ERSP data is drawn from windows with center times < DEFAULT_BASELN (0) 
%           Click on a subplot to view separately.
% Usage: 
%      >> [ersp,itc,powbase,times,freqs,erspboot,itcboot] = timef(data,        ...
%                                              frames,tlimits,titl,            ...
%                                              srate,cycles,winsize,timesout,  ...
%                                              padratio,maxfreq,tvec,eloc_file,...
%                                              alpha,marktime,powbase,pboot,rboot);
% Inputs:                                                   {defaults}
...
%       marktime    = time to mark with a dotted vertical line{def|nan->none}
...
function [P,R,mbase,times,freqs,Pboot,Rboot] = timef(X,epoch,timelim,ftitle,Fs,varwin,winsize,nwin,oversmp,maxfreq,topovec,eloc_file,alpha,marktime,powbase,pboot,rboot)
...
BASELINE_END    = 200;            % times < this are in the baseline.
...
if ~isnan(alpha)
   if BASE_BOOT
     fprintf('Bootstrap analysis will use averages drawn from the baseline only.\n')
   else
     fprintf('Bootstrap analysis will use averages drawn from the whole epochs.\n')
   end
end

if nargin<14
  marktime = DEFAULT_MARKTIME;
end
...
fprintf('Computing Event Related Spectrum Perturbation (ERSP) and\n');
fprintf('  Inter-trial Coherence (ITC) images based on %d trials\n',length(X)/epoch);
fprintf('  of %d frames sampled at %gHz.\n',epoch,Fs);
fprintf('Each trial contains samples from %d ms before to\n',timelim(1));
fprintf('  %d ms after the timelocking event.\n',timelim(2));
fprintf('The window size used is %d samples (%gms) wide.\n',winsize,2*wintime);
fprintf('The window is applied %d times at an average step\n',nwin);
fprintf('  size of %g samples (%gms).\n',stp,1000*stp/Fs);
fprintf('Results are oversampled %d times; the highest\n',oversmp);
fprintf('  frequency displayed is %gHz.\n',maxfreq);
if ~isnan(alpha)
  fprintf('Only significant values (bootstrap max/min %g) will be colored;\n',alpha) 
...
for i=1:trials
    ...
    Wn = find(Wn>0);
    if length(Wn)>0
	  P(:,Wn) = P(:,Wn) + PP(:,Wn); % add ~nan windows
	  R(:,Wn) = R(:,Wn) + RR(:,Wn);
	  Rn(Wn) = Rn(Wn) + ones(1,length(Wn));
    end
end % trial
fprintf('\nNow plotting...\n');

if min(Rn) == 0
  fprintf('timef(): No valid timef estimates for windows %s.\n',...
                         find(Rn==0));
  return
end
...
R = abs(R) ./ (ones(size(R,1),1)*Rn); % normalize coherence
...
if (PLOT_ERSP & PLOT_ITC) %%%%%%%% image both ERSP and ITC %%%%%%%%%%%%%%%%%%
    ...
    if ~isnan(marktime) % plot marked time
	   plot([marktime marktime],[0 freqs(max(dispf))],'--k','LineWidth',LINEWIDTH)
    end
	hold off
	...
	RR = R;
	if ~isnan(alpha)
		RR(find(RR < repmat(Rboot',[1 nwin]))) = 0;
	end
	...
	set(h(8),'YLim',[0 min(max(max(R(dispf,:))),1)]) % ,1 WAS 0.4 !
	title('ITC')
...
elseif PLOT_ERSP %%%%%%%%%%%% Image ERSP only %%%%%%%%%%%%%%%%%%
	...
	plot([0 0],[0 freqs(max(dispf))],'--m','LineWidth',LINEWIDTH)
    if ~isnan(marktime)
	   plot([marktime marktime],[0 freqs(max(dispf))],...
