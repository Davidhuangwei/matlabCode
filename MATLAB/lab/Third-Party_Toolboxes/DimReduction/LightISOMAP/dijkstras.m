function [D,Path] = dijk_sjaak(A,s,t)
...
P = zeros(length(s),n);
for i = 1:length(s)
   for j=1:length(t)  Path{i,j}=[]; end;  % init paths from s(i)
end
for i = 1:length(s)
   j = s(i); source=j;
   ...
   elseif isAcyclic == 2
      ...
      nLab = 0; UnLab = 1:n;isUnLab = logical(ones(n,1));
   end   
   while nLab < n & ~all(isLab)
      if isAcyclic
         ...
         j = UnLab(jj); UnLab(jj) = [];isUnLab(j) = 0;
      end; nLab = nLab + 1;
      
      if length(t) < n, isLab = isLab | (j == t); end
      Arcs = A(:,j); 
      %Arcs(Path{source,j})=0; Arcs(j)=0; 
      %Arcs(find(Arcs>0))=-log(Arcs(find(Arcs>0))/sum(Arcs))/log(2)+eps;
      [jA,kA,Aj] = find(Arcs);Aj(isnan(Aj)) = 0;            
      if isempty(Aj), Dk = Inf; else Dk = Dj + Aj; end;
      for ind=1:length(jA)
        node=jA(ind);
        if Dk(ind)<Di(node) 
          Path{source,node}=[Path{source,j} j];
         end
      end
      P(i,jA(Dk < Di(jA))) = j;
      Di(jA) = min(Di(jA),Dk);
