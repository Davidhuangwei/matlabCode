...
if (length(FreqCycl)>0)
    ...
    CCyclicCorrelationCoeff = 4*sum(abs(1/length(r)*abs(r).^2*(Exp.')).^2);
    NCCyclicCorrelationCoeff = 2*sum(abs(1/length(r)*r.^2*(Exp.')).^2);
else
    CCyclicCorrelationCoeff = 0;
    NCCyclicCorrelationCoeff = 0;
end

J = (mean(abs(r).^4)-abs(mean(r.^2)).^2-CCyclicCorrelationCoeff-NCCyclicCorrelationCoeff)/Den - 2;
