...
if size(sw,2)==1
 ...
 for ii=1:clunumcx   
    [CCHistograms{1,ii}(:,1) CCHistograms{1,ii}(:,2) ] =PointCorrel(spkhip,spkcx{ii},tbin*20,halfbinnum,0,20000,'scale');

    hipsw=spkinsw(spkhip,sw);
    [cxsw cxfr] =spkinsw(spkcx{ii},sw);
    [CCHistograms{2,ii}(:,1) CCHistograms{2,ii}(:,2) ] = PointCorrelEp(hipsw{1},cxsw{1},tbin*20,halfbinnum,0,20000,'scale',cxfr(1));
    [CCHistograms{3,ii}(:,1) CCHistograms{3,ii}(:,2) ] =PointCorrelEp(hipsw{2},cxsw{2},tbin*20,halfbinnum,0,20000,'scale',cxfr(2));
end
...
if ifprint==1
    ...
    plotCC(CCHistograms,{'all','in','out'});
