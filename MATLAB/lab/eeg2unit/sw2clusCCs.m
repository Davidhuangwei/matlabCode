% CCHistograms = sw2clusCCs(filename,elind,ifprint,tbin,halfbinnum,swtype,printname)
...
function CCHistograms = sw2clusCCs(filename,elind,ifprint,tbin,halfbinnum,swtype,printname)
...
%sw=sw(:,2);
[CCHistograms{1}(:,1) CCHistograms{1}(:,2)] = PointCorrel(sw,spk,tbin*20,halfbinnum,0,20000,'scale');
for ii=2:clunum
   spk=res(find(clu==ii));
   [CCHistograms{ii}(:,1) CCHistograms{ii}(:,2)] =PointCorrel(sw,spk,tbin*20,halfbinnum,0,20000,'scale');
end 
