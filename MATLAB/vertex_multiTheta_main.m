...
load('~/data/NeuronModel/Model1.mat')
usemodel=1;
SimulationSettings.simulationTime = 50000;%ms
...
if usemodel
    NeuronParams(1)=Neuron1;
    NeuronParams(1).compartmentXPositionMat=nX;
    NeuronParams(1).compartmentYPositionMat=nY;
    NeuronParams(1).compartmentZPositionMat=nZ;
    NeuronParams(1).compartmentLengthArr=sqrt(diff(nX,1,2).^2+diff(nY,1,2).^2+diff(nZ,1,2).^2)';
else
...
phases=[90 180];% CA3 MEC3
sphase=fix(phases/360*125);

NeuronParams(3).somaLayer = 1;     % input from MEC3
NeuronParams(3).modelProportion = 0.2;
PoissonN=0;
simuserials=1:SimulationSettings.simulationTime;
coserials=rand(1,floor(SimulationSettings.simulationTime))<.3;
beforepoints=find(abs(mod(1:floor(SimulationSettings.simulationTime/2),125)-sphase(2))<1);
coserials(beforepoints)=coserials(beforepoints-sphase(2)+sphase(1));
if PoissonN
    ...
    tt=1:(15+conum*10);% [1:SimulationSettings.simulationTime]
...
%     [zeros(1,ceil(SimulationSettings.simulationTime/2)), rand(1,floor(SimulationSettings.simulationTime/2))<.05]);
    sshape=(1+sin(70*pi/625*tt)).*exp(-(tt-max(tt)*.3).^2/2/(max(tt)/5).^2);
    Inputshape=filter(sshape/max(sshape),1,...rand(1,SimulationSettings.simulationTime)<.1);
    (abs(mod(simuserials,125)-sphase(2))<1).*coserials);
    % frequency =8; 
...
%         [rand(1,floor(SimulationSettings.simulationTime/2))<.05, zeros(1,ceil(SimulationSettings.simulationTime/2))]);
    sshape=(1+sin(40*pi/625*tt)).*exp(-(tt-max(tt)*.3).^2/2/(max(tt)/5).^2);
     Inputshape=filter(sshape/max(sshape),1,...rand(1,SimulationSettings.simulationTime)<.1);
    (abs(mod(simuserials,125)-sphase(1))<1).*coserials);
