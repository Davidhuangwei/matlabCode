% function FiltLFP(FileBase, FreqBins)
clear all
close all
...
sname=sprintf('LFP.Channels%d_%d.Freq%d_%d.mat',Channels,FreqBins(k,:))
save(sname,'filtLFP_temp')
filtLFP_temp=exp(sqrt(-1)*(angle(filtLFP_temp(2:end,:))-angle(filtLFP_temp(1:(end-1),:))));
for n=1:(2*nlag-1)
...
save 8_2% 724all bursts
% showFiltLFP
figure
imagesc(sq(abs(pl(:,2,:))-abs(pl(:,1,:))))%,[0,.26]
set(gca,'Xtick', xticks ,'Xticklabel',xlabels,'Ytick',ytick,'Yticklabel',ylabels)
title(cnames{1})