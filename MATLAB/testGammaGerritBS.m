...
Channels=[mec3,lm];%,radmec1,mec2,
chbelong=[length(mec3),length(lm)];%length(mec1),length(mec2),,length(rad)
...
Bursts=HP.BurstTime(HP.BurstChan>32 & HP.BurstChan<37 & HP.BurstFreq<100 & HP.BurstFreq>60)*FS;
...
save([FileBase,'.HPGB30-60Hz.fHighGamma.mat'])
clear all
close all
FileBase='g09-20120330';
load([FileBase,'.HPGB30-60Hz.fHighGamma.mat'])
    ...
    for n=1:nfr
        for m=1:t1
            for h=1:t2
                ...
                if timelag2<0
                    pn(m,h,n,k)=UInd_KCItestnb(sq(fangle(lfp(:,tb+1+timelag+timelag2,n,k))), sq(fangle(lfp(:,tb+1+timelag+timelag2,n,k))).*sq(conj(rlfp(:,tb+1+timelag,n))));
                else
                    pn(m,h,n,k)=UInd_KCItestnb(sq(conj(rlfp(:,tb+1+timelag,n))), sq(fangle(lfp(:,tb+1+timelag+timelag2,n,k))).*sq(conj(rlfp(:,tb+1+timelag,n))));
                end
